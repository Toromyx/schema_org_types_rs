...
/// This trait is for properties from <https://schema.org/PostOffice>.
pub trait PostOfficeTrait {}
impl PostOfficeTrait for PostOffice {}
impl GovernmentOfficeTrait for PostOffice {}
impl LocalBusinessTrait for PostOffice {
...
impl ThingTrait for PostOffice {
...
mod serde {
	...
	impl Serialize for PostOffice {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "PostOffice", len)?;
	...
	impl<'de> Deserialize<'de> for PostOffice {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = PostOffice;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema PostOffice")
				}
			...
			deserializer.deserialize_struct("PostOffice", FIELDS, ClassVisitor)
		}
	}
}
