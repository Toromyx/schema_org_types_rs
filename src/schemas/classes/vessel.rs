...
/// This trait is for properties from <https://schema.org/Vessel>.
pub trait VesselTrait {}
impl VesselTrait for Vessel {}
impl AnatomicalStructureTrait for Vessel {
...
impl MedicalEntityTrait for Vessel {
...
mod serde {
	...
	impl Serialize for Vessel {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Vessel", len)?;
	...
	impl<'de> Deserialize<'de> for Vessel {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Vessel;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Vessel")
				}
			...
			deserializer.deserialize_struct("Vessel", FIELDS, ClassVisitor)
		}
	}
}
