...
/// This trait is for properties from <https://schema.org/BusStop>.
pub trait BusStopTrait {}
impl BusStopTrait for BusStop {}
impl CivicStructureTrait for BusStop {
...
mod serde {
	...
	impl Serialize for BusStop {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "BusStop", len)?;
	...
	impl<'de> Deserialize<'de> for BusStop {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = BusStop;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema BusStop")
				}
			...
			deserializer.deserialize_struct("BusStop", FIELDS, ClassVisitor)
		}
	}
}
