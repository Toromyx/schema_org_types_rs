...
pub trait Model3DTrait {
	/// Get <https://schema.org/isResizable> from [`Self`] as borrowed slice.
	fn get_is_resizable(&self) -> &[IsResizableProperty];
	/// Take <https://schema.org/isResizable> from [`Self`] as owned vector.
	fn take_is_resizable(&mut self) -> Vec<IsResizableProperty>;
}
impl Model3DTrait for Model3D {
	fn get_is_resizable(&self) -> &[IsResizableProperty] {
		self.r#is_resizable.as_slice()
	}
	fn take_is_resizable(&mut self) -> Vec<IsResizableProperty> {
		std::mem::take(&mut self.r#is_resizable)
	}
}
impl CreativeWorkTrait for Model3D {
...
impl MediaObjectTrait for Model3D {
...
mod serde {
	...
	use super::*;
	impl Serialize for Model3D {
		fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
		where
			S: Serializer,
		{
			let len: usize = [
				!Vec::is_empty(&self.r#is_resizable) as usize,
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Model3D", len)?;
			if !Vec::is_empty(&self.r#is_resizable) {
				serialize_struct.serialize_field("isResizable", {
					struct SerializeWith<'a>(&'a Vec<IsResizableProperty>);
					...
					&SerializeWith(&self.r#is_resizable)
				})?;
			} else {
				serialize_struct.skip_field("isResizable")?;
			}
	...
	impl<'de> Deserialize<'de> for Model3D {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
				...
				{
					match value {
						"isResizable" => Ok(Field::IsResizable),
				...
				{
					match value {
						b"isResizable" => Ok(Field::IsResizable),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Model3D;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Model3D")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#is_resizable_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::IsResizable => {
								if r#is_resizable_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"isResizable",
									));
								}
								r#is_resizable_property = Some({
									struct DeserializeWith(Vec<IsResizableProperty>);
					...
					Ok(Model3D {
						r#is_resizable: r#is_resizable_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("Model3D", FIELDS, ClassVisitor)
		}
	}
}
