...
/// This trait is for properties from <https://schema.org/Residence>.
pub trait ResidenceTrait {
...
impl ResidenceTrait for Residence {
	fn get_accommodation_floor_plan(&self) -> &[AccommodationFloorPlanProperty] {
...
mod serde {
	...
	impl Serialize for Residence {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Residence", len)?;
	...
	impl<'de> Deserialize<'de> for Residence {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Residence;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Residence")
				}
			...
			deserializer.deserialize_struct("Residence", FIELDS, ClassVisitor)
		}
	}
}
