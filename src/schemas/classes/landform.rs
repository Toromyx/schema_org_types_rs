...
/// This trait is for properties from <https://schema.org/Landform>.
pub trait LandformTrait {}
impl LandformTrait for Landform {}
...
mod serde {
	...
	impl Serialize for Landform {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Landform", len)?;
	...
	impl<'de> Deserialize<'de> for Landform {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Landform;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Landform")
				}
			...
			deserializer.deserialize_struct("Landform", FIELDS, ClassVisitor)
		}
	}
}
