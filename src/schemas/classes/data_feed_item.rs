...
impl DataFeedItemTrait for DataFeedItem {
	...
	fn get_date_deleted(&self) -> &[DateDeletedProperty] {
		self.r#date_deleted.as_slice()
	}
	fn take_date_deleted(&mut self) -> Vec<DateDeletedProperty> {
		std::mem::take(&mut self.r#date_deleted)
	}
	fn get_date_modified(&self) -> &[DateModifiedProperty] {
...
impl ThingTrait for DataFeedItem {
...
mod serde {
	...
	use super::*;
	impl Serialize for DataFeedItem {
			...
			S: Serializer,
		{
			let len: usize = [
				...
				!Vec::is_empty(&self.r#date_deleted) as usize,
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "DataFeedItem", len)?;
			...
			if !Vec::is_empty(&self.r#date_deleted) {
				serialize_struct.serialize_field("dateDeleted", {
					struct SerializeWith<'a>(&'a Vec<DateDeletedProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#date_deleted)
				})?;
			} else {
				serialize_struct.skip_field("dateDeleted")?;
			}
	...
	impl<'de> Deserialize<'de> for DataFeedItem {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
					...
					E: de::Error,
				{
					match value {
						...
						"dateDeleted" => Ok(Field::DateDeleted),
					...
					E: de::Error,
				{
					match value {
						...
						b"dateDeleted" => Ok(Field::DateDeleted),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = DataFeedItem;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema DataFeedItem")
				}
					...
					A: de::MapAccess<'de>,
				{
					...
					let mut r#date_deleted_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							...
							Field::DateDeleted => {
								if r#date_deleted_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"dateDeleted",
									));
								}
								r#date_deleted_property = Some({
									struct DeserializeWith(Vec<DateDeletedProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
					...
					Ok(DataFeedItem {
						...
						r#date_deleted: r#date_deleted_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("DataFeedItem", FIELDS, ClassVisitor)
		}
	}
}
