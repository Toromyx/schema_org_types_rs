...
impl CreativeWorkTrait for HowTo {
...
mod serde {
	...
	impl Serialize for HowTo {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "HowTo", len)?;
	...
	impl<'de> Deserialize<'de> for HowTo {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = HowTo;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema HowTo")
				}
			...
			deserializer.deserialize_struct("HowTo", FIELDS, ClassVisitor)
		}
	}
}
