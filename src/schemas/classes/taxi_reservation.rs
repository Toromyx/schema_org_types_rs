...
impl TaxiReservationTrait for TaxiReservation {
	fn get_party_size(&self) -> &[PartySizeProperty] {
		self.r#party_size.as_slice()
	}
	fn take_party_size(&mut self) -> Vec<PartySizeProperty> {
		std::mem::take(&mut self.r#party_size)
	}
	fn get_pickup_location(&self) -> &[PickupLocationProperty] {
...
mod serde {
	...
	use super::*;
	impl Serialize for TaxiReservation {
			...
			S: Serializer,
		{
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "TaxiReservation", len)?;
	...
	impl<'de> Deserialize<'de> for TaxiReservation {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = TaxiReservation;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema TaxiReservation")
				}
			...
			deserializer.deserialize_struct("TaxiReservation", FIELDS, ClassVisitor)
		}
	}
}
