	...
	impl Serialize for CreativeWork {
			...
			S: Serializer,
		{
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "CreativeWork", len)?;
	...
	impl<'de> Deserialize<'de> for CreativeWork {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = CreativeWork;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema CreativeWork")
				}
			...
			deserializer.deserialize_struct("CreativeWork", FIELDS, ClassVisitor)
		}
	}
}
