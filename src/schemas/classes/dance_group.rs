...
/// This trait is for properties from <https://schema.org/DanceGroup>.
pub trait DanceGroupTrait {}
impl DanceGroupTrait for DanceGroup {}
...
impl PerformingGroupTrait for DanceGroup {}
impl ThingTrait for DanceGroup {
...
mod serde {
	...
	impl Serialize for DanceGroup {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "DanceGroup", len)?;
	...
	impl<'de> Deserialize<'de> for DanceGroup {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = DanceGroup;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema DanceGroup")
				}
			...
			deserializer.deserialize_struct("DanceGroup", FIELDS, ClassVisitor)
		}
	}
}
