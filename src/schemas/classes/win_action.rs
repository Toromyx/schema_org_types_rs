...
impl WinActionTrait for WinAction {
	fn get_loser(&self) -> &[LoserProperty] {
		self.r#loser.as_slice()
	}
	fn take_loser(&mut self) -> Vec<LoserProperty> {
		std::mem::take(&mut self.r#loser)
	}
}
impl AchieveActionTrait for WinAction {}
...
mod serde {
	...
	use super::*;
	impl Serialize for WinAction {
		fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
		where
			S: Serializer,
		{
			let len: usize = [
				!Vec::is_empty(&self.r#loser) as usize,
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "WinAction", len)?;
			if !Vec::is_empty(&self.r#loser) {
				serialize_struct.serialize_field("loser", {
					struct SerializeWith<'a>(&'a Vec<LoserProperty>);
					...
					&SerializeWith(&self.r#loser)
				})?;
			} else {
				serialize_struct.skip_field("loser")?;
			}
	...
	impl<'de> Deserialize<'de> for WinAction {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = WinAction;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema WinAction")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#loser_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::Loser => {
								if r#loser_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field("loser"));
								}
								r#loser_property = Some({
									struct DeserializeWith(Vec<LoserProperty>);
					...
					Ok(WinAction {
						r#loser: r#loser_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("WinAction", FIELDS, ClassVisitor)
		}
	}
}
