...
/// This trait is for properties from <https://schema.org/BikeStore>.
pub trait BikeStoreTrait {}
impl BikeStoreTrait for BikeStore {}
impl LocalBusinessTrait for BikeStore {
...
impl StoreTrait for BikeStore {}
impl ThingTrait for BikeStore {
...
mod serde {
	...
	impl Serialize for BikeStore {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "BikeStore", len)?;
	...
	impl<'de> Deserialize<'de> for BikeStore {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = BikeStore;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema BikeStore")
				}
			...
			deserializer.deserialize_struct("BikeStore", FIELDS, ClassVisitor)
		}
	}
}
