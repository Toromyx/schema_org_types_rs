...
/// This trait is for properties from <https://schema.org/Room>.
pub trait RoomTrait {}
impl RoomTrait for Room {}
impl AccommodationTrait for Room {
...
mod serde {
	...
	impl Serialize for Room {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Room", len)?;
	...
	impl<'de> Deserialize<'de> for Room {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Room;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Room")
				}
			...
			deserializer.deserialize_struct("Room", FIELDS, ClassVisitor)
		}
	}
}
