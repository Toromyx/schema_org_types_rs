...
pub trait ThesisTrait {
	/// Get <https://schema.org/inSupportOf> from [`Self`] as borrowed slice.
	fn get_in_support_of(&self) -> &[InSupportOfProperty];
	/// Take <https://schema.org/inSupportOf> from [`Self`] as owned vector.
	fn take_in_support_of(&mut self) -> Vec<InSupportOfProperty>;
}
impl ThesisTrait for Thesis {
	fn get_in_support_of(&self) -> &[InSupportOfProperty] {
		self.r#in_support_of.as_slice()
	}
	fn take_in_support_of(&mut self) -> Vec<InSupportOfProperty> {
		std::mem::take(&mut self.r#in_support_of)
	}
}
impl CreativeWorkTrait for Thesis {
...
mod serde {
	...
	impl Serialize for Thesis {
		...
		{
			let len: usize = [
				!Vec::is_empty(&self.r#in_support_of) as usize,
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Thesis", len)?;
			if !Vec::is_empty(&self.r#in_support_of) {
				serialize_struct.serialize_field("inSupportOf", {
					struct SerializeWith<'a>(&'a Vec<InSupportOfProperty>);
					...
					&SerializeWith(&self.r#in_support_of)
				})?;
			} else {
				serialize_struct.skip_field("inSupportOf")?;
			}
	...
	impl<'de> Deserialize<'de> for Thesis {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
				...
				{
					match value {
						"inSupportOf" => Ok(Field::InSupportOf),
				...
				{
					match value {
						b"inSupportOf" => Ok(Field::InSupportOf),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Thesis;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Thesis")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#in_support_of_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::InSupportOf => {
								if r#in_support_of_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"inSupportOf",
									));
								}
								r#in_support_of_property = Some({
									struct DeserializeWith(Vec<InSupportOfProperty>);
					...
					Ok(Thesis {
						r#in_support_of: r#in_support_of_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("Thesis", FIELDS, ClassVisitor)
		}
	}
}
