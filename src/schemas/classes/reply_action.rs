...
/// This trait is for properties from <https://schema.org/ReplyAction>.
pub trait ReplyActionTrait {
	/// Get <https://schema.org/resultComment> from [`Self`] as borrowed slice.
	fn get_result_comment(&self) -> &[ResultCommentProperty];
	/// Take <https://schema.org/resultComment> from [`Self`] as owned vector.
	fn take_result_comment(&mut self) -> Vec<ResultCommentProperty>;
}
impl ReplyActionTrait for ReplyAction {
	fn get_result_comment(&self) -> &[ResultCommentProperty] {
		self.r#result_comment.as_slice()
	}
	fn take_result_comment(&mut self) -> Vec<ResultCommentProperty> {
		std::mem::take(&mut self.r#result_comment)
	}
}
...
impl CommunicateActionTrait for ReplyAction {
...
impl InteractActionTrait for ReplyAction {}
impl ThingTrait for ReplyAction {
...
mod serde {
	...
	use super::*;
	impl Serialize for ReplyAction {
		fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
		where
			S: Serializer,
		{
			let len: usize = [
				!Vec::is_empty(&self.r#result_comment) as usize,
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "ReplyAction", len)?;
			if !Vec::is_empty(&self.r#result_comment) {
				serialize_struct.serialize_field("resultComment", {
					struct SerializeWith<'a>(&'a Vec<ResultCommentProperty>);
					...
					&SerializeWith(&self.r#result_comment)
				})?;
			} else {
				serialize_struct.skip_field("resultComment")?;
			}
	...
	impl<'de> Deserialize<'de> for ReplyAction {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
				...
				{
					match value {
						"resultComment" => Ok(Field::ResultComment),
				...
				{
					match value {
						b"resultComment" => Ok(Field::ResultComment),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = ReplyAction;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema ReplyAction")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#result_comment_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::ResultComment => {
								if r#result_comment_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"resultComment",
									));
								}
								r#result_comment_property = Some({
									struct DeserializeWith(Vec<ResultCommentProperty>);
					...
					Ok(ReplyAction {
						r#result_comment: r#result_comment_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("ReplyAction", FIELDS, ClassVisitor)
		}
	}
}
