...
pub struct MedicalTestPanel {
	...
	pub r#sub_test: Vec<SubTestProperty>,
...
/// This trait is for properties from <https://schema.org/MedicalTestPanel>.
pub trait MedicalTestPanelTrait {
	/// Get <https://schema.org/subTest> from [`Self`] as borrowed slice.
	fn get_sub_test(&self) -> &[SubTestProperty];
	/// Take <https://schema.org/subTest> from [`Self`] as owned vector.
	fn take_sub_test(&mut self) -> Vec<SubTestProperty>;
}
impl MedicalTestPanelTrait for MedicalTestPanel {
	fn get_sub_test(&self) -> &[SubTestProperty] {
		self.r#sub_test.as_slice()
	}
	fn take_sub_test(&mut self) -> Vec<SubTestProperty> {
		std::mem::take(&mut self.r#sub_test)
	}
}
impl MedicalEntityTrait for MedicalTestPanel {
...
impl MedicalTestTrait for MedicalTestPanel {
...
impl ThingTrait for MedicalTestPanel {
...
mod serde {
	...
	use super::*;
	impl Serialize for MedicalTestPanel {
		fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
		where
			S: Serializer,
		{
			let len: usize = [
				!Vec::is_empty(&self.r#sub_test) as usize,
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "MedicalTestPanel", len)?;
			if !Vec::is_empty(&self.r#sub_test) {
				serialize_struct.serialize_field("subTest", {
					struct SerializeWith<'a>(&'a Vec<SubTestProperty>);
					...
					&SerializeWith(&self.r#sub_test)
				})?;
			} else {
				serialize_struct.skip_field("subTest")?;
			}
	...
	impl<'de> Deserialize<'de> for MedicalTestPanel {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = MedicalTestPanel;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema MedicalTestPanel")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#sub_test_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::SubTest => {
								if r#sub_test_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"subTest",
									));
								}
								r#sub_test_property = Some({
									struct DeserializeWith(Vec<SubTestProperty>);
					...
					Ok(MedicalTestPanel {
						r#sub_test: r#sub_test_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("MedicalTestPanel", FIELDS, ClassVisitor)
		}
	}
}
