...
/// This trait is for properties from <https://schema.org/SheetMusic>.
pub trait SheetMusicTrait {}
impl SheetMusicTrait for SheetMusic {}
impl CreativeWorkTrait for SheetMusic {
...
impl ThingTrait for SheetMusic {
...
mod serde {
	...
	impl Serialize for SheetMusic {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "SheetMusic", len)?;
	...
	impl<'de> Deserialize<'de> for SheetMusic {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = SheetMusic;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema SheetMusic")
				}
			...
			deserializer.deserialize_struct("SheetMusic", FIELDS, ClassVisitor)
		}
	}
}
