...
impl EducationalAudienceTrait for EducationalAudience {
	fn get_educational_role(&self) -> &[EducationalRoleProperty] {
		self.r#educational_role.as_slice()
	}
	fn take_educational_role(&mut self) -> Vec<EducationalRoleProperty> {
		std::mem::take(&mut self.r#educational_role)
	}
}
impl AudienceTrait for EducationalAudience {
...
impl ThingTrait for EducationalAudience {
...
mod serde {
	...
	use super::*;
	impl Serialize for EducationalAudience {
		fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
		where
			S: Serializer,
		{
			let len: usize = [
				!Vec::is_empty(&self.r#educational_role) as usize,
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "EducationalAudience", len)?;
			if !Vec::is_empty(&self.r#educational_role) {
				serialize_struct.serialize_field("educationalRole", {
					struct SerializeWith<'a>(&'a Vec<EducationalRoleProperty>);
					...
					&SerializeWith(&self.r#educational_role)
				})?;
			} else {
				serialize_struct.skip_field("educationalRole")?;
			}
	...
	impl<'de> Deserialize<'de> for EducationalAudience {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
				...
				{
					match value {
						"educationalRole" => Ok(Field::EducationalRole),
				...
				{
					match value {
						b"educationalRole" => Ok(Field::EducationalRole),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = EducationalAudience;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema EducationalAudience")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#educational_role_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::EducationalRole => {
								if r#educational_role_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"educationalRole",
									));
								}
								r#educational_role_property = Some({
									struct DeserializeWith(Vec<EducationalRoleProperty>);
					...
					Ok(EducationalAudience {
						r#educational_role: r#educational_role_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("EducationalAudience", FIELDS, ClassVisitor)
		}
	}
}
