...
mod serde {
	...
	use super::*;
	impl Serialize for MedicalSymptom {
			...
			S: Serializer,
		{
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "MedicalSymptom", len)?;
	...
	impl<'de> Deserialize<'de> for MedicalSymptom {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = MedicalSymptom;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema MedicalSymptom")
				}
			...
			deserializer.deserialize_struct("MedicalSymptom", FIELDS, ClassVisitor)
		}
	}
}
