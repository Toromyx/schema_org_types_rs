...
impl SubstanceTrait for Substance {
	fn get_active_ingredient(&self) -> &[ActiveIngredientProperty] {
...
impl MedicalEntityTrait for Substance {
...
mod serde {
	...
	impl Serialize for Substance {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Substance", len)?;
	...
	impl<'de> Deserialize<'de> for Substance {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				...
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Substance")
				}
			...
			deserializer.deserialize_struct("Substance", FIELDS, ClassVisitor)
		}
	}
}
