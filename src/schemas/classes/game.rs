...
impl CreativeWorkTrait for Game {
...
mod serde {
	...
	impl Serialize for Game {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Game", len)?;
	...
	impl<'de> Deserialize<'de> for Game {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Game;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Game")
				}
			...
			deserializer.deserialize_struct("Game", FIELDS, ClassVisitor)
		}
	}
}
