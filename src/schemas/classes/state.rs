...
/// This trait is for properties from <https://schema.org/State>.
pub trait StateTrait {}
impl StateTrait for State {}
impl AdministrativeAreaTrait for State {}
...
mod serde {
	...
	impl Serialize for State {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "State", len)?;
	...
	impl<'de> Deserialize<'de> for State {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = State;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema State")
				}
			...
			deserializer.deserialize_struct("State", FIELDS, ClassVisitor)
		}
	}
}
