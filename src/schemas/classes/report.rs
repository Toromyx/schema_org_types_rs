...
/// This trait is for properties from <https://schema.org/Report>.
pub trait ReportTrait {
	/// Get <https://schema.org/reportNumber> from [`Self`] as borrowed slice.
	fn get_report_number(&self) -> &[ReportNumberProperty];
	/// Take <https://schema.org/reportNumber> from [`Self`] as owned vector.
	fn take_report_number(&mut self) -> Vec<ReportNumberProperty>;
}
impl ReportTrait for Report {
	fn get_report_number(&self) -> &[ReportNumberProperty] {
		self.r#report_number.as_slice()
	}
	fn take_report_number(&mut self) -> Vec<ReportNumberProperty> {
		std::mem::take(&mut self.r#report_number)
	}
}
impl ArticleTrait for Report {
...
impl CreativeWorkTrait for Report {
...
mod serde {
	...
	impl Serialize for Report {
		...
		{
			let len: usize = [
				!Vec::is_empty(&self.r#report_number) as usize,
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Report", len)?;
			if !Vec::is_empty(&self.r#report_number) {
				serialize_struct.serialize_field("reportNumber", {
					struct SerializeWith<'a>(&'a Vec<ReportNumberProperty>);
					...
					&SerializeWith(&self.r#report_number)
				})?;
			} else {
				serialize_struct.skip_field("reportNumber")?;
			}
	...
	impl<'de> Deserialize<'de> for Report {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
				...
				{
					match value {
						"reportNumber" => Ok(Field::ReportNumber),
				...
				{
					match value {
						b"reportNumber" => Ok(Field::ReportNumber),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Report;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Report")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#report_number_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::ReportNumber => {
								if r#report_number_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"reportNumber",
									));
								}
								r#report_number_property = Some({
									struct DeserializeWith(Vec<ReportNumberProperty>);
					...
					Ok(Report {
						r#report_number: r#report_number_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("Report", FIELDS, ClassVisitor)
		}
	}
}
