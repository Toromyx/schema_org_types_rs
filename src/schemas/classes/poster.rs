...
/// This trait is for properties from <https://schema.org/Poster>.
pub trait PosterTrait {}
impl PosterTrait for Poster {}
impl CreativeWorkTrait for Poster {
...
mod serde {
	...
	impl Serialize for Poster {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Poster", len)?;
	...
	impl<'de> Deserialize<'de> for Poster {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Poster;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Poster")
				}
			...
			deserializer.deserialize_struct("Poster", FIELDS, ClassVisitor)
		}
	}
}
