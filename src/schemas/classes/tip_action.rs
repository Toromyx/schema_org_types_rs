...
/// This trait is for properties from <https://schema.org/TipAction>.
pub trait TipActionTrait {
...
impl TipActionTrait for TipAction {
	fn get_recipient(&self) -> &[RecipientProperty] {
...
impl TradeActionTrait for TipAction {
...
mod serde {
	...
	impl Serialize for TipAction {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "TipAction", len)?;
	...
	impl<'de> Deserialize<'de> for TipAction {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = TipAction;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema TipAction")
				}
			...
			deserializer.deserialize_struct("TipAction", FIELDS, ClassVisitor)
		}
	}
}
