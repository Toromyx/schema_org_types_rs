...
/// This trait is for properties from <https://schema.org/AmpStory>.
pub trait AmpStoryTrait {}
impl AmpStoryTrait for AmpStory {}
impl CreativeWorkTrait for AmpStory {
...
impl MediaObjectTrait for AmpStory {
...
mod serde {
	...
	impl Serialize for AmpStory {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "AmpStory", len)?;
	...
	impl<'de> Deserialize<'de> for AmpStory {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = AmpStory;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema AmpStory")
				}
			...
			deserializer.deserialize_struct("AmpStory", FIELDS, ClassVisitor)
		}
	}
}
