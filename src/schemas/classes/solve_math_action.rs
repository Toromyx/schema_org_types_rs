...
impl SolveMathActionTrait for SolveMathAction {
	fn get_edu_question_type(&self) -> &[EduQuestionTypeProperty] {
...
impl ThingTrait for SolveMathAction {
...
mod serde {
	...
	use super::*;
	impl Serialize for SolveMathAction {
			...
			S: Serializer,
		{
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "SolveMathAction", len)?;
	...
	impl<'de> Deserialize<'de> for SolveMathAction {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = SolveMathAction;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema SolveMathAction")
				}
			...
			deserializer.deserialize_struct("SolveMathAction", FIELDS, ClassVisitor)
		}
	}
}
