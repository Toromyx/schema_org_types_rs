...
pub trait CorporationTrait {
	/// Get <https://schema.org/tickerSymbol> from [`Self`] as borrowed slice.
	fn get_ticker_symbol(&self) -> &[TickerSymbolProperty];
	/// Take <https://schema.org/tickerSymbol> from [`Self`] as owned vector.
	fn take_ticker_symbol(&mut self) -> Vec<TickerSymbolProperty>;
}
impl CorporationTrait for Corporation {
	fn get_ticker_symbol(&self) -> &[TickerSymbolProperty] {
		self.r#ticker_symbol.as_slice()
	}
	fn take_ticker_symbol(&mut self) -> Vec<TickerSymbolProperty> {
		std::mem::take(&mut self.r#ticker_symbol)
	}
}
...
impl ThingTrait for Corporation {
