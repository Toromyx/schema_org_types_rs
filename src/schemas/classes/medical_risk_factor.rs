...
pub struct MedicalRiskFactor {
	...
	pub r#increases_risk_of: Vec<IncreasesRiskOfProperty>,
...
pub trait MedicalRiskFactorTrait {
	/// Get <https://schema.org/increasesRiskOf> from [`Self`] as borrowed slice.
	fn get_increases_risk_of(&self) -> &[IncreasesRiskOfProperty];
	/// Take <https://schema.org/increasesRiskOf> from [`Self`] as owned vector.
	fn take_increases_risk_of(&mut self) -> Vec<IncreasesRiskOfProperty>;
}
impl MedicalRiskFactorTrait for MedicalRiskFactor {
	fn get_increases_risk_of(&self) -> &[IncreasesRiskOfProperty] {
		self.r#increases_risk_of.as_slice()
	}
	fn take_increases_risk_of(&mut self) -> Vec<IncreasesRiskOfProperty> {
		std::mem::take(&mut self.r#increases_risk_of)
	}
}
impl MedicalEntityTrait for MedicalRiskFactor {
...
impl ThingTrait for MedicalRiskFactor {
...
mod serde {
	...
	use super::*;
	impl Serialize for MedicalRiskFactor {
		fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
		where
			S: Serializer,
		{
			let len: usize = [
				!Vec::is_empty(&self.r#increases_risk_of) as usize,
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "MedicalRiskFactor", len)?;
			if !Vec::is_empty(&self.r#increases_risk_of) {
				serialize_struct.serialize_field("increasesRiskOf", {
					struct SerializeWith<'a>(&'a Vec<IncreasesRiskOfProperty>);
					...
					&SerializeWith(&self.r#increases_risk_of)
				})?;
			} else {
				serialize_struct.skip_field("increasesRiskOf")?;
			}
	...
	impl<'de> Deserialize<'de> for MedicalRiskFactor {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
				...
				{
					match value {
						"increasesRiskOf" => Ok(Field::IncreasesRiskOf),
						"code" => Ok(Field::Code),
				...
				{
					match value {
						b"increasesRiskOf" => Ok(Field::IncreasesRiskOf),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = MedicalRiskFactor;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema MedicalRiskFactor")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#increases_risk_of_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::IncreasesRiskOf => {
								if r#increases_risk_of_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"increasesRiskOf",
									));
								}
								r#increases_risk_of_property = Some({
									struct DeserializeWith(Vec<IncreasesRiskOfProperty>);
					...
					Ok(MedicalRiskFactor {
						r#increases_risk_of: r#increases_risk_of_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("MedicalRiskFactor", FIELDS, ClassVisitor)
		}
	}
}
