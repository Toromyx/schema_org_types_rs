...
/// This trait is for properties from <https://schema.org/UserLikes>.
pub trait UserLikesTrait {}
impl UserLikesTrait for UserLikes {}
impl EventTrait for UserLikes {
...
impl UserInteractionTrait for UserLikes {}
#[cfg(feature = "serde")]
mod serde {
	...
	impl Serialize for UserLikes {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "UserLikes", len)?;
	...
	impl<'de> Deserialize<'de> for UserLikes {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = UserLikes;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema UserLikes")
				}
			...
			deserializer.deserialize_struct("UserLikes", FIELDS, ClassVisitor)
		}
	}
}
