...
/// This trait is for properties from <https://schema.org/MenuSection>.
pub trait MenuSectionTrait {
	/// Get <https://schema.org/hasMenuItem> from [`Self`] as borrowed slice.
	fn get_has_menu_item(&self) -> &[HasMenuItemProperty];
	/// Take <https://schema.org/hasMenuItem> from [`Self`] as owned vector.
	fn take_has_menu_item(&mut self) -> Vec<HasMenuItemProperty>;
	/// Get <https://schema.org/hasMenuSection> from [`Self`] as borrowed slice.
	fn get_has_menu_section(&self) -> &[HasMenuSectionProperty];
	/// Take <https://schema.org/hasMenuSection> from [`Self`] as owned vector.
	fn take_has_menu_section(&mut self) -> Vec<HasMenuSectionProperty>;
}
impl MenuSectionTrait for MenuSection {
	fn get_has_menu_item(&self) -> &[HasMenuItemProperty] {
		self.r#has_menu_item.as_slice()
	}
	fn take_has_menu_item(&mut self) -> Vec<HasMenuItemProperty> {
		std::mem::take(&mut self.r#has_menu_item)
	}
	fn get_has_menu_section(&self) -> &[HasMenuSectionProperty] {
		self.r#has_menu_section.as_slice()
	}
	fn take_has_menu_section(&mut self) -> Vec<HasMenuSectionProperty> {
		std::mem::take(&mut self.r#has_menu_section)
	}
}
impl CreativeWorkTrait for MenuSection {
...
impl ThingTrait for MenuSection {
...
mod serde {
	...
	use super::*;
	impl Serialize for MenuSection {
		fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
		where
			S: Serializer,
		{
			let len: usize = [
				!Vec::is_empty(&self.r#has_menu_item) as usize,
				!Vec::is_empty(&self.r#has_menu_section) as usize,
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "MenuSection", len)?;
			if !Vec::is_empty(&self.r#has_menu_item) {
				serialize_struct.serialize_field("hasMenuItem", {
					struct SerializeWith<'a>(&'a Vec<HasMenuItemProperty>);
					...
					&SerializeWith(&self.r#has_menu_item)
				})?;
			} else {
				serialize_struct.skip_field("hasMenuItem")?;
			}
			if !Vec::is_empty(&self.r#has_menu_section) {
				serialize_struct.serialize_field("hasMenuSection", {
					struct SerializeWith<'a>(&'a Vec<HasMenuSectionProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#has_menu_section)
				})?;
			} else {
				serialize_struct.skip_field("hasMenuSection")?;
			}
	...
	impl<'de> Deserialize<'de> for MenuSection {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
				...
				{
					match value {
						"hasMenuItem" => Ok(Field::HasMenuItem),
						"hasMenuSection" => Ok(Field::HasMenuSection),
				...
				{
					match value {
						b"hasMenuItem" => Ok(Field::HasMenuItem),
						b"hasMenuSection" => Ok(Field::HasMenuSection),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				...
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema MenuSection")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#has_menu_item_property = None;
					let mut r#has_menu_section_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::HasMenuItem => {
								if r#has_menu_item_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"hasMenuItem",
									));
								}
								r#has_menu_item_property = Some({
									struct DeserializeWith(Vec<HasMenuItemProperty>);
							...
							Field::HasMenuSection => {
								if r#has_menu_section_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"hasMenuSection",
									));
								}
								r#has_menu_section_property = Some({
									struct DeserializeWith(Vec<HasMenuSectionProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
					...
					Ok(MenuSection {
						r#has_menu_item: r#has_menu_item_property.unwrap_or_default(),
						r#has_menu_section: r#has_menu_section_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("MenuSection", FIELDS, ClassVisitor)
		}
	}
}
