...
/// This trait is for properties from <https://schema.org/Atlas>.
pub trait AtlasTrait {}
impl AtlasTrait for Atlas {}
impl CreativeWorkTrait for Atlas {
...
mod serde {
	...
	impl Serialize for Atlas {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Atlas", len)?;
	...
	impl<'de> Deserialize<'de> for Atlas {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Atlas;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Atlas")
				}
			...
			deserializer.deserialize_struct("Atlas", FIELDS, ClassVisitor)
		}
	}
}
