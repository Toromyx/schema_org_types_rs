...
pub struct LoseAction {
	pub r#winner: Vec<WinnerProperty>,
...
impl LoseActionTrait for LoseAction {
	fn get_winner(&self) -> &[WinnerProperty] {
		self.r#winner.as_slice()
	}
	fn take_winner(&mut self) -> Vec<WinnerProperty> {
		std::mem::take(&mut self.r#winner)
	}
}
impl AchieveActionTrait for LoseAction {}
...
impl ThingTrait for LoseAction {
...
mod serde {
	...
	use super::*;
	impl Serialize for LoseAction {
		fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
		where
			S: Serializer,
		{
			let len: usize = [
				!Vec::is_empty(&self.r#winner) as usize,
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "LoseAction", len)?;
			if !Vec::is_empty(&self.r#winner) {
				serialize_struct.serialize_field("winner", {
					struct SerializeWith<'a>(&'a Vec<WinnerProperty>);
					...
					&SerializeWith(&self.r#winner)
				})?;
			} else {
				serialize_struct.skip_field("winner")?;
			}
	...
	impl<'de> Deserialize<'de> for LoseAction {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = LoseAction;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema LoseAction")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#winner_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::Winner => {
								if r#winner_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field("winner"));
								}
								r#winner_property = Some({
									struct DeserializeWith(Vec<WinnerProperty>);
					...
					Ok(LoseAction {
						r#winner: r#winner_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("LoseAction", FIELDS, ClassVisitor)
		}
	}
}
