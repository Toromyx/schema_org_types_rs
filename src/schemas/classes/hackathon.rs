...
/// This trait is for properties from <https://schema.org/Hackathon>.
pub trait HackathonTrait {}
impl HackathonTrait for Hackathon {}
impl EventTrait for Hackathon {
...
mod serde {
	...
	impl Serialize for Hackathon {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Hackathon", len)?;
	...
	impl<'de> Deserialize<'de> for Hackathon {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Hackathon;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Hackathon")
				}
			...
			deserializer.deserialize_struct("Hackathon", FIELDS, ClassVisitor)
		}
	}
}
