...
pub trait ConstraintNodeTrait {
	/// Get <https://schema.org/constraintProperty> from [`Self`] as borrowed slice.
	fn get_constraint_property(&self) -> &[ConstraintPropertyProperty];
	/// Take <https://schema.org/constraintProperty> from [`Self`] as owned vector.
	fn take_constraint_property(&mut self) -> Vec<ConstraintPropertyProperty>;
	/// Get <https://schema.org/numConstraints> from [`Self`] as borrowed slice.
	fn get_num_constraints(&self) -> &[NumConstraintsProperty];
	/// Take <https://schema.org/numConstraints> from [`Self`] as owned vector.
	fn take_num_constraints(&mut self) -> Vec<NumConstraintsProperty>;
}
impl ConstraintNodeTrait for ConstraintNode {
	fn get_constraint_property(&self) -> &[ConstraintPropertyProperty] {
...
impl ThingTrait for ConstraintNode {
...
mod serde {
	...
	use super::*;
	impl Serialize for ConstraintNode {
			...
			S: Serializer,
		{
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "ConstraintNode", len)?;
	...
	impl<'de> Deserialize<'de> for ConstraintNode {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = ConstraintNode;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema ConstraintNode")
				}
			...
			deserializer.deserialize_struct("ConstraintNode", FIELDS, ClassVisitor)
		}
	}
}
