...
pub trait PublicationIssueTrait {
	/// Get <https://schema.org/issueNumber> from [`Self`] as borrowed slice.
	fn get_issue_number(&self) -> &[IssueNumberProperty];
	/// Take <https://schema.org/issueNumber> from [`Self`] as owned vector.
...
impl PublicationIssueTrait for PublicationIssue {
	fn get_issue_number(&self) -> &[IssueNumberProperty] {
...
impl CreativeWorkTrait for PublicationIssue {
...
impl ThingTrait for PublicationIssue {
...
mod serde {
	...
	use super::*;
	impl Serialize for PublicationIssue {
			...
			S: Serializer,
		{
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "PublicationIssue", len)?;
	...
	impl<'de> Deserialize<'de> for PublicationIssue {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = PublicationIssue;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema PublicationIssue")
				}
			...
			deserializer.deserialize_struct("PublicationIssue", FIELDS, ClassVisitor)
		}
	}
}
