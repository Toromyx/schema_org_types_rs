...
pub struct MobileApplication {
	...
	pub r#carrier_requirements: Vec<CarrierRequirementsProperty>,
...
/// This trait is for properties from <https://schema.org/MobileApplication>.
pub trait MobileApplicationTrait {
	/// Get <https://schema.org/carrierRequirements> from [`Self`] as borrowed slice.
	fn get_carrier_requirements(&self) -> &[CarrierRequirementsProperty];
	/// Take <https://schema.org/carrierRequirements> from [`Self`] as owned vector.
	fn take_carrier_requirements(&mut self) -> Vec<CarrierRequirementsProperty>;
}
impl MobileApplicationTrait for MobileApplication {
	fn get_carrier_requirements(&self) -> &[CarrierRequirementsProperty] {
		self.r#carrier_requirements.as_slice()
	}
	fn take_carrier_requirements(&mut self) -> Vec<CarrierRequirementsProperty> {
		std::mem::take(&mut self.r#carrier_requirements)
	}
}
impl CreativeWorkTrait for MobileApplication {
...
impl SoftwareApplicationTrait for MobileApplication {
...
impl ThingTrait for MobileApplication {
...
mod serde {
	...
	use super::*;
	impl Serialize for MobileApplication {
		fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
		where
			S: Serializer,
		{
			let len: usize = [
				!Vec::is_empty(&self.r#carrier_requirements) as usize,
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "MobileApplication", len)?;
			if !Vec::is_empty(&self.r#carrier_requirements) {
				serialize_struct.serialize_field("carrierRequirements", {
					struct SerializeWith<'a>(&'a Vec<CarrierRequirementsProperty>);
					...
					&SerializeWith(&self.r#carrier_requirements)
				})?;
			} else {
				serialize_struct.skip_field("carrierRequirements")?;
			}
	...
	impl<'de> Deserialize<'de> for MobileApplication {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
				...
				{
					match value {
						"carrierRequirements" => Ok(Field::CarrierRequirements),
				...
				{
					match value {
						b"carrierRequirements" => Ok(Field::CarrierRequirements),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = MobileApplication;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema MobileApplication")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#carrier_requirements_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::CarrierRequirements => {
								if r#carrier_requirements_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"carrierRequirements",
									));
								}
								r#carrier_requirements_property = Some({
									struct DeserializeWith(Vec<CarrierRequirementsProperty>);
					...
					Ok(MobileApplication {
						r#carrier_requirements: r#carrier_requirements_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("MobileApplication", FIELDS, ClassVisitor)
		}
	}
}
