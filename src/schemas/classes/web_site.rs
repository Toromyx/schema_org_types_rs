...
impl CreativeWorkTrait for WebSite {
...
mod serde {
	...
	impl Serialize for WebSite {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "WebSite", len)?;
	...
	impl<'de> Deserialize<'de> for WebSite {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = WebSite;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema WebSite")
				}
			...
			deserializer.deserialize_struct("WebSite", FIELDS, ClassVisitor)
		}
	}
}
