...
/// This trait is for properties from <https://schema.org/Quiz>.
pub trait QuizTrait {}
impl QuizTrait for Quiz {}
impl CreativeWorkTrait for Quiz {
...
impl LearningResourceTrait for Quiz {
	fn get_assesses(&self) -> &[AssessesProperty] {
		self.r#assesses.as_slice()
	}
	fn take_assesses(&mut self) -> Vec<AssessesProperty> {
...
mod serde {
	...
	impl Serialize for Quiz {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Quiz", len)?;
	...
	impl<'de> Deserialize<'de> for Quiz {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Quiz;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Quiz")
				}
			...
			deserializer.deserialize_struct("Quiz", FIELDS, ClassVisitor)
		}
	}
}
