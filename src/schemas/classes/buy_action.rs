...
impl BuyActionTrait for BuyAction {
	...
	fn get_vendor(&self) -> &[VendorProperty] {
		self.r#vendor.as_slice()
	}
	fn take_vendor(&mut self) -> Vec<VendorProperty> {
		std::mem::take(&mut self.r#vendor)
	}
	fn get_warranty_promise(&self) -> &[WarrantyPromiseProperty] {
...
impl TradeActionTrait for BuyAction {
...
mod serde {
	...
	impl Serialize for BuyAction {
			...
			S: Serializer,
		{
			let len: usize = [
				...
				!Vec::is_empty(&self.r#vendor) as usize,
				!Vec::is_empty(&self.r#warranty_promise) as usize,
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "BuyAction", len)?;
			...
			if !Vec::is_empty(&self.r#vendor) {
				serialize_struct.serialize_field("vendor", {
					struct SerializeWith<'a>(&'a Vec<VendorProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#vendor)
				})?;
			} else {
				serialize_struct.skip_field("vendor")?;
			}
			if !Vec::is_empty(&self.r#warranty_promise) {
				serialize_struct.serialize_field("warrantyPromise", {
					struct SerializeWith<'a>(&'a Vec<WarrantyPromiseProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#warranty_promise)
				})?;
			} else {
				serialize_struct.skip_field("warrantyPromise")?;
			}
	...
	impl<'de> Deserialize<'de> for BuyAction {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
					...
					E: de::Error,
				{
					match value {
						...
						"warrantyPromise" => Ok(Field::WarrantyPromise),
					...
					E: de::Error,
				{
					match value {
						...
						b"vendor" => Ok(Field::Vendor),
						b"warrantyPromise" => Ok(Field::WarrantyPromise),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = BuyAction;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema BuyAction")
				}
					...
					A: de::MapAccess<'de>,
				{
					...
					let mut r#vendor_property = None;
					let mut r#warranty_promise_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							...
							Field::Vendor => {
								if r#vendor_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field("vendor"));
								}
								r#vendor_property = Some({
									struct DeserializeWith(Vec<VendorProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
							...
							Field::WarrantyPromise => {
								if r#warranty_promise_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"warrantyPromise",
									));
								}
								r#warranty_promise_property = Some({
									struct DeserializeWith(Vec<WarrantyPromiseProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
					...
					Ok(BuyAction {
						...
						r#vendor: r#vendor_property.unwrap_or_default(),
						r#warranty_promise: r#warranty_promise_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("BuyAction", FIELDS, ClassVisitor)
		}
	}
}
