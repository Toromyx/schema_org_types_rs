...
impl CreativeWorkTrait for Menu {
...
mod serde {
	...
	impl Serialize for Menu {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Menu", len)?;
	...
	impl<'de> Deserialize<'de> for Menu {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Menu;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Menu")
				}
			...
			deserializer.deserialize_struct("Menu", FIELDS, ClassVisitor)
		}
	}
}
