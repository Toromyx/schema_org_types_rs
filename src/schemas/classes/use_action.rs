...
impl ConsumeActionTrait for UseAction {
...
mod serde {
	...
	impl Serialize for UseAction {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "UseAction", len)?;
	...
	impl<'de> Deserialize<'de> for UseAction {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = UseAction;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema UseAction")
				}
			...
			deserializer.deserialize_struct("UseAction", FIELDS, ClassVisitor)
		}
	}
}
