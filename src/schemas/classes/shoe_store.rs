...
/// This trait is for properties from <https://schema.org/ShoeStore>.
pub trait ShoeStoreTrait {}
impl ShoeStoreTrait for ShoeStore {}
impl LocalBusinessTrait for ShoeStore {
...
impl StoreTrait for ShoeStore {}
impl ThingTrait for ShoeStore {
...
mod serde {
	...
	impl Serialize for ShoeStore {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "ShoeStore", len)?;
	...
	impl<'de> Deserialize<'de> for ShoeStore {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = ShoeStore;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema ShoeStore")
				}
			...
			deserializer.deserialize_struct("ShoeStore", FIELDS, ClassVisitor)
		}
	}
}
