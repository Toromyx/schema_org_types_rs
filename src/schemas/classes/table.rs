...
/// This trait is for properties from <https://schema.org/Table>.
pub trait TableTrait {}
impl TableTrait for Table {}
impl CreativeWorkTrait for Table {
...
impl WebPageElementTrait for Table {
...
mod serde {
	...
	impl Serialize for Table {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Table", len)?;
	...
	impl<'de> Deserialize<'de> for Table {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Table;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Table")
				}
			...
			deserializer.deserialize_struct("Table", FIELDS, ClassVisitor)
		}
	}
}
