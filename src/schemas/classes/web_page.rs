...
mod serde {
	...
	impl Serialize for WebPage {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "WebPage", len)?;
	...
	impl<'de> Deserialize<'de> for WebPage {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = WebPage;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema WebPage")
				}
			...
			deserializer.deserialize_struct("WebPage", FIELDS, ClassVisitor)
		}
	}
}
