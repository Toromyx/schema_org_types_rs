...
mod serde {
	...
	use super::*;
	impl Serialize for ProductCollection {
			...
			S: Serializer,
		{
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "ProductCollection", len)?;
	...
	impl<'de> Deserialize<'de> for ProductCollection {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = ProductCollection;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema ProductCollection")
				}
			...
			deserializer.deserialize_struct("ProductCollection", FIELDS, ClassVisitor)
		}
	}
}
