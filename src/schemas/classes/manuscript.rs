...
mod serde {
	...
	impl Serialize for Manuscript {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Manuscript", len)?;
	...
	impl<'de> Deserialize<'de> for Manuscript {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Manuscript;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Manuscript")
				}
			...
			deserializer.deserialize_struct("Manuscript", FIELDS, ClassVisitor)
		}
	}
}
