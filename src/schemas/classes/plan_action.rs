...
pub struct PlanAction {
	...
	pub r#scheduled_time: Vec<ScheduledTimeProperty>,
...
/// This trait is for properties from <https://schema.org/PlanAction>.
pub trait PlanActionTrait {
	/// Get <https://schema.org/scheduledTime> from [`Self`] as borrowed slice.
	fn get_scheduled_time(&self) -> &[ScheduledTimeProperty];
	/// Take <https://schema.org/scheduledTime> from [`Self`] as owned vector.
	fn take_scheduled_time(&mut self) -> Vec<ScheduledTimeProperty>;
}
impl PlanActionTrait for PlanAction {
	fn get_scheduled_time(&self) -> &[ScheduledTimeProperty] {
		self.r#scheduled_time.as_slice()
	}
	fn take_scheduled_time(&mut self) -> Vec<ScheduledTimeProperty> {
		std::mem::take(&mut self.r#scheduled_time)
	}
}
...
impl OrganizeActionTrait for PlanAction {}
impl ThingTrait for PlanAction {
...
mod serde {
	...
	impl Serialize for PlanAction {
		...
		{
			let len: usize = [
				!Vec::is_empty(&self.r#scheduled_time) as usize,
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "PlanAction", len)?;
			if !Vec::is_empty(&self.r#scheduled_time) {
				serialize_struct.serialize_field("scheduledTime", {
					struct SerializeWith<'a>(&'a Vec<ScheduledTimeProperty>);
					...
					&SerializeWith(&self.r#scheduled_time)
				})?;
			} else {
				serialize_struct.skip_field("scheduledTime")?;
			}
	...
	impl<'de> Deserialize<'de> for PlanAction {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
				...
				{
					match value {
						"scheduledTime" => Ok(Field::ScheduledTime),
				...
				{
					match value {
						b"scheduledTime" => Ok(Field::ScheduledTime),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = PlanAction;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema PlanAction")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#scheduled_time_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::ScheduledTime => {
								if r#scheduled_time_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"scheduledTime",
									));
								}
								r#scheduled_time_property = Some({
									struct DeserializeWith(Vec<ScheduledTimeProperty>);
					...
					Ok(PlanAction {
						r#scheduled_time: r#scheduled_time_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("PlanAction", FIELDS, ClassVisitor)
		}
	}
}
