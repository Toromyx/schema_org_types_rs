...
/// This trait is for properties from <https://schema.org/Syllabus>.
pub trait SyllabusTrait {}
impl SyllabusTrait for Syllabus {}
impl CreativeWorkTrait for Syllabus {
...
impl LearningResourceTrait for Syllabus {
	fn get_assesses(&self) -> &[AssessesProperty] {
		self.r#assesses.as_slice()
	}
	fn take_assesses(&mut self) -> Vec<AssessesProperty> {
...
mod serde {
	...
	impl Serialize for Syllabus {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Syllabus", len)?;
	...
	impl<'de> Deserialize<'de> for Syllabus {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Syllabus;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Syllabus")
				}
			...
			deserializer.deserialize_struct("Syllabus", FIELDS, ClassVisitor)
		}
	}
}
