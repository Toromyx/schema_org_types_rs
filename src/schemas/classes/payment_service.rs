...
/// This trait is for properties from <https://schema.org/PaymentService>.
...
mod serde {
	...
	use super::*;
	impl Serialize for PaymentService {
			...
			S: Serializer,
		{
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "PaymentService", len)?;
	...
	impl<'de> Deserialize<'de> for PaymentService {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = PaymentService;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema PaymentService")
				}
			...
			deserializer.deserialize_struct("PaymentService", FIELDS, ClassVisitor)
		}
	}
}
