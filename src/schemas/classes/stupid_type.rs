...
impl ThingTrait for StupidType {
...
mod serde {
	...
	impl Serialize for StupidType {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "StupidType", len)?;
	...
	impl<'de> Deserialize<'de> for StupidType {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = StupidType;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema StupidType")
				}
			...
			deserializer.deserialize_struct("StupidType", FIELDS, ClassVisitor)
		}
	}
}
