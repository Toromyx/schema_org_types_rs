...
/// This trait is for properties from <https://schema.org/TaxiStand>.
pub trait TaxiStandTrait {}
impl TaxiStandTrait for TaxiStand {}
impl CivicStructureTrait for TaxiStand {
...
mod serde {
	...
	impl Serialize for TaxiStand {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "TaxiStand", len)?;
	...
	impl<'de> Deserialize<'de> for TaxiStand {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = TaxiStand;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema TaxiStand")
				}
			...
			deserializer.deserialize_struct("TaxiStand", FIELDS, ClassVisitor)
		}
	}
}
