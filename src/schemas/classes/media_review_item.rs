...
/// This trait is for properties from <https://schema.org/MediaReviewItem>.
pub trait MediaReviewItemTrait {
	/// Get <https://schema.org/mediaItemAppearance> from [`Self`] as borrowed slice.
	fn get_media_item_appearance(&self) -> &[MediaItemAppearanceProperty];
	/// Take <https://schema.org/mediaItemAppearance> from [`Self`] as owned vector.
	fn take_media_item_appearance(&mut self) -> Vec<MediaItemAppearanceProperty>;
}
impl MediaReviewItemTrait for MediaReviewItem {
	fn get_media_item_appearance(&self) -> &[MediaItemAppearanceProperty] {
		self.r#media_item_appearance.as_slice()
	}
	fn take_media_item_appearance(&mut self) -> Vec<MediaItemAppearanceProperty> {
		std::mem::take(&mut self.r#media_item_appearance)
	}
}
impl CreativeWorkTrait for MediaReviewItem {
...
impl ThingTrait for MediaReviewItem {
...
mod serde {
	...
	use super::*;
	impl Serialize for MediaReviewItem {
		fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
		where
			S: Serializer,
		{
			let len: usize = [
				!Vec::is_empty(&self.r#media_item_appearance) as usize,
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "MediaReviewItem", len)?;
			if !Vec::is_empty(&self.r#media_item_appearance) {
				serialize_struct.serialize_field("mediaItemAppearance", {
					struct SerializeWith<'a>(&'a Vec<MediaItemAppearanceProperty>);
					...
					&SerializeWith(&self.r#media_item_appearance)
				})?;
			} else {
				serialize_struct.skip_field("mediaItemAppearance")?;
			}
	...
	impl<'de> Deserialize<'de> for MediaReviewItem {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
				...
				{
					match value {
						"mediaItemAppearance" => Ok(Field::MediaItemAppearance),
				...
				{
					match value {
						b"mediaItemAppearance" => Ok(Field::MediaItemAppearance),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = MediaReviewItem;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema MediaReviewItem")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#media_item_appearance_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::MediaItemAppearance => {
								if r#media_item_appearance_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"mediaItemAppearance",
									));
								}
								r#media_item_appearance_property = Some({
									struct DeserializeWith(Vec<MediaItemAppearanceProperty>);
					...
					Ok(MediaReviewItem {
						r#media_item_appearance: r#media_item_appearance_property
							.unwrap_or_default(),
			...
			deserializer.deserialize_struct("MediaReviewItem", FIELDS, ClassVisitor)
		}
	}
}
