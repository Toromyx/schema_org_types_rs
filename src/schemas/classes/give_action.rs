...
/// This trait is for properties from <https://schema.org/GiveAction>.
pub trait GiveActionTrait {
...
impl GiveActionTrait for GiveAction {
	fn get_recipient(&self) -> &[RecipientProperty] {
...
impl ThingTrait for GiveAction {
...
impl TransferActionTrait for GiveAction {
...
mod serde {
	...
	impl Serialize for GiveAction {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "GiveAction", len)?;
	...
	impl<'de> Deserialize<'de> for GiveAction {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = GiveAction;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema GiveAction")
				}
			...
			deserializer.deserialize_struct("GiveAction", FIELDS, ClassVisitor)
		}
	}
}
