...
pub struct Recipe {
	...
	pub r#cook_time: Vec<CookTimeProperty>,
	/// <https://schema.org/cookingMethod>
	pub r#cooking_method: Vec<CookingMethodProperty>,
	/// <https://schema.org/ingredients>
	#[deprecated = "This schema is superseded by <https://schema.org/recipeIngredient>."]
	pub r#ingredients: Vec<IngredientsProperty>,
	/// <https://schema.org/nutrition>
	pub r#nutrition: Vec<NutritionProperty>,
	/// <https://schema.org/recipeCategory>
	pub r#recipe_category: Vec<RecipeCategoryProperty>,
	/// <https://schema.org/recipeCuisine>
	pub r#recipe_cuisine: Vec<RecipeCuisineProperty>,
	/// <https://schema.org/recipeIngredient>
	pub r#recipe_ingredient: Vec<RecipeIngredientProperty>,
	/// <https://schema.org/recipeInstructions>
	pub r#recipe_instructions: Vec<RecipeInstructionsProperty>,
	/// <https://schema.org/recipeYield>
	pub r#recipe_yield: Vec<RecipeYieldProperty>,
	...
	pub r#perform_time: Vec<PerformTimeProperty>,
	/// <https://schema.org/prepTime>
	pub r#prep_time: Vec<PrepTimeProperty>,
	/// <https://schema.org/step>
	pub r#step: Vec<StepProperty>,
	...
	pub r#supply: Vec<SupplyProperty>,
	/// <https://schema.org/tool>
	pub r#tool: Vec<ToolProperty>,
	/// <https://schema.org/totalTime>
	pub r#total_time: Vec<TotalTimeProperty>,
	/// <https://schema.org/yield>
	pub r#yield: Vec<YieldProperty>,
...
impl RecipeTrait for Recipe {
	fn get_cook_time(&self) -> &[CookTimeProperty] {
		self.r#cook_time.as_slice()
	}
	fn take_cook_time(&mut self) -> Vec<CookTimeProperty> {
		std::mem::take(&mut self.r#cook_time)
	}
	fn get_cooking_method(&self) -> &[CookingMethodProperty] {
		self.r#cooking_method.as_slice()
	}
	fn take_cooking_method(&mut self) -> Vec<CookingMethodProperty> {
		std::mem::take(&mut self.r#cooking_method)
	}
	fn get_ingredients(&self) -> &[IngredientsProperty] {
		self.r#ingredients.as_slice()
	}
	fn take_ingredients(&mut self) -> Vec<IngredientsProperty> {
		std::mem::take(&mut self.r#ingredients)
	}
	fn get_nutrition(&self) -> &[NutritionProperty] {
	...
	fn get_recipe_category(&self) -> &[RecipeCategoryProperty] {
		self.r#recipe_category.as_slice()
	}
	fn take_recipe_category(&mut self) -> Vec<RecipeCategoryProperty> {
		std::mem::take(&mut self.r#recipe_category)
	}
	fn get_recipe_cuisine(&self) -> &[RecipeCuisineProperty] {
		self.r#recipe_cuisine.as_slice()
	}
	fn take_recipe_cuisine(&mut self) -> Vec<RecipeCuisineProperty> {
		std::mem::take(&mut self.r#recipe_cuisine)
	}
	fn get_recipe_ingredient(&self) -> &[RecipeIngredientProperty] {
		self.r#recipe_ingredient.as_slice()
	}
	fn take_recipe_ingredient(&mut self) -> Vec<RecipeIngredientProperty> {
		std::mem::take(&mut self.r#recipe_ingredient)
	}
	fn get_recipe_instructions(&self) -> &[RecipeInstructionsProperty] {
		self.r#recipe_instructions.as_slice()
	}
	fn take_recipe_instructions(&mut self) -> Vec<RecipeInstructionsProperty> {
		std::mem::take(&mut self.r#recipe_instructions)
	}
	fn get_recipe_yield(&self) -> &[RecipeYieldProperty] {
		self.r#recipe_yield.as_slice()
	}
	fn take_recipe_yield(&mut self) -> Vec<RecipeYieldProperty> {
		std::mem::take(&mut self.r#recipe_yield)
	}
	fn get_suitable_for_diet(&self) -> &[SuitableForDietProperty] {
...
impl CreativeWorkTrait for Recipe {
...
mod serde {
	...
	impl Serialize for Recipe {
		...
		{
			let len: usize = [
				!Vec::is_empty(&self.r#cook_time) as usize,
				!Vec::is_empty(&self.r#cooking_method) as usize,
				!Vec::is_empty(&self.r#ingredients) as usize,
				!Vec::is_empty(&self.r#nutrition) as usize,
				!Vec::is_empty(&self.r#recipe_category) as usize,
				!Vec::is_empty(&self.r#recipe_cuisine) as usize,
				!Vec::is_empty(&self.r#recipe_ingredient) as usize,
				!Vec::is_empty(&self.r#recipe_instructions) as usize,
				!Vec::is_empty(&self.r#recipe_yield) as usize,
				...
				!Vec::is_empty(&self.r#estimated_cost) as usize,
				!Vec::is_empty(&self.r#perform_time) as usize,
				!Vec::is_empty(&self.r#prep_time) as usize,
				!Vec::is_empty(&self.r#step) as usize,
				!Vec::is_empty(&self.r#steps) as usize,
				!Vec::is_empty(&self.r#supply) as usize,
				!Vec::is_empty(&self.r#tool) as usize,
				!Vec::is_empty(&self.r#total_time) as usize,
				!Vec::is_empty(&self.r#yield) as usize,
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Recipe", len)?;
			if !Vec::is_empty(&self.r#cook_time) {
				serialize_struct.serialize_field("cookTime", {
					struct SerializeWith<'a>(&'a Vec<CookTimeProperty>);
					...
					&SerializeWith(&self.r#cook_time)
				})?;
			} else {
				serialize_struct.skip_field("cookTime")?;
			}
			if !Vec::is_empty(&self.r#cooking_method) {
				serialize_struct.serialize_field("cookingMethod", {
					struct SerializeWith<'a>(&'a Vec<CookingMethodProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#cooking_method)
				})?;
			} else {
				serialize_struct.skip_field("cookingMethod")?;
			}
			if !Vec::is_empty(&self.r#ingredients) {
				serialize_struct.serialize_field("ingredients", {
					struct SerializeWith<'a>(&'a Vec<IngredientsProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#ingredients)
				})?;
			} else {
				serialize_struct.skip_field("ingredients")?;
			}
			...
			if !Vec::is_empty(&self.r#recipe_category) {
				serialize_struct.serialize_field("recipeCategory", {
					struct SerializeWith<'a>(&'a Vec<RecipeCategoryProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#recipe_category)
				})?;
			} else {
				serialize_struct.skip_field("recipeCategory")?;
			}
			if !Vec::is_empty(&self.r#recipe_cuisine) {
				serialize_struct.serialize_field("recipeCuisine", {
					struct SerializeWith<'a>(&'a Vec<RecipeCuisineProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#recipe_cuisine)
				})?;
			} else {
				serialize_struct.skip_field("recipeCuisine")?;
			}
			if !Vec::is_empty(&self.r#recipe_ingredient) {
				serialize_struct.serialize_field("recipeIngredient", {
					struct SerializeWith<'a>(&'a Vec<RecipeIngredientProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#recipe_ingredient)
				})?;
			} else {
				serialize_struct.skip_field("recipeIngredient")?;
			}
			if !Vec::is_empty(&self.r#recipe_instructions) {
				serialize_struct.serialize_field("recipeInstructions", {
					struct SerializeWith<'a>(&'a Vec<RecipeInstructionsProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#recipe_instructions)
				})?;
			} else {
				serialize_struct.skip_field("recipeInstructions")?;
			}
			if !Vec::is_empty(&self.r#recipe_yield) {
				serialize_struct.serialize_field("recipeYield", {
					struct SerializeWith<'a>(&'a Vec<RecipeYieldProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#recipe_yield)
				})?;
			} else {
				serialize_struct.skip_field("recipeYield")?;
			}
			...
			if !Vec::is_empty(&self.r#estimated_cost) {
				serialize_struct.serialize_field("estimatedCost", {
					struct SerializeWith<'a>(&'a Vec<EstimatedCostProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#estimated_cost)
				})?;
			} else {
				serialize_struct.skip_field("estimatedCost")?;
			}
			if !Vec::is_empty(&self.r#perform_time) {
				serialize_struct.serialize_field("performTime", {
					struct SerializeWith<'a>(&'a Vec<PerformTimeProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#perform_time)
				})?;
			} else {
				serialize_struct.skip_field("performTime")?;
			}
			if !Vec::is_empty(&self.r#prep_time) {
				serialize_struct.serialize_field("prepTime", {
					struct SerializeWith<'a>(&'a Vec<PrepTimeProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#prep_time)
				})?;
			} else {
				serialize_struct.skip_field("prepTime")?;
			}
			if !Vec::is_empty(&self.r#step) {
				serialize_struct.serialize_field("step", {
					struct SerializeWith<'a>(&'a Vec<StepProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#step)
				})?;
			} else {
				serialize_struct.skip_field("step")?;
			}
			if !Vec::is_empty(&self.r#steps) {
				serialize_struct.serialize_field("steps", {
					struct SerializeWith<'a>(&'a Vec<StepsProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#steps)
				})?;
			} else {
				serialize_struct.skip_field("steps")?;
			}
			if !Vec::is_empty(&self.r#supply) {
				serialize_struct.serialize_field("supply", {
					struct SerializeWith<'a>(&'a Vec<SupplyProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#supply)
				})?;
			} else {
				serialize_struct.skip_field("supply")?;
			}
			if !Vec::is_empty(&self.r#tool) {
				serialize_struct.serialize_field("tool", {
					struct SerializeWith<'a>(&'a Vec<ToolProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#tool)
				})?;
			} else {
				serialize_struct.skip_field("tool")?;
			}
			if !Vec::is_empty(&self.r#total_time) {
				serialize_struct.serialize_field("totalTime", {
					struct SerializeWith<'a>(&'a Vec<TotalTimeProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#total_time)
				})?;
			} else {
				serialize_struct.skip_field("totalTime")?;
			}
			if !Vec::is_empty(&self.r#yield) {
				serialize_struct.serialize_field("yield", {
					struct SerializeWith<'a>(&'a Vec<YieldProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#yield)
				})?;
			} else {
				serialize_struct.skip_field("yield")?;
			}
	...
	impl<'de> Deserialize<'de> for Recipe {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
				...
				{
					match value {
						"cookTime" => Ok(Field::CookTime),
						"cookingMethod" => Ok(Field::CookingMethod),
						"ingredients" => Ok(Field::Ingredients),
						"nutrition" => Ok(Field::Nutrition),
						"recipeCategory" => Ok(Field::RecipeCategory),
						"recipeCuisine" => Ok(Field::RecipeCuisine),
						"recipeIngredient" => Ok(Field::RecipeIngredient),
						"recipeInstructions" => Ok(Field::RecipeInstructions),
						"recipeYield" => Ok(Field::RecipeYield),
						...
						"estimatedCost" => Ok(Field::EstimatedCost),
						"performTime" => Ok(Field::PerformTime),
						"prepTime" => Ok(Field::PrepTime),
						"step" => Ok(Field::Step),
						"steps" => Ok(Field::Steps),
						"supply" => Ok(Field::Supply),
						"tool" => Ok(Field::Tool),
						"totalTime" => Ok(Field::TotalTime),
						"yield" => Ok(Field::Yield),
				...
				{
					match value {
						b"cookTime" => Ok(Field::CookTime),
						b"cookingMethod" => Ok(Field::CookingMethod),
						b"ingredients" => Ok(Field::Ingredients),
						b"nutrition" => Ok(Field::Nutrition),
						b"recipeCategory" => Ok(Field::RecipeCategory),
						b"recipeCuisine" => Ok(Field::RecipeCuisine),
						b"recipeIngredient" => Ok(Field::RecipeIngredient),
						b"recipeInstructions" => Ok(Field::RecipeInstructions),
						b"recipeYield" => Ok(Field::RecipeYield),
						...
						b"estimatedCost" => Ok(Field::EstimatedCost),
						b"performTime" => Ok(Field::PerformTime),
						b"prepTime" => Ok(Field::PrepTime),
						b"step" => Ok(Field::Step),
						b"steps" => Ok(Field::Steps),
						b"supply" => Ok(Field::Supply),
						b"tool" => Ok(Field::Tool),
						b"totalTime" => Ok(Field::TotalTime),
						b"yield" => Ok(Field::Yield),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Recipe;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Recipe")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#cook_time_property = None;
					let mut r#cooking_method_property = None;
					let mut r#ingredients_property = None;
					let mut r#nutrition_property = None;
					let mut r#recipe_category_property = None;
					let mut r#recipe_cuisine_property = None;
					let mut r#recipe_ingredient_property = None;
					let mut r#recipe_instructions_property = None;
					let mut r#recipe_yield_property = None;
					...
					let mut r#estimated_cost_property = None;
					let mut r#perform_time_property = None;
					let mut r#prep_time_property = None;
					let mut r#step_property = None;
					let mut r#steps_property = None;
					let mut r#supply_property = None;
					let mut r#tool_property = None;
					let mut r#total_time_property = None;
					let mut r#yield_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::CookTime => {
								if r#cook_time_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"cookTime",
									));
								}
								r#cook_time_property = Some({
									struct DeserializeWith(Vec<CookTimeProperty>);
							...
							Field::CookingMethod => {
								if r#cooking_method_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"cookingMethod",
									));
								}
								r#cooking_method_property = Some({
									struct DeserializeWith(Vec<CookingMethodProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
							...
							Field::Ingredients => {
								if r#ingredients_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"ingredients",
									));
								}
								r#ingredients_property = Some({
									struct DeserializeWith(Vec<IngredientsProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
							...
							Field::RecipeCategory => {
								if r#recipe_category_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"recipeCategory",
									));
								}
								r#recipe_category_property = Some({
									struct DeserializeWith(Vec<RecipeCategoryProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
							...
							Field::RecipeCuisine => {
								if r#recipe_cuisine_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"recipeCuisine",
									));
								}
								r#recipe_cuisine_property = Some({
									struct DeserializeWith(Vec<RecipeCuisineProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
							...
							Field::RecipeIngredient => {
								if r#recipe_ingredient_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"recipeIngredient",
									));
								}
								r#recipe_ingredient_property = Some({
									struct DeserializeWith(Vec<RecipeIngredientProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
							...
							Field::RecipeInstructions => {
								if r#recipe_instructions_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"recipeInstructions",
									));
								}
								r#recipe_instructions_property = Some({
									struct DeserializeWith(Vec<RecipeInstructionsProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
							...
							Field::RecipeYield => {
								if r#recipe_yield_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"recipeYield",
									));
								}
								r#recipe_yield_property = Some({
									struct DeserializeWith(Vec<RecipeYieldProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
							...
							Field::EstimatedCost => {
								if r#estimated_cost_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"estimatedCost",
									));
								}
								r#estimated_cost_property = Some({
									struct DeserializeWith(Vec<EstimatedCostProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
							...
							Field::PerformTime => {
								if r#perform_time_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"performTime",
									));
								}
								r#perform_time_property = Some({
									struct DeserializeWith(Vec<PerformTimeProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
							...
							Field::PrepTime => {
								if r#prep_time_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"prepTime",
									));
								}
								r#prep_time_property = Some({
									struct DeserializeWith(Vec<PrepTimeProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
							...
							Field::Step => {
								if r#step_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field("step"));
								}
								r#step_property = Some({
									struct DeserializeWith(Vec<StepProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
							...
							Field::Steps => {
								if r#steps_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field("steps"));
								}
								r#steps_property = Some({
									struct DeserializeWith(Vec<StepsProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
							...
							Field::Supply => {
								if r#supply_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field("supply"));
								}
								r#supply_property = Some({
									struct DeserializeWith(Vec<SupplyProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
							...
							Field::Tool => {
								if r#tool_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field("tool"));
								}
								r#tool_property = Some({
									struct DeserializeWith(Vec<ToolProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
							...
							Field::TotalTime => {
								if r#total_time_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"totalTime",
									));
								}
								r#total_time_property = Some({
									struct DeserializeWith(Vec<TotalTimeProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
							...
							Field::Yield => {
								if r#yield_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field("yield"));
								}
								r#yield_property = Some({
									struct DeserializeWith(Vec<YieldProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
					...
					Ok(Recipe {
						r#cook_time: r#cook_time_property.unwrap_or_default(),
						r#cooking_method: r#cooking_method_property.unwrap_or_default(),
						r#ingredients: r#ingredients_property.unwrap_or_default(),
						r#nutrition: r#nutrition_property.unwrap_or_default(),
						r#recipe_category: r#recipe_category_property.unwrap_or_default(),
						r#recipe_cuisine: r#recipe_cuisine_property.unwrap_or_default(),
						r#recipe_ingredient: r#recipe_ingredient_property.unwrap_or_default(),
						r#recipe_instructions: r#recipe_instructions_property.unwrap_or_default(),
						r#recipe_yield: r#recipe_yield_property.unwrap_or_default(),
						...
						r#estimated_cost: r#estimated_cost_property.unwrap_or_default(),
						r#perform_time: r#perform_time_property.unwrap_or_default(),
						r#prep_time: r#prep_time_property.unwrap_or_default(),
						r#step: r#step_property.unwrap_or_default(),
						r#steps: r#steps_property.unwrap_or_default(),
						r#supply: r#supply_property.unwrap_or_default(),
						r#tool: r#tool_property.unwrap_or_default(),
						r#total_time: r#total_time_property.unwrap_or_default(),
						r#yield: r#yield_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("Recipe", FIELDS, ClassVisitor)
		}
	}
}
