...
mod serde {
	...
	use super::*;
	impl Serialize for OwnershipInfo {
			...
			S: Serializer,
		{
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "OwnershipInfo", len)?;
			if !Vec::is_empty(&self.r#acquired_from) {
				serialize_struct.serialize_field("acquiredFrom", {
					struct SerializeWith<'a>(&'a Vec<AcquiredFromProperty>);
					...
					&SerializeWith(&self.r#acquired_from)
				})?;
			} else {
	...
	impl<'de> Deserialize<'de> for OwnershipInfo {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = OwnershipInfo;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema OwnershipInfo")
				}
					...
					A: de::MapAccess<'de>,
				{
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::AcquiredFrom => {
								if r#acquired_from_property.is_some() {
