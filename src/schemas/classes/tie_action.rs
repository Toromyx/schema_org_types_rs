...
/// This trait is for properties from <https://schema.org/TieAction>.
pub trait TieActionTrait {}
impl TieActionTrait for TieAction {}
impl AchieveActionTrait for TieAction {}
...
mod serde {
	...
	impl Serialize for TieAction {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "TieAction", len)?;
	...
	impl<'de> Deserialize<'de> for TieAction {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = TieAction;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema TieAction")
				}
			...
			deserializer.deserialize_struct("TieAction", FIELDS, ClassVisitor)
		}
	}
}
