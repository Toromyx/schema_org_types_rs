...
/// This trait is for properties from <https://schema.org/Canal>.
pub trait CanalTrait {}
impl CanalTrait for Canal {}
impl BodyOfWaterTrait for Canal {}
impl LandformTrait for Canal {}
...
mod serde {
	...
	impl Serialize for Canal {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Canal", len)?;
	...
	impl<'de> Deserialize<'de> for Canal {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Canal;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Canal")
				}
			...
			deserializer.deserialize_struct("Canal", FIELDS, ClassVisitor)
		}
	}
}
