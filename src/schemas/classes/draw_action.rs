...
/// This trait is for properties from <https://schema.org/DrawAction>.
pub trait DrawActionTrait {}
impl DrawActionTrait for DrawAction {}
...
impl CreateActionTrait for DrawAction {}
impl ThingTrait for DrawAction {
...
mod serde {
	...
	impl Serialize for DrawAction {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "DrawAction", len)?;
	...
	impl<'de> Deserialize<'de> for DrawAction {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = DrawAction;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema DrawAction")
				}
			...
			deserializer.deserialize_struct("DrawAction", FIELDS, ClassVisitor)
		}
	}
}
