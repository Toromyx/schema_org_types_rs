...
/// This trait is for properties from <https://schema.org/UserPlays>.
#[deprecated = "This schema is superseded by <https://schema.org/InteractionCounter>."]
pub trait UserPlaysTrait {}
impl UserPlaysTrait for UserPlays {}
impl EventTrait for UserPlays {
...
impl UserInteractionTrait for UserPlays {}
#[cfg(feature = "serde")]
mod serde {
	...
	impl Serialize for UserPlays {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "UserPlays", len)?;
	...
	impl<'de> Deserialize<'de> for UserPlays {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = UserPlays;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema UserPlays")
				}
			...
			deserializer.deserialize_struct("UserPlays", FIELDS, ClassVisitor)
		}
	}
}
