...
impl SportsTeamTrait for SportsTeam {
	fn get_athlete(&self) -> &[AthleteProperty] {
		self.r#athlete.as_slice()
	}
	fn take_athlete(&mut self) -> Vec<AthleteProperty> {
		std::mem::take(&mut self.r#athlete)
	}
	fn get_coach(&self) -> &[CoachProperty] {
		self.r#coach.as_slice()
	}
	fn take_coach(&mut self) -> Vec<CoachProperty> {
		std::mem::take(&mut self.r#coach)
	}
	fn get_gender(&self) -> &[GenderProperty] {
...
impl SportsOrganizationTrait for SportsTeam {
...
impl ThingTrait for SportsTeam {
...
mod serde {
	...
	use super::*;
	impl Serialize for SportsTeam {
		fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
		where
			S: Serializer,
		{
			let len: usize = [
				!Vec::is_empty(&self.r#athlete) as usize,
				!Vec::is_empty(&self.r#coach) as usize,
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "SportsTeam", len)?;
			if !Vec::is_empty(&self.r#athlete) {
				serialize_struct.serialize_field("athlete", {
					struct SerializeWith<'a>(&'a Vec<AthleteProperty>);
					...
					&SerializeWith(&self.r#athlete)
				})?;
			} else {
				serialize_struct.skip_field("athlete")?;
			}
			if !Vec::is_empty(&self.r#coach) {
				serialize_struct.serialize_field("coach", {
					struct SerializeWith<'a>(&'a Vec<CoachProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#coach)
				})?;
			} else {
				serialize_struct.skip_field("coach")?;
			}
	...
	impl<'de> Deserialize<'de> for SportsTeam {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
				...
				{
					match value {
						"athlete" => Ok(Field::Athlete),
						"coach" => Ok(Field::Coach),
				...
				{
					match value {
						b"athlete" => Ok(Field::Athlete),
						b"coach" => Ok(Field::Coach),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = SportsTeam;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema SportsTeam")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#athlete_property = None;
					let mut r#coach_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::Athlete => {
								if r#athlete_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"athlete",
									));
								}
								r#athlete_property = Some({
									struct DeserializeWith(Vec<AthleteProperty>);
							...
							Field::Coach => {
								if r#coach_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field("coach"));
								}
								r#coach_property = Some({
									struct DeserializeWith(Vec<CoachProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
					...
					Ok(SportsTeam {
						r#athlete: r#athlete_property.unwrap_or_default(),
						r#coach: r#coach_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("SportsTeam", FIELDS, ClassVisitor)
		}
	}
}
