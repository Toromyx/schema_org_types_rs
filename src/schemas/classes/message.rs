...
impl CreativeWorkTrait for Message {
...
mod serde {
	...
	impl Serialize for Message {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Message", len)?;
	...
	impl<'de> Deserialize<'de> for Message {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Message;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Message")
				}
			...
			deserializer.deserialize_struct("Message", FIELDS, ClassVisitor)
		}
	}
}
