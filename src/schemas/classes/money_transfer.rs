...
pub struct MoneyTransfer {
	...
	pub r#beneficiary_bank: Vec<BeneficiaryBankProperty>,
...
pub trait MoneyTransferTrait {
	...
	/// Get <https://schema.org/beneficiaryBank> from [`Self`] as borrowed slice.
	fn get_beneficiary_bank(&self) -> &[BeneficiaryBankProperty];
	/// Take <https://schema.org/beneficiaryBank> from [`Self`] as owned vector.
	fn take_beneficiary_bank(&mut self) -> Vec<BeneficiaryBankProperty>;
}
impl MoneyTransferTrait for MoneyTransfer {
	fn get_amount(&self) -> &[AmountProperty] {
	...
	fn get_beneficiary_bank(&self) -> &[BeneficiaryBankProperty] {
		self.r#beneficiary_bank.as_slice()
	}
	fn take_beneficiary_bank(&mut self) -> Vec<BeneficiaryBankProperty> {
		std::mem::take(&mut self.r#beneficiary_bank)
	}
}
...
impl ThingTrait for MoneyTransfer {
...
impl TransferActionTrait for MoneyTransfer {
...
mod serde {
	...
	use super::*;
	impl Serialize for MoneyTransfer {
			...
			S: Serializer,
		{
			let len: usize = [
				...
				!Vec::is_empty(&self.r#beneficiary_bank) as usize,
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "MoneyTransfer", len)?;
			...
			if !Vec::is_empty(&self.r#beneficiary_bank) {
				serialize_struct.serialize_field("beneficiaryBank", {
					struct SerializeWith<'a>(&'a Vec<BeneficiaryBankProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#beneficiary_bank)
				})?;
			} else {
				serialize_struct.skip_field("beneficiaryBank")?;
			}
	...
	impl<'de> Deserialize<'de> for MoneyTransfer {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
					...
					E: de::Error,
				{
					match value {
						...
						"beneficiaryBank" => Ok(Field::BeneficiaryBank),
					...
					E: de::Error,
				{
					match value {
						...
						b"beneficiaryBank" => Ok(Field::BeneficiaryBank),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = MoneyTransfer;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema MoneyTransfer")
				}
					...
					A: de::MapAccess<'de>,
				{
					...
					let mut r#beneficiary_bank_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							...
							Field::BeneficiaryBank => {
								if r#beneficiary_bank_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"beneficiaryBank",
									));
								}
								r#beneficiary_bank_property = Some({
									struct DeserializeWith(Vec<BeneficiaryBankProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
					...
					Ok(MoneyTransfer {
						...
						r#beneficiary_bank: r#beneficiary_bank_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("MoneyTransfer", FIELDS, ClassVisitor)
		}
	}
}
