...
/// This trait is for properties from <https://schema.org/SaleEvent>.
pub trait SaleEventTrait {}
impl SaleEventTrait for SaleEvent {}
impl EventTrait for SaleEvent {
...
mod serde {
	...
	impl Serialize for SaleEvent {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "SaleEvent", len)?;
	...
	impl<'de> Deserialize<'de> for SaleEvent {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = SaleEvent;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema SaleEvent")
				}
			...
			deserializer.deserialize_struct("SaleEvent", FIELDS, ClassVisitor)
		}
	}
}
