...
impl PayActionTrait for PayAction {
	fn get_recipient(&self) -> &[RecipientProperty] {
...
impl TradeActionTrait for PayAction {
...
mod serde {
	...
	impl Serialize for PayAction {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "PayAction", len)?;
	...
	impl<'de> Deserialize<'de> for PayAction {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = PayAction;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema PayAction")
				}
			...
			deserializer.deserialize_struct("PayAction", FIELDS, ClassVisitor)
		}
	}
}
