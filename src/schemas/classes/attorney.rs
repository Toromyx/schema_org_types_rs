...
/// This trait is for properties from <https://schema.org/Attorney>.
pub trait AttorneyTrait {}
impl AttorneyTrait for Attorney {}
impl LegalServiceTrait for Attorney {}
impl LocalBusinessTrait for Attorney {
...
mod serde {
	...
	impl Serialize for Attorney {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Attorney", len)?;
	...
	impl<'de> Deserialize<'de> for Attorney {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Attorney;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Attorney")
				}
			...
			deserializer.deserialize_struct("Attorney", FIELDS, ClassVisitor)
		}
	}
}
