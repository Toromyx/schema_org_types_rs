...
pub struct VoteAction {
	...
	pub r#candidate: Vec<CandidateProperty>,
...
impl VoteActionTrait for VoteAction {
	fn get_candidate(&self) -> &[CandidateProperty] {
		self.r#candidate.as_slice()
	}
	fn take_candidate(&mut self) -> Vec<CandidateProperty> {
		std::mem::take(&mut self.r#candidate)
	}
}
...
impl AssessActionTrait for VoteAction {}
impl ChooseActionTrait for VoteAction {
	fn get_action_option(&self) -> &[ActionOptionProperty] {
		self.r#action_option.as_slice()
	}
	fn take_action_option(&mut self) -> Vec<ActionOptionProperty> {
		std::mem::take(&mut self.r#action_option)
	}
	fn get_option(&self) -> &[OptionProperty] {
		self.r#option.as_slice()
	}
	fn take_option(&mut self) -> Vec<OptionProperty> {
		std::mem::take(&mut self.r#option)
	}
}
impl ThingTrait for VoteAction {
...
mod serde {
	...
	impl Serialize for VoteAction {
		...
		{
			let len: usize = [
				!Vec::is_empty(&self.r#candidate) as usize,
				...
				!Vec::is_empty(&self.r#action_option) as usize,
				!Vec::is_empty(&self.r#option) as usize,
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "VoteAction", len)?;
			if !Vec::is_empty(&self.r#candidate) {
				serialize_struct.serialize_field("candidate", {
					struct SerializeWith<'a>(&'a Vec<CandidateProperty>);
					...
					&SerializeWith(&self.r#candidate)
				})?;
			} else {
				serialize_struct.skip_field("candidate")?;
			}
			...
			if !Vec::is_empty(&self.r#action_option) {
				serialize_struct.serialize_field("actionOption", {
					struct SerializeWith<'a>(&'a Vec<ActionOptionProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#action_option)
				})?;
			} else {
				serialize_struct.skip_field("actionOption")?;
			}
			if !Vec::is_empty(&self.r#option) {
				serialize_struct.serialize_field("option", {
					struct SerializeWith<'a>(&'a Vec<OptionProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#option)
				})?;
			} else {
				serialize_struct.skip_field("option")?;
			}
	...
	impl<'de> Deserialize<'de> for VoteAction {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
					...
					E: de::Error,
				{
					match value {
						...
						"actionOption" => Ok(Field::ActionOption),
						"option" => Ok(Field::Option),
				...
				{
					match value {
						b"candidate" => Ok(Field::Candidate),
						...
						b"actionOption" => Ok(Field::ActionOption),
						b"option" => Ok(Field::Option),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = VoteAction;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema VoteAction")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#candidate_property = None;
					...
					let mut r#action_option_property = None;
					let mut r#option_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::Candidate => {
								if r#candidate_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"candidate",
									));
								}
								r#candidate_property = Some({
									struct DeserializeWith(Vec<CandidateProperty>);
							...
							Field::ActionOption => {
								if r#action_option_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"actionOption",
									));
								}
								r#action_option_property = Some({
									struct DeserializeWith(Vec<ActionOptionProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
							...
							Field::Option => {
								if r#option_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field("option"));
								}
								r#option_property = Some({
									struct DeserializeWith(Vec<OptionProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
					...
					Ok(VoteAction {
						r#candidate: r#candidate_property.unwrap_or_default(),
						...
						r#action_option: r#action_option_property.unwrap_or_default(),
						r#option: r#option_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("VoteAction", FIELDS, ClassVisitor)
		}
	}
}
