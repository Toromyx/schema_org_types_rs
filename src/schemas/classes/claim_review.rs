...
/// This trait is for properties from <https://schema.org/ClaimReview>.
pub trait ClaimReviewTrait {
	/// Get <https://schema.org/claimReviewed> from [`Self`] as borrowed slice.
	fn get_claim_reviewed(&self) -> &[ClaimReviewedProperty];
	/// Take <https://schema.org/claimReviewed> from [`Self`] as owned vector.
	fn take_claim_reviewed(&mut self) -> Vec<ClaimReviewedProperty>;
}
impl ClaimReviewTrait for ClaimReview {
	fn get_claim_reviewed(&self) -> &[ClaimReviewedProperty] {
		self.r#claim_reviewed.as_slice()
	}
	fn take_claim_reviewed(&mut self) -> Vec<ClaimReviewedProperty> {
		std::mem::take(&mut self.r#claim_reviewed)
	}
}
impl CreativeWorkTrait for ClaimReview {
...
impl ReviewTrait for ClaimReview {
...
impl ThingTrait for ClaimReview {
...
mod serde {
	...
	use super::*;
	impl Serialize for ClaimReview {
		fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
		where
			S: Serializer,
		{
			let len: usize = [
				!Vec::is_empty(&self.r#claim_reviewed) as usize,
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "ClaimReview", len)?;
			if !Vec::is_empty(&self.r#claim_reviewed) {
				serialize_struct.serialize_field("claimReviewed", {
					struct SerializeWith<'a>(&'a Vec<ClaimReviewedProperty>);
					...
					&SerializeWith(&self.r#claim_reviewed)
				})?;
			} else {
				serialize_struct.skip_field("claimReviewed")?;
			}
	...
	impl<'de> Deserialize<'de> for ClaimReview {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
				...
				{
					match value {
						"claimReviewed" => Ok(Field::ClaimReviewed),
				...
				{
					match value {
						b"claimReviewed" => Ok(Field::ClaimReviewed),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = ClaimReview;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema ClaimReview")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#claim_reviewed_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::ClaimReviewed => {
								if r#claim_reviewed_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"claimReviewed",
									));
								}
								r#claim_reviewed_property = Some({
									struct DeserializeWith(Vec<ClaimReviewedProperty>);
					...
					Ok(ClaimReview {
						r#claim_reviewed: r#claim_reviewed_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("ClaimReview", FIELDS, ClassVisitor)
		}
	}
}
