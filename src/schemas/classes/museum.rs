...
/// This trait is for properties from <https://schema.org/Museum>.
pub trait MuseumTrait {}
impl MuseumTrait for Museum {}
impl CivicStructureTrait for Museum {
...
mod serde {
	...
	impl Serialize for Museum {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Museum", len)?;
	...
	impl<'de> Deserialize<'de> for Museum {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Museum;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Museum")
				}
			...
			deserializer.deserialize_struct("Museum", FIELDS, ClassVisitor)
		}
	}
}
