...
/// This trait is for properties from <https://schema.org/Zoo>.
pub trait ZooTrait {}
impl ZooTrait for Zoo {}
impl CivicStructureTrait for Zoo {
...
mod serde {
	...
	impl Serialize for Zoo {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Zoo", len)?;
	...
	impl<'de> Deserialize<'de> for Zoo {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Zoo;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Zoo")
				}
			...
			deserializer.deserialize_struct("Zoo", FIELDS, ClassVisitor)
		}
	}
}
