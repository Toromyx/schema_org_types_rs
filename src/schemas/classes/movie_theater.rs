...
pub trait MovieTheaterTrait {
	/// Get <https://schema.org/screenCount> from [`Self`] as borrowed slice.
	fn get_screen_count(&self) -> &[ScreenCountProperty];
	/// Take <https://schema.org/screenCount> from [`Self`] as owned vector.
	fn take_screen_count(&mut self) -> Vec<ScreenCountProperty>;
}
impl MovieTheaterTrait for MovieTheater {
	fn get_screen_count(&self) -> &[ScreenCountProperty] {
		self.r#screen_count.as_slice()
	}
	fn take_screen_count(&mut self) -> Vec<ScreenCountProperty> {
		std::mem::take(&mut self.r#screen_count)
	}
}
impl CivicStructureTrait for MovieTheater {
...
impl EntertainmentBusinessTrait for MovieTheater {}
impl LocalBusinessTrait for MovieTheater {
...
impl ThingTrait for MovieTheater {
...
mod serde {
	...
	use super::*;
	impl Serialize for MovieTheater {
		fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
		where
			S: Serializer,
		{
			let len: usize = [
				!Vec::is_empty(&self.r#screen_count) as usize,
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "MovieTheater", len)?;
			if !Vec::is_empty(&self.r#screen_count) {
				serialize_struct.serialize_field("screenCount", {
					struct SerializeWith<'a>(&'a Vec<ScreenCountProperty>);
					...
					&SerializeWith(&self.r#screen_count)
				})?;
			} else {
				serialize_struct.skip_field("screenCount")?;
			}
	...
	impl<'de> Deserialize<'de> for MovieTheater {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
				...
				{
					match value {
						"screenCount" => Ok(Field::ScreenCount),
				...
				{
					match value {
						b"screenCount" => Ok(Field::ScreenCount),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = MovieTheater;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema MovieTheater")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#screen_count_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::ScreenCount => {
								if r#screen_count_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"screenCount",
									));
								}
								r#screen_count_property = Some({
									struct DeserializeWith(Vec<ScreenCountProperty>);
					...
					Ok(MovieTheater {
						r#screen_count: r#screen_count_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("MovieTheater", FIELDS, ClassVisitor)
		}
	}
}
