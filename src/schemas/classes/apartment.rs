...
impl ApartmentTrait for Apartment {
	fn get_number_of_rooms(&self) -> &[NumberOfRoomsProperty] {
...
impl AccommodationTrait for Apartment {
...
mod serde {
	...
	impl Serialize for Apartment {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Apartment", len)?;
	...
	impl<'de> Deserialize<'de> for Apartment {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				...
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Apartment")
				}
			...
			deserializer.deserialize_struct("Apartment", FIELDS, ClassVisitor)
		}
	}
}
