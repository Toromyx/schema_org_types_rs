...
impl MedicalCauseTrait for MedicalCause {
	fn get_cause_of(&self) -> &[CauseOfProperty] {
		self.r#cause_of.as_slice()
	}
	fn take_cause_of(&mut self) -> Vec<CauseOfProperty> {
		std::mem::take(&mut self.r#cause_of)
	}
}
impl MedicalEntityTrait for MedicalCause {
...
impl ThingTrait for MedicalCause {
...
mod serde {
	...
	use super::*;
	impl Serialize for MedicalCause {
		fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
		where
			S: Serializer,
		{
			let len: usize = [
				!Vec::is_empty(&self.r#cause_of) as usize,
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "MedicalCause", len)?;
			if !Vec::is_empty(&self.r#cause_of) {
				serialize_struct.serialize_field("causeOf", {
					struct SerializeWith<'a>(&'a Vec<CauseOfProperty>);
					...
					&SerializeWith(&self.r#cause_of)
				})?;
			} else {
				serialize_struct.skip_field("causeOf")?;
			}
	...
	impl<'de> Deserialize<'de> for MedicalCause {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = MedicalCause;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema MedicalCause")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#cause_of_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::CauseOf => {
								if r#cause_of_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"causeOf",
									));
								}
								r#cause_of_property = Some({
									struct DeserializeWith(Vec<CauseOfProperty>);
					...
					Ok(MedicalCause {
						r#cause_of: r#cause_of_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("MedicalCause", FIELDS, ClassVisitor)
		}
	}
}
