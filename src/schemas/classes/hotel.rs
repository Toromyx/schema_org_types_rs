...
/// This trait is for properties from <https://schema.org/Hotel>.
pub trait HotelTrait {}
impl HotelTrait for Hotel {}
impl LocalBusinessTrait for Hotel {
...
impl LodgingBusinessTrait for Hotel {
...
mod serde {
	...
	impl Serialize for Hotel {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Hotel", len)?;
	...
	impl<'de> Deserialize<'de> for Hotel {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Hotel;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Hotel")
				}
			...
			deserializer.deserialize_struct("Hotel", FIELDS, ClassVisitor)
		}
	}
}
