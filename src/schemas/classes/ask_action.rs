...
impl AskActionTrait for AskAction {
	fn get_question(&self) -> &[QuestionProperty] {
		self.r#question.as_slice()
	}
	fn take_question(&mut self) -> Vec<QuestionProperty> {
		std::mem::take(&mut self.r#question)
	}
}
...
impl CommunicateActionTrait for AskAction {
...
impl InteractActionTrait for AskAction {}
impl ThingTrait for AskAction {
...
mod serde {
	...
	use super::*;
	impl Serialize for AskAction {
		fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
		where
			S: Serializer,
		{
			let len: usize = [
				!Vec::is_empty(&self.r#question) as usize,
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "AskAction", len)?;
			if !Vec::is_empty(&self.r#question) {
				serialize_struct.serialize_field("question", {
					struct SerializeWith<'a>(&'a Vec<QuestionProperty>);
					...
					&SerializeWith(&self.r#question)
				})?;
			} else {
				serialize_struct.skip_field("question")?;
			}
	...
	impl<'de> Deserialize<'de> for AskAction {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = AskAction;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema AskAction")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#question_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::Question => {
								if r#question_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"question",
									));
								}
								r#question_property = Some({
									struct DeserializeWith(Vec<QuestionProperty>);
					...
					Ok(AskAction {
						r#question: r#question_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("AskAction", FIELDS, ClassVisitor)
		}
	}
}
