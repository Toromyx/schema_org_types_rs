...
/// This trait is for properties from <https://schema.org/TakeAction>.
pub trait TakeActionTrait {}
impl TakeActionTrait for TakeAction {}
...
impl ThingTrait for TakeAction {
...
impl TransferActionTrait for TakeAction {
...
mod serde {
	...
	impl Serialize for TakeAction {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "TakeAction", len)?;
	...
	impl<'de> Deserialize<'de> for TakeAction {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = TakeAction;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema TakeAction")
				}
			...
			deserializer.deserialize_struct("TakeAction", FIELDS, ClassVisitor)
		}
	}
}
