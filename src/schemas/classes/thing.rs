...
mod serde {
	...
	impl Serialize for Thing {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Thing", len)?;
	...
	impl<'de> Deserialize<'de> for Thing {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Thing;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Thing")
				}
			...
			deserializer.deserialize_struct("Thing", FIELDS, ClassVisitor)
		}
	}
}
