...
/// This trait is for properties from <https://schema.org/Aquarium>.
pub trait AquariumTrait {}
impl AquariumTrait for Aquarium {}
impl CivicStructureTrait for Aquarium {
...
mod serde {
	...
	impl Serialize for Aquarium {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Aquarium", len)?;
	...
	impl<'de> Deserialize<'de> for Aquarium {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Aquarium;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Aquarium")
				}
			...
			deserializer.deserialize_struct("Aquarium", FIELDS, ClassVisitor)
		}
	}
}
