...
/// This trait is for properties from <https://schema.org/Play>.
pub trait PlayTrait {}
impl PlayTrait for Play {}
impl CreativeWorkTrait for Play {
...
mod serde {
	...
	impl Serialize for Play {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Play", len)?;
	...
	impl<'de> Deserialize<'de> for Play {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Play;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Play")
				}
			...
			deserializer.deserialize_struct("Play", FIELDS, ClassVisitor)
		}
	}
}
