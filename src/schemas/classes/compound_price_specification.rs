...
mod serde {
	...
	use super::*;
	impl Serialize for CompoundPriceSpecification {
		fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
		where
			S: Serializer,
		{
			let len: usize = [
				!Vec::is_empty(&self.r#price_component) as usize,
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "CompoundPriceSpecification", len)?;
			if !Vec::is_empty(&self.r#price_component) {
				serialize_struct.serialize_field("priceComponent", {
					struct SerializeWith<'a>(&'a Vec<PriceComponentProperty>);
					...
					&SerializeWith(&self.r#price_component)
				})?;
			} else {
				serialize_struct.skip_field("priceComponent")?;
			}
	...
	impl<'de> Deserialize<'de> for CompoundPriceSpecification {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
				...
				{
					match value {
						"priceComponent" => Ok(Field::PriceComponent),
				...
				{
					match value {
						b"priceComponent" => Ok(Field::PriceComponent),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = CompoundPriceSpecification;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema CompoundPriceSpecification")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#price_component_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::PriceComponent => {
								if r#price_component_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"priceComponent",
									));
								}
								r#price_component_property = Some({
									struct DeserializeWith(Vec<PriceComponentProperty>);
					...
					Ok(CompoundPriceSpecification {
						r#price_component: r#price_component_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("CompoundPriceSpecification", FIELDS, ClassVisitor)
		}
	}
}
