...
pub trait DataCatalogTrait {
	/// Get <https://schema.org/dataset> from [`Self`] as borrowed slice.
	fn get_dataset(&self) -> &[DatasetProperty];
	/// Take <https://schema.org/dataset> from [`Self`] as owned vector.
...
impl DataCatalogTrait for DataCatalog {
	fn get_dataset(&self) -> &[DatasetProperty] {
		self.r#dataset.as_slice()
	}
	fn take_dataset(&mut self) -> Vec<DatasetProperty> {
		std::mem::take(&mut self.r#dataset)
	}
	fn get_measurement_method(&self) -> &[MeasurementMethodProperty] {
...
impl CreativeWorkTrait for DataCatalog {
...
impl ThingTrait for DataCatalog {
...
mod serde {
	...
	use super::*;
	impl Serialize for DataCatalog {
		fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
		where
			S: Serializer,
		{
			let len: usize = [
				!Vec::is_empty(&self.r#dataset) as usize,
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "DataCatalog", len)?;
			if !Vec::is_empty(&self.r#dataset) {
				serialize_struct.serialize_field("dataset", {
					struct SerializeWith<'a>(&'a Vec<DatasetProperty>);
					...
					&SerializeWith(&self.r#dataset)
				})?;
			} else {
				serialize_struct.skip_field("dataset")?;
			}
	...
	impl<'de> Deserialize<'de> for DataCatalog {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = DataCatalog;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema DataCatalog")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#dataset_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::Dataset => {
								if r#dataset_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"dataset",
									));
								}
								r#dataset_property = Some({
									struct DeserializeWith(Vec<DatasetProperty>);
					...
					Ok(DataCatalog {
						r#dataset: r#dataset_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("DataCatalog", FIELDS, ClassVisitor)
		}
	}
}
