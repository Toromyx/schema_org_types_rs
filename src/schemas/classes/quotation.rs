...
/// This trait is for properties from <https://schema.org/Quotation>.
pub trait QuotationTrait {
	/// Get <https://schema.org/spokenByCharacter> from [`Self`] as borrowed slice.
	fn get_spoken_by_character(&self) -> &[SpokenByCharacterProperty];
	/// Take <https://schema.org/spokenByCharacter> from [`Self`] as owned vector.
	fn take_spoken_by_character(&mut self) -> Vec<SpokenByCharacterProperty>;
}
impl QuotationTrait for Quotation {
	fn get_spoken_by_character(&self) -> &[SpokenByCharacterProperty] {
		self.r#spoken_by_character.as_slice()
	}
	fn take_spoken_by_character(&mut self) -> Vec<SpokenByCharacterProperty> {
		std::mem::take(&mut self.r#spoken_by_character)
	}
}
impl CreativeWorkTrait for Quotation {
...
mod serde {
	...
	impl Serialize for Quotation {
		...
		{
			let len: usize = [
				!Vec::is_empty(&self.r#spoken_by_character) as usize,
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Quotation", len)?;
			if !Vec::is_empty(&self.r#spoken_by_character) {
				serialize_struct.serialize_field("spokenByCharacter", {
					struct SerializeWith<'a>(&'a Vec<SpokenByCharacterProperty>);
					...
					&SerializeWith(&self.r#spoken_by_character)
				})?;
			} else {
				serialize_struct.skip_field("spokenByCharacter")?;
			}
	...
	impl<'de> Deserialize<'de> for Quotation {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
				...
				{
					match value {
						"spokenByCharacter" => Ok(Field::SpokenByCharacter),
				...
				{
					match value {
						b"spokenByCharacter" => Ok(Field::SpokenByCharacter),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				...
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Quotation")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#spoken_by_character_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::SpokenByCharacter => {
								if r#spoken_by_character_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"spokenByCharacter",
									));
								}
								r#spoken_by_character_property = Some({
									struct DeserializeWith(Vec<SpokenByCharacterProperty>);
					...
					Ok(Quotation {
						r#spoken_by_character: r#spoken_by_character_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("Quotation", FIELDS, ClassVisitor)
		}
	}
}
