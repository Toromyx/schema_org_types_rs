...
pub struct WebApi {
	...
	pub r#documentation: Vec<DocumentationProperty>,
...
/// This trait is for properties from <https://schema.org/WebAPI>.
pub trait WebApiTrait {
	/// Get <https://schema.org/documentation> from [`Self`] as borrowed slice.
	fn get_documentation(&self) -> &[DocumentationProperty];
	/// Take <https://schema.org/documentation> from [`Self`] as owned vector.
	fn take_documentation(&mut self) -> Vec<DocumentationProperty>;
}
impl WebApiTrait for WebApi {
	fn get_documentation(&self) -> &[DocumentationProperty] {
		self.r#documentation.as_slice()
	}
	fn take_documentation(&mut self) -> Vec<DocumentationProperty> {
		std::mem::take(&mut self.r#documentation)
	}
}
impl ServiceTrait for WebApi {
...
mod serde {
	...
	use super::*;
	impl Serialize for WebApi {
		fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
		where
			S: Serializer,
		{
			let len: usize = [
				!Vec::is_empty(&self.r#documentation) as usize,
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "WebApi", len)?;
			if !Vec::is_empty(&self.r#documentation) {
				serialize_struct.serialize_field("documentation", {
					struct SerializeWith<'a>(&'a Vec<DocumentationProperty>);
					...
					&SerializeWith(&self.r#documentation)
				})?;
			} else {
				serialize_struct.skip_field("documentation")?;
			}
	...
	impl<'de> Deserialize<'de> for WebApi {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
				...
				{
					match value {
						"documentation" => Ok(Field::Documentation),
				...
				{
					match value {
						b"documentation" => Ok(Field::Documentation),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = WebApi;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema WebAPI")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#documentation_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::Documentation => {
								if r#documentation_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"documentation",
									));
								}
								r#documentation_property = Some({
									struct DeserializeWith(Vec<DocumentationProperty>);
					...
					Ok(WebApi {
						r#documentation: r#documentation_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("WebApi", FIELDS, ClassVisitor)
		}
	}
}
