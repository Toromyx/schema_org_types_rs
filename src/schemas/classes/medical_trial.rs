...
pub struct MedicalTrial {
	...
	pub r#trial_design: Vec<TrialDesignProperty>,
...
/// This trait is for properties from <https://schema.org/MedicalTrial>.
pub trait MedicalTrialTrait {
	/// Get <https://schema.org/trialDesign> from [`Self`] as borrowed slice.
	fn get_trial_design(&self) -> &[TrialDesignProperty];
	/// Take <https://schema.org/trialDesign> from [`Self`] as owned vector.
	fn take_trial_design(&mut self) -> Vec<TrialDesignProperty>;
}
impl MedicalTrialTrait for MedicalTrial {
	fn get_trial_design(&self) -> &[TrialDesignProperty] {
		self.r#trial_design.as_slice()
	}
	fn take_trial_design(&mut self) -> Vec<TrialDesignProperty> {
		std::mem::take(&mut self.r#trial_design)
	}
}
impl MedicalEntityTrait for MedicalTrial {
...
impl MedicalStudyTrait for MedicalTrial {
...
impl ThingTrait for MedicalTrial {
...
mod serde {
	...
	use super::*;
	impl Serialize for MedicalTrial {
		fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
		where
			S: Serializer,
		{
			let len: usize = [
				!Vec::is_empty(&self.r#trial_design) as usize,
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "MedicalTrial", len)?;
			if !Vec::is_empty(&self.r#trial_design) {
				serialize_struct.serialize_field("trialDesign", {
					struct SerializeWith<'a>(&'a Vec<TrialDesignProperty>);
					...
					&SerializeWith(&self.r#trial_design)
				})?;
			} else {
				serialize_struct.skip_field("trialDesign")?;
			}
	...
	impl<'de> Deserialize<'de> for MedicalTrial {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
				...
				{
					match value {
						"trialDesign" => Ok(Field::TrialDesign),
						"code" => Ok(Field::Code),
				...
				{
					match value {
						b"trialDesign" => Ok(Field::TrialDesign),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = MedicalTrial;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema MedicalTrial")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#trial_design_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::TrialDesign => {
								if r#trial_design_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"trialDesign",
									));
								}
								r#trial_design_property = Some({
									struct DeserializeWith(Vec<TrialDesignProperty>);
					...
					Ok(MedicalTrial {
						r#trial_design: r#trial_design_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("MedicalTrial", FIELDS, ClassVisitor)
		}
	}
}
