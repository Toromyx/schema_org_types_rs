...
/// This trait is for properties from <https://schema.org/Painting>.
pub trait PaintingTrait {}
impl PaintingTrait for Painting {}
impl CreativeWorkTrait for Painting {
...
mod serde {
	...
	impl Serialize for Painting {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Painting", len)?;
	...
	impl<'de> Deserialize<'de> for Painting {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Painting;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Painting")
				}
			...
			deserializer.deserialize_struct("Painting", FIELDS, ClassVisitor)
		}
	}
}
