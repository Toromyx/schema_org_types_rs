...
mod serde {
	...
	impl Serialize for Bakery {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Bakery", len)?;
			#[cfg(any(
				any(
					feature = "accepts-reservations-property-schema",
					feature = "general-schema-section"
	...
	impl<'de> Deserialize<'de> for Bakery {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Bakery;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Bakery")
				}
			...
			deserializer.deserialize_struct("Bakery", FIELDS, ClassVisitor)
		}
	}
}
