...
/// This trait is for properties from <https://schema.org/Beach>.
pub trait BeachTrait {}
impl BeachTrait for Beach {}
impl CivicStructureTrait for Beach {
...
mod serde {
	...
	impl Serialize for Beach {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Beach", len)?;
	...
	impl<'de> Deserialize<'de> for Beach {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Beach;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Beach")
				}
			...
			deserializer.deserialize_struct("Beach", FIELDS, ClassVisitor)
		}
	}
}
