...
impl FollowActionTrait for FollowAction {
	fn get_followee(&self) -> &[FolloweeProperty] {
		self.r#followee.as_slice()
	}
	fn take_followee(&mut self) -> Vec<FolloweeProperty> {
		std::mem::take(&mut self.r#followee)
	}
}
...
impl InteractActionTrait for FollowAction {}
impl ThingTrait for FollowAction {
...
mod serde {
	...
	use super::*;
	impl Serialize for FollowAction {
			...
			S: Serializer,
		{
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "FollowAction", len)?;
			if !Vec::is_empty(&self.r#followee) {
				serialize_struct.serialize_field("followee", {
					struct SerializeWith<'a>(&'a Vec<FolloweeProperty>);
	...
	impl<'de> Deserialize<'de> for FollowAction {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = FollowAction;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema FollowAction")
				}
					...
					A: de::MapAccess<'de>,
				{
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::Followee => {
								if r#followee_property.is_some() {
			...
			deserializer.deserialize_struct("FollowAction", FIELDS, ClassVisitor)
		}
	}
}
