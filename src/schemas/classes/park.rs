...
/// This trait is for properties from <https://schema.org/Park>.
pub trait ParkTrait {}
impl ParkTrait for Park {}
impl CivicStructureTrait for Park {
...
mod serde {
	...
	impl Serialize for Park {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Park", len)?;
	...
	impl<'de> Deserialize<'de> for Park {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Park;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Park")
				}
			...
			deserializer.deserialize_struct("Park", FIELDS, ClassVisitor)
		}
	}
}
