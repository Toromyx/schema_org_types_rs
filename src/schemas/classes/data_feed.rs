...
/// This trait is for properties from <https://schema.org/DataFeed>.
pub trait DataFeedTrait {
	/// Get <https://schema.org/dataFeedElement> from [`Self`] as borrowed slice.
	fn get_data_feed_element(&self) -> &[DataFeedElementProperty];
	/// Take <https://schema.org/dataFeedElement> from [`Self`] as owned vector.
	fn take_data_feed_element(&mut self) -> Vec<DataFeedElementProperty>;
}
impl DataFeedTrait for DataFeed {
	fn get_data_feed_element(&self) -> &[DataFeedElementProperty] {
...
impl CreativeWorkTrait for DataFeed {
...
impl DatasetTrait for DataFeed {
...
mod serde {
	...
	impl Serialize for DataFeed {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "DataFeed", len)?;
	...
	impl<'de> Deserialize<'de> for DataFeed {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = DataFeed;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema DataFeed")
				}
			...
			deserializer.deserialize_struct("DataFeed", FIELDS, ClassVisitor)
		}
	}
}
