...
/// This trait is for properties from <https://schema.org/ImageObjectSnapshot>.
pub trait ImageObjectSnapshotTrait {}
impl ImageObjectSnapshotTrait for ImageObjectSnapshot {}
impl CreativeWorkTrait for ImageObjectSnapshot {
...
impl ImageObjectTrait for ImageObjectSnapshot {
	...
	fn get_exif_data(&self) -> &[ExifDataProperty] {
		self.r#exif_data.as_slice()
	}
	fn take_exif_data(&mut self) -> Vec<ExifDataProperty> {
		std::mem::take(&mut self.r#exif_data)
	}
	fn get_representative_of_page(&self) -> &[RepresentativeOfPageProperty] {
		self.r#representative_of_page.as_slice()
	}
	fn take_representative_of_page(&mut self) -> Vec<RepresentativeOfPageProperty> {
		std::mem::take(&mut self.r#representative_of_page)
	}
}
impl MediaObjectTrait for ImageObjectSnapshot {
...
impl ThingTrait for ImageObjectSnapshot {
...
mod serde {
	...
	use super::*;
	impl Serialize for ImageObjectSnapshot {
			...
			S: Serializer,
		{
			let len: usize = [
				...
				!Vec::is_empty(&self.r#exif_data) as usize,
				!Vec::is_empty(&self.r#representative_of_page) as usize,
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "ImageObjectSnapshot", len)?;
			...
			if !Vec::is_empty(&self.r#exif_data) {
				serialize_struct.serialize_field("exifData", {
					struct SerializeWith<'a>(&'a Vec<ExifDataProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#exif_data)
				})?;
			} else {
				serialize_struct.skip_field("exifData")?;
			}
			if !Vec::is_empty(&self.r#representative_of_page) {
				serialize_struct.serialize_field("representativeOfPage", {
					struct SerializeWith<'a>(&'a Vec<RepresentativeOfPageProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#representative_of_page)
				})?;
			} else {
				serialize_struct.skip_field("representativeOfPage")?;
			}
	...
	impl<'de> Deserialize<'de> for ImageObjectSnapshot {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
					...
					E: de::Error,
				{
					match value {
						...
						"exifData" => Ok(Field::ExifData),
						"representativeOfPage" => Ok(Field::RepresentativeOfPage),
					...
					E: de::Error,
				{
					match value {
						...
						b"exifData" => Ok(Field::ExifData),
						b"representativeOfPage" => Ok(Field::RepresentativeOfPage),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = ImageObjectSnapshot;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema ImageObjectSnapshot")
				}
					...
					A: de::MapAccess<'de>,
				{
					...
					let mut r#exif_data_property = None;
					let mut r#representative_of_page_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							...
							Field::ExifData => {
								if r#exif_data_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"exifData",
									));
								}
								r#exif_data_property = Some({
									struct DeserializeWith(Vec<ExifDataProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
							...
							Field::RepresentativeOfPage => {
								if r#representative_of_page_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"representativeOfPage",
									));
								}
								r#representative_of_page_property = Some({
									struct DeserializeWith(Vec<RepresentativeOfPageProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
					...
					Ok(ImageObjectSnapshot {
						...
						r#embedded_text_caption: r#embedded_text_caption_property
							.unwrap_or_default(),
						r#exif_data: r#exif_data_property.unwrap_or_default(),
						r#representative_of_page: r#representative_of_page_property
							.unwrap_or_default(),
			...
			deserializer.deserialize_struct("ImageObjectSnapshot", FIELDS, ClassVisitor)
		}
	}
}
