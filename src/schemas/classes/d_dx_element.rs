...
impl DDxElementTrait for DDxElement {
	...
	fn get_distinguishing_sign(&self) -> &[DistinguishingSignProperty] {
		self.r#distinguishing_sign.as_slice()
	}
	fn take_distinguishing_sign(&mut self) -> Vec<DistinguishingSignProperty> {
		std::mem::take(&mut self.r#distinguishing_sign)
	}
}
impl MedicalEntityTrait for DDxElement {
...
impl MedicalIntangibleTrait for DDxElement {}
impl ThingTrait for DDxElement {
...
mod serde {
	...
	impl Serialize for DDxElement {
			...
			S: Serializer,
		{
			let len: usize = [
				...
				!Vec::is_empty(&self.r#distinguishing_sign) as usize,
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "DDxElement", len)?;
			...
			if !Vec::is_empty(&self.r#distinguishing_sign) {
				serialize_struct.serialize_field("distinguishingSign", {
					struct SerializeWith<'a>(&'a Vec<DistinguishingSignProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#distinguishing_sign)
				})?;
			} else {
				serialize_struct.skip_field("distinguishingSign")?;
			}
	...
	impl<'de> Deserialize<'de> for DDxElement {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
					...
					E: de::Error,
				{
					match value {
						...
						"distinguishingSign" => Ok(Field::DistinguishingSign),
						"code" => Ok(Field::Code),
					...
					E: de::Error,
				{
					match value {
						...
						b"distinguishingSign" => Ok(Field::DistinguishingSign),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = DDxElement;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema DDxElement")
				}
					...
					A: de::MapAccess<'de>,
				{
					...
					let mut r#distinguishing_sign_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							...
							Field::DistinguishingSign => {
								if r#distinguishing_sign_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"distinguishingSign",
									));
								}
								r#distinguishing_sign_property = Some({
									struct DeserializeWith(Vec<DistinguishingSignProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
					...
					Ok(DDxElement {
						...
						r#distinguishing_sign: r#distinguishing_sign_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("DDxElement", FIELDS, ClassVisitor)
		}
	}
}
