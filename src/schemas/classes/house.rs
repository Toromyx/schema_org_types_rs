...
impl AccommodationTrait for House {
...
mod serde {
	...
	impl Serialize for House {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "House", len)?;
	...
	impl<'de> Deserialize<'de> for House {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = House;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema House")
				}
			...
			deserializer.deserialize_struct("House", FIELDS, ClassVisitor)
		}
	}
}
