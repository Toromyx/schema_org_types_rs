			...
			S: Serializer,
		{
			...
			if !Vec::is_empty(&self.r#occupation_location) {
				serialize_struct.serialize_field("occupationLocation", {
					...
					&SerializeWith(&self.r#occupation_location)
				})?;
			} else {
	...
	impl<'de> Deserialize<'de> for Occupation {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Occupation;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Occupation")
				}
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							...
							Field::OccupationLocation => {
								...
								r#occupation_location_property = Some({
									struct DeserializeWith(Vec<OccupationLocationProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
