...
pub trait AnswerTrait {
	/// Get <https://schema.org/answerExplanation> from [`Self`] as borrowed slice.
	fn get_answer_explanation(&self) -> &[AnswerExplanationProperty];
	/// Take <https://schema.org/answerExplanation> from [`Self`] as owned vector.
...
impl AnswerTrait for Answer {
	fn get_answer_explanation(&self) -> &[AnswerExplanationProperty] {
		self.r#answer_explanation.as_slice()
	}
	fn take_answer_explanation(&mut self) -> Vec<AnswerExplanationProperty> {
		std::mem::take(&mut self.r#answer_explanation)
	}
	fn get_parent_item(&self) -> &[ParentItemProperty] {
...
impl CreativeWorkTrait for Answer {
...
mod serde {
	...
	impl Serialize for Answer {
		...
		{
			let len: usize = [
				!Vec::is_empty(&self.r#answer_explanation) as usize,
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Answer", len)?;
			if !Vec::is_empty(&self.r#answer_explanation) {
				serialize_struct.serialize_field("answerExplanation", {
					struct SerializeWith<'a>(&'a Vec<AnswerExplanationProperty>);
					...
					&SerializeWith(&self.r#answer_explanation)
				})?;
			} else {
				serialize_struct.skip_field("answerExplanation")?;
			}
	...
	impl<'de> Deserialize<'de> for Answer {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
				...
				{
					match value {
						"answerExplanation" => Ok(Field::AnswerExplanation),
				...
				{
					match value {
						b"answerExplanation" => Ok(Field::AnswerExplanation),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Answer;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Answer")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#answer_explanation_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::AnswerExplanation => {
								if r#answer_explanation_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"answerExplanation",
									));
								}
								r#answer_explanation_property = Some({
									struct DeserializeWith(Vec<AnswerExplanationProperty>);
					...
					Ok(Answer {
						r#answer_explanation: r#answer_explanation_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("Answer", FIELDS, ClassVisitor)
		}
	}
}
