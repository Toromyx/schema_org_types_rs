...
mod serde {
	...
	use super::*;
	impl Serialize for ParentAudience {
		fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
		where
			S: Serializer,
		{
			let len: usize = [
				!Vec::is_empty(&self.r#child_max_age) as usize,
				!Vec::is_empty(&self.r#child_min_age) as usize,
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "ParentAudience", len)?;
			if !Vec::is_empty(&self.r#child_max_age) {
				serialize_struct.serialize_field("childMaxAge", {
					struct SerializeWith<'a>(&'a Vec<ChildMaxAgeProperty>);
					...
					&SerializeWith(&self.r#child_max_age)
				})?;
			} else {
				serialize_struct.skip_field("childMaxAge")?;
			}
			if !Vec::is_empty(&self.r#child_min_age) {
				serialize_struct.serialize_field("childMinAge", {
					struct SerializeWith<'a>(&'a Vec<ChildMinAgeProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#child_min_age)
				})?;
			} else {
				serialize_struct.skip_field("childMinAge")?;
			}
	...
	impl<'de> Deserialize<'de> for ParentAudience {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
				...
				{
					match value {
						"childMaxAge" => Ok(Field::ChildMaxAge),
						"childMinAge" => Ok(Field::ChildMinAge),
				...
				{
					match value {
						b"childMaxAge" => Ok(Field::ChildMaxAge),
						b"childMinAge" => Ok(Field::ChildMinAge),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = ParentAudience;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema ParentAudience")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#child_max_age_property = None;
					let mut r#child_min_age_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::ChildMaxAge => {
								if r#child_max_age_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"childMaxAge",
									));
								}
								r#child_max_age_property = Some({
									struct DeserializeWith(Vec<ChildMaxAgeProperty>);
							...
							Field::ChildMinAge => {
								if r#child_min_age_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"childMinAge",
									));
								}
								r#child_min_age_property = Some({
									struct DeserializeWith(Vec<ChildMinAgeProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
					...
					Ok(ParentAudience {
						r#child_max_age: r#child_max_age_property.unwrap_or_default(),
						r#child_min_age: r#child_min_age_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("ParentAudience", FIELDS, ClassVisitor)
		}
	}
}
