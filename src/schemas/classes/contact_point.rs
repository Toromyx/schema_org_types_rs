	...
	impl Serialize for ContactPoint {
			...
			S: Serializer,
		{
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "ContactPoint", len)?;
	...
	impl<'de> Deserialize<'de> for ContactPoint {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				...
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema ContactPoint")
				}
			...
			deserializer.deserialize_struct("ContactPoint", FIELDS, ClassVisitor)
		}
	}
}
