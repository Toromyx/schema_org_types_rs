...
impl TvSeasonTrait for TvSeason {
	fn get_country_of_origin(&self) -> &[CountryOfOriginProperty] {
	...
	fn get_part_of_tv_series(&self) -> &[PartOfTvSeriesProperty] {
		self.r#part_of_tv_series.as_slice()
	}
	fn take_part_of_tv_series(&mut self) -> Vec<PartOfTvSeriesProperty> {
		std::mem::take(&mut self.r#part_of_tv_series)
	}
	fn get_title_eidr(&self) -> &[TitleEidrProperty] {
...
impl CreativeWorkTrait for TvSeason {
...
impl CreativeWorkSeasonTrait for TvSeason {
	...
	fn get_season_number(&self) -> &[SeasonNumberProperty] {
		self.r#season_number.as_slice()
	}
	fn take_season_number(&mut self) -> Vec<SeasonNumberProperty> {
		std::mem::take(&mut self.r#season_number)
	}
...
mod serde {
	...
	impl Serialize for TvSeason {
			...
			S: Serializer,
		{
			let len: usize = [
				...
				!Vec::is_empty(&self.r#part_of_tv_series) as usize,
				...
				!Vec::is_empty(&self.r#season_number) as usize,
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "TvSeason", len)?;
			...
			if !Vec::is_empty(&self.r#part_of_tv_series) {
				serialize_struct.serialize_field("partOfTVSeries", {
					struct SerializeWith<'a>(&'a Vec<PartOfTvSeriesProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#part_of_tv_series)
				})?;
			} else {
				serialize_struct.skip_field("partOfTVSeries")?;
			}
			...
			if !Vec::is_empty(&self.r#season_number) {
				serialize_struct.serialize_field("seasonNumber", {
					struct SerializeWith<'a>(&'a Vec<SeasonNumberProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#season_number)
				})?;
			} else {
				serialize_struct.skip_field("seasonNumber")?;
			}
	...
	impl<'de> Deserialize<'de> for TvSeason {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
					...
					E: de::Error,
				{
					match value {
						...
						"partOfTVSeries" => Ok(Field::PartOfTvSeries),
						...
						"seasonNumber" => Ok(Field::SeasonNumber),
					...
					E: de::Error,
				{
					match value {
						...
						b"partOfTVSeries" => Ok(Field::PartOfTvSeries),
						...
						b"seasonNumber" => Ok(Field::SeasonNumber),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = TvSeason;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema TVSeason")
				}
					...
					A: de::MapAccess<'de>,
				{
					...
					let mut r#part_of_tv_series_property = None;
					...
					let mut r#season_number_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							...
							Field::PartOfTvSeries => {
								if r#part_of_tv_series_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"partOfTVSeries",
									));
								}
								r#part_of_tv_series_property = Some({
									struct DeserializeWith(Vec<PartOfTvSeriesProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
							...
							Field::SeasonNumber => {
								if r#season_number_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"seasonNumber",
									));
								}
								r#season_number_property = Some({
									struct DeserializeWith(Vec<SeasonNumberProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
					...
					Ok(TvSeason {
						...
						r#part_of_tv_series: r#part_of_tv_series_property.unwrap_or_default(),
						...
						r#season_number: r#season_number_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("TvSeason", FIELDS, ClassVisitor)
		}
	}
}
