...
impl CreativeWorkTrait for Clip {
...
mod serde {
	...
	impl Serialize for Clip {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Clip", len)?;
	...
	impl<'de> Deserialize<'de> for Clip {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Clip;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Clip")
				}
			...
			deserializer.deserialize_struct("Clip", FIELDS, ClassVisitor)
		}
	}
}
