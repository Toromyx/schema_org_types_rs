...
pub trait ReceiveActionTrait {
	/// Get <https://schema.org/deliveryMethod> from [`Self`] as borrowed slice.
	fn get_delivery_method(&self) -> &[DeliveryMethodProperty];
	/// Take <https://schema.org/deliveryMethod> from [`Self`] as owned vector.
...
impl ReceiveActionTrait for ReceiveAction {
	fn get_delivery_method(&self) -> &[DeliveryMethodProperty] {
		self.r#delivery_method.as_slice()
	}
	fn take_delivery_method(&mut self) -> Vec<DeliveryMethodProperty> {
		std::mem::take(&mut self.r#delivery_method)
	}
	fn get_sender(&self) -> &[SenderProperty] {
...
impl ThingTrait for ReceiveAction {
...
impl TransferActionTrait for ReceiveAction {
...
mod serde {
	...
	use super::*;
	impl Serialize for ReceiveAction {
			...
			S: Serializer,
		{
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "ReceiveAction", len)?;
			if !Vec::is_empty(&self.r#delivery_method) {
				serialize_struct.serialize_field("deliveryMethod", {
					struct SerializeWith<'a>(&'a Vec<DeliveryMethodProperty>);
					...
					&SerializeWith(&self.r#delivery_method)
				})?;
			} else {
				serialize_struct.skip_field("deliveryMethod")?;
			}
	...
	impl<'de> Deserialize<'de> for ReceiveAction {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = ReceiveAction;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema ReceiveAction")
				}
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::DeliveryMethod => {
								if r#delivery_method_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"deliveryMethod",
									));
								}
								r#delivery_method_property = Some({
									struct DeserializeWith(Vec<DeliveryMethodProperty>);
			...
			deserializer.deserialize_struct("ReceiveAction", FIELDS, ClassVisitor)
		}
	}
}
