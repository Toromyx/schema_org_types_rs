...
impl HotelRoomTrait for HotelRoom {
	fn get_bed(&self) -> &[BedProperty] {
		self.r#bed.as_slice()
	}
	fn take_bed(&mut self) -> Vec<BedProperty> {
...
impl AccommodationTrait for HotelRoom {
...
impl RoomTrait for HotelRoom {}
impl ThingTrait for HotelRoom {
...
mod serde {
	...
	impl Serialize for HotelRoom {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "HotelRoom", len)?;
	...
	impl<'de> Deserialize<'de> for HotelRoom {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = HotelRoom;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema HotelRoom")
				}
			...
			deserializer.deserialize_struct("HotelRoom", FIELDS, ClassVisitor)
		}
	}
}
