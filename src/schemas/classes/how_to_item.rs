...
/// This trait is for properties from <https://schema.org/HowToItem>.
pub trait HowToItemTrait {
	/// Get <https://schema.org/requiredQuantity> from [`Self`] as borrowed slice.
	fn get_required_quantity(&self) -> &[RequiredQuantityProperty];
	/// Take <https://schema.org/requiredQuantity> from [`Self`] as owned vector.
...
mod serde {
	...
	impl<'de> Deserialize<'de> for HowToItem {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = HowToItem;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema HowToItem")
				}
			...
			deserializer.deserialize_struct("HowToItem", FIELDS, ClassVisitor)
		}
	}
}
