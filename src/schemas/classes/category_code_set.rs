...
/// This trait is for properties from <https://schema.org/CategoryCodeSet>.
pub trait CategoryCodeSetTrait {
	/// Get <https://schema.org/hasCategoryCode> from [`Self`] as borrowed slice.
	fn get_has_category_code(&self) -> &[HasCategoryCodeProperty];
	/// Take <https://schema.org/hasCategoryCode> from [`Self`] as owned vector.
	fn take_has_category_code(&mut self) -> Vec<HasCategoryCodeProperty>;
}
impl CategoryCodeSetTrait for CategoryCodeSet {
	fn get_has_category_code(&self) -> &[HasCategoryCodeProperty] {
		self.r#has_category_code.as_slice()
	}
	fn take_has_category_code(&mut self) -> Vec<HasCategoryCodeProperty> {
		std::mem::take(&mut self.r#has_category_code)
	}
}
impl CreativeWorkTrait for CategoryCodeSet {
...
impl DefinedTermSetTrait for CategoryCodeSet {
...
impl ThingTrait for CategoryCodeSet {
...
mod serde {
	...
	use super::*;
	impl Serialize for CategoryCodeSet {
		fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
		where
			S: Serializer,
		{
			let len: usize = [
				!Vec::is_empty(&self.r#has_category_code) as usize,
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "CategoryCodeSet", len)?;
			if !Vec::is_empty(&self.r#has_category_code) {
				serialize_struct.serialize_field("hasCategoryCode", {
					struct SerializeWith<'a>(&'a Vec<HasCategoryCodeProperty>);
					...
					&SerializeWith(&self.r#has_category_code)
				})?;
			} else {
				serialize_struct.skip_field("hasCategoryCode")?;
			}
	...
	impl<'de> Deserialize<'de> for CategoryCodeSet {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
				...
				{
					match value {
						"hasCategoryCode" => Ok(Field::HasCategoryCode),
				...
				{
					match value {
						b"hasCategoryCode" => Ok(Field::HasCategoryCode),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = CategoryCodeSet;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema CategoryCodeSet")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#has_category_code_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::HasCategoryCode => {
								if r#has_category_code_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"hasCategoryCode",
									));
								}
								r#has_category_code_property = Some({
									struct DeserializeWith(Vec<HasCategoryCodeProperty>);
					...
					Ok(CategoryCodeSet {
						r#has_category_code: r#has_category_code_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("CategoryCodeSet", FIELDS, ClassVisitor)
		}
	}
}
