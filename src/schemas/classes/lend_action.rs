...
pub struct LendAction {
	...
	pub r#borrower: Vec<BorrowerProperty>,
...
/// This trait is for properties from <https://schema.org/LendAction>.
pub trait LendActionTrait {
	/// Get <https://schema.org/borrower> from [`Self`] as borrowed slice.
	fn get_borrower(&self) -> &[BorrowerProperty];
	/// Take <https://schema.org/borrower> from [`Self`] as owned vector.
	fn take_borrower(&mut self) -> Vec<BorrowerProperty>;
}
impl LendActionTrait for LendAction {
	fn get_borrower(&self) -> &[BorrowerProperty] {
		self.r#borrower.as_slice()
	}
	fn take_borrower(&mut self) -> Vec<BorrowerProperty> {
		std::mem::take(&mut self.r#borrower)
	}
}
...
impl ThingTrait for LendAction {
...
impl TransferActionTrait for LendAction {
...
mod serde {
	...
	use super::*;
	impl Serialize for LendAction {
		fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
		where
			S: Serializer,
		{
			let len: usize = [
				!Vec::is_empty(&self.r#borrower) as usize,
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "LendAction", len)?;
			if !Vec::is_empty(&self.r#borrower) {
				serialize_struct.serialize_field("borrower", {
					struct SerializeWith<'a>(&'a Vec<BorrowerProperty>);
					...
					&SerializeWith(&self.r#borrower)
				})?;
			} else {
				serialize_struct.skip_field("borrower")?;
			}
	...
	impl<'de> Deserialize<'de> for LendAction {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = LendAction;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema LendAction")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#borrower_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::Borrower => {
								if r#borrower_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"borrower",
									));
								}
								r#borrower_property = Some({
									struct DeserializeWith(Vec<BorrowerProperty>);
					...
					Ok(LendAction {
						r#borrower: r#borrower_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("LendAction", FIELDS, ClassVisitor)
		}
	}
}
