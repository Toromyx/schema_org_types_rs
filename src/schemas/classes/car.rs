...
mod serde {
	...
	impl Serialize for Car {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Car", len)?;
	...
	impl<'de> Deserialize<'de> for Car {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Car;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Car")
				}
			...
			deserializer.deserialize_struct("Car", FIELDS, ClassVisitor)
		}
	}
}
