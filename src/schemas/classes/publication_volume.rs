...
/// This trait is for properties from <https://schema.org/PublicationVolume>.
pub trait PublicationVolumeTrait {
	...
	/// Get <https://schema.org/volumeNumber> from [`Self`] as borrowed slice.
	fn get_volume_number(&self) -> &[VolumeNumberProperty];
	/// Take <https://schema.org/volumeNumber> from [`Self`] as owned vector.
	fn take_volume_number(&mut self) -> Vec<VolumeNumberProperty>;
}
impl PublicationVolumeTrait for PublicationVolume {
	fn get_page_end(&self) -> &[PageEndProperty] {
	...
	fn get_volume_number(&self) -> &[VolumeNumberProperty] {
		self.r#volume_number.as_slice()
	}
	fn take_volume_number(&mut self) -> Vec<VolumeNumberProperty> {
		std::mem::take(&mut self.r#volume_number)
	}
}
impl CreativeWorkTrait for PublicationVolume {
...
impl ThingTrait for PublicationVolume {
...
mod serde {
	...
	use super::*;
	impl Serialize for PublicationVolume {
			...
			S: Serializer,
		{
			let len: usize = [
				...
				!Vec::is_empty(&self.r#volume_number) as usize,
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "PublicationVolume", len)?;
			...
			if !Vec::is_empty(&self.r#volume_number) {
				serialize_struct.serialize_field("volumeNumber", {
					struct SerializeWith<'a>(&'a Vec<VolumeNumberProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#volume_number)
				})?;
			} else {
				serialize_struct.skip_field("volumeNumber")?;
			}
	...
	impl<'de> Deserialize<'de> for PublicationVolume {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
					...
					E: de::Error,
				{
					match value {
						...
						"volumeNumber" => Ok(Field::VolumeNumber),
					...
					E: de::Error,
				{
					match value {
						...
						b"volumeNumber" => Ok(Field::VolumeNumber),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = PublicationVolume;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema PublicationVolume")
				}
					...
					A: de::MapAccess<'de>,
				{
					...
					let mut r#volume_number_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							...
							Field::VolumeNumber => {
								if r#volume_number_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"volumeNumber",
									));
								}
								r#volume_number_property = Some({
									struct DeserializeWith(Vec<VolumeNumberProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
					...
					Ok(PublicationVolume {
						...
						r#volume_number: r#volume_number_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("PublicationVolume", FIELDS, ClassVisitor)
		}
	}
}
