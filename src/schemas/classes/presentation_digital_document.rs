...
/// This trait is for properties from <https://schema.org/PresentationDigitalDocument>.
pub trait PresentationDigitalDocumentTrait {}
impl PresentationDigitalDocumentTrait for PresentationDigitalDocument {}
impl CreativeWorkTrait for PresentationDigitalDocument {
...
impl DigitalDocumentTrait for PresentationDigitalDocument {
	fn get_has_digital_document_permission(&self) -> &[HasDigitalDocumentPermissionProperty] {
		self.r#has_digital_document_permission.as_slice()
	}
	fn take_has_digital_document_permission(
		&mut self,
	) -> Vec<HasDigitalDocumentPermissionProperty> {
		std::mem::take(&mut self.r#has_digital_document_permission)
	}
}
impl ThingTrait for PresentationDigitalDocument {
...
mod serde {
	...
	use super::*;
	impl Serialize for PresentationDigitalDocument {
			...
			S: Serializer,
		{
			let len: usize = [
				...
				!Vec::is_empty(&self.r#has_digital_document_permission) as usize,
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "PresentationDigitalDocument", len)?;
			...
			if !Vec::is_empty(&self.r#has_digital_document_permission) {
				serialize_struct.serialize_field("hasDigitalDocumentPermission", {
					struct SerializeWith<'a>(&'a Vec<HasDigitalDocumentPermissionProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#has_digital_document_permission)
				})?;
			} else {
				serialize_struct.skip_field("hasDigitalDocumentPermission")?;
			}
	...
	impl<'de> Deserialize<'de> for PresentationDigitalDocument {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
					...
					E: de::Error,
				{
					match value {
						...
						"hasDigitalDocumentPermission" => Ok(Field::HasDigitalDocumentPermission),
					...
					E: de::Error,
				{
					match value {
						...
						b"hasDigitalDocumentPermission" => Ok(Field::HasDigitalDocumentPermission),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = PresentationDigitalDocument;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema PresentationDigitalDocument")
				}
					...
					A: de::MapAccess<'de>,
				{
					...
					let mut r#has_digital_document_permission_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							...
							Field::HasDigitalDocumentPermission => {
								if r#has_digital_document_permission_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"hasDigitalDocumentPermission",
									));
								}
								r#has_digital_document_permission_property = Some({
					...
					Ok(PresentationDigitalDocument {
						...
						r#has_digital_document_permission:
							r#has_digital_document_permission_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("PresentationDigitalDocument", FIELDS, ClassVisitor)
		}
	}
}
