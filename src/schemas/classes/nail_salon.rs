...
/// This trait is for properties from <https://schema.org/NailSalon>.
pub trait NailSalonTrait {}
impl NailSalonTrait for NailSalon {}
impl HealthAndBeautyBusinessTrait for NailSalon {}
impl LocalBusinessTrait for NailSalon {
...
mod serde {
	...
	impl Serialize for NailSalon {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "NailSalon", len)?;
	...
	impl<'de> Deserialize<'de> for NailSalon {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = NailSalon;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema NailSalon")
				}
			...
			deserializer.deserialize_struct("NailSalon", FIELDS, ClassVisitor)
		}
	}
}
