...
impl SendActionTrait for SendAction {
	fn get_delivery_method(&self) -> &[DeliveryMethodProperty] {
...
impl ThingTrait for SendAction {
...
impl TransferActionTrait for SendAction {
...
mod serde {
	...
	impl Serialize for SendAction {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "SendAction", len)?;
	...
	impl<'de> Deserialize<'de> for SendAction {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = SendAction;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema SendAction")
				}
			...
			deserializer.deserialize_struct("SendAction", FIELDS, ClassVisitor)
		}
	}
}
