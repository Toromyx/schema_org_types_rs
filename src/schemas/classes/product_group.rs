...
pub trait ProductGroupTrait {
	/// Get <https://schema.org/hasVariant> from [`Self`] as borrowed slice.
	fn get_has_variant(&self) -> &[HasVariantProperty];
	/// Take <https://schema.org/hasVariant> from [`Self`] as owned vector.
	fn take_has_variant(&mut self) -> Vec<HasVariantProperty>;
	/// Get <https://schema.org/productGroupID> from [`Self`] as borrowed slice.
	fn get_product_group_id(&self) -> &[ProductGroupIdProperty];
	/// Take <https://schema.org/productGroupID> from [`Self`] as owned vector.
	fn take_product_group_id(&mut self) -> Vec<ProductGroupIdProperty>;
	/// Get <https://schema.org/variesBy> from [`Self`] as borrowed slice.
	fn get_varies_by(&self) -> &[VariesByProperty];
	/// Take <https://schema.org/variesBy> from [`Self`] as owned vector.
	fn take_varies_by(&mut self) -> Vec<VariesByProperty>;
}
impl ProductGroupTrait for ProductGroup {
	fn get_has_variant(&self) -> &[HasVariantProperty] {
		self.r#has_variant.as_slice()
	}
	fn take_has_variant(&mut self) -> Vec<HasVariantProperty> {
		std::mem::take(&mut self.r#has_variant)
	}
	fn get_product_group_id(&self) -> &[ProductGroupIdProperty] {
		self.r#product_group_id.as_slice()
	}
	fn take_product_group_id(&mut self) -> Vec<ProductGroupIdProperty> {
		std::mem::take(&mut self.r#product_group_id)
	}
	fn get_varies_by(&self) -> &[VariesByProperty] {
		self.r#varies_by.as_slice()
	}
	fn take_varies_by(&mut self) -> Vec<VariesByProperty> {
		std::mem::take(&mut self.r#varies_by)
	}
}
