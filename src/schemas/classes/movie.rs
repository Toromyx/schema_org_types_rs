...
impl CreativeWorkTrait for Movie {
...
mod serde {
	...
	impl Serialize for Movie {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Movie", len)?;
	...
	impl<'de> Deserialize<'de> for Movie {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Movie;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Movie")
				}
			...
			deserializer.deserialize_struct("Movie", FIELDS, ClassVisitor)
		}
	}
}
