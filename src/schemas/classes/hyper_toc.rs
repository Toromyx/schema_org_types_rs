...
pub trait HyperTocTrait {
	...
	/// Get <https://schema.org/tocEntry> from [`Self`] as borrowed slice.
	fn get_toc_entry(&self) -> &[TocEntryProperty];
	/// Take <https://schema.org/tocEntry> from [`Self`] as owned vector.
	fn take_toc_entry(&mut self) -> Vec<TocEntryProperty>;
}
impl HyperTocTrait for HyperToc {
	fn get_associated_media(&self) -> &[AssociatedMediaProperty] {
	...
	fn get_toc_entry(&self) -> &[TocEntryProperty] {
		self.r#toc_entry.as_slice()
	}
	fn take_toc_entry(&mut self) -> Vec<TocEntryProperty> {
		std::mem::take(&mut self.r#toc_entry)
	}
}
impl CreativeWorkTrait for HyperToc {
...
mod serde {
	...
	impl Serialize for HyperToc {
			...
			S: Serializer,
		{
			let len: usize = [
				...
				!Vec::is_empty(&self.r#toc_entry) as usize,
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "HyperToc", len)?;
			...
			if !Vec::is_empty(&self.r#toc_entry) {
				serialize_struct.serialize_field("tocEntry", {
					struct SerializeWith<'a>(&'a Vec<TocEntryProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#toc_entry)
				})?;
			} else {
				serialize_struct.skip_field("tocEntry")?;
			}
	...
	impl<'de> Deserialize<'de> for HyperToc {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = HyperToc;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema HyperToc")
				}
					...
					A: de::MapAccess<'de>,
				{
					...
					let mut r#toc_entry_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							...
							Field::TocEntry => {
								if r#toc_entry_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"tocEntry",
									));
								}
								r#toc_entry_property = Some({
									struct DeserializeWith(Vec<TocEntryProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
					...
					Ok(HyperToc {
						...
						r#toc_entry: r#toc_entry_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("HyperToc", FIELDS, ClassVisitor)
		}
	}
}
