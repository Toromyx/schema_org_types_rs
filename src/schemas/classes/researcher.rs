...
/// This trait is for properties from <https://schema.org/Researcher>.
pub trait ResearcherTrait {}
impl ResearcherTrait for Researcher {}
impl AudienceTrait for Researcher {
...
impl ThingTrait for Researcher {
...
mod serde {
	...
	impl Serialize for Researcher {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Researcher", len)?;
	...
	impl<'de> Deserialize<'de> for Researcher {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Researcher;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Researcher")
				}
			...
			deserializer.deserialize_struct("Researcher", FIELDS, ClassVisitor)
		}
	}
}
