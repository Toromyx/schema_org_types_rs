...
/// This trait is for properties from <https://schema.org/Festival>.
pub trait FestivalTrait {}
...
mod serde {
	...
	impl Serialize for Festival {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Festival", len)?;
	...
	impl<'de> Deserialize<'de> for Festival {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Festival;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Festival")
				}
			...
			deserializer.deserialize_struct("Festival", FIELDS, ClassVisitor)
		}
	}
}
