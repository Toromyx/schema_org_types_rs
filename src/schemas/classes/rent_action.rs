...
pub struct RentAction {
	...
	pub r#landlord: Vec<LandlordProperty>,
	/// <https://schema.org/realEstateAgent>
	pub r#real_estate_agent: Vec<RealEstateAgentProperty>,
...
pub trait RentActionTrait {
	/// Get <https://schema.org/landlord> from [`Self`] as borrowed slice.
	fn get_landlord(&self) -> &[LandlordProperty];
	/// Take <https://schema.org/landlord> from [`Self`] as owned vector.
	fn take_landlord(&mut self) -> Vec<LandlordProperty>;
	/// Get <https://schema.org/realEstateAgent> from [`Self`] as borrowed slice.
	fn get_real_estate_agent(&self) -> &[RealEstateAgentProperty];
	/// Take <https://schema.org/realEstateAgent> from [`Self`] as owned vector.
	fn take_real_estate_agent(&mut self) -> Vec<RealEstateAgentProperty>;
}
impl RentActionTrait for RentAction {
	fn get_landlord(&self) -> &[LandlordProperty] {
		self.r#landlord.as_slice()
	}
	fn take_landlord(&mut self) -> Vec<LandlordProperty> {
		std::mem::take(&mut self.r#landlord)
	}
	fn get_real_estate_agent(&self) -> &[RealEstateAgentProperty] {
		self.r#real_estate_agent.as_slice()
	}
	fn take_real_estate_agent(&mut self) -> Vec<RealEstateAgentProperty> {
		std::mem::take(&mut self.r#real_estate_agent)
	}
}
...
impl ThingTrait for RentAction {
...
impl TradeActionTrait for RentAction {
...
mod serde {
	...
	use super::*;
	impl Serialize for RentAction {
		fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
		where
			S: Serializer,
		{
			let len: usize = [
				!Vec::is_empty(&self.r#landlord) as usize,
				!Vec::is_empty(&self.r#real_estate_agent) as usize,
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "RentAction", len)?;
			if !Vec::is_empty(&self.r#landlord) {
				serialize_struct.serialize_field("landlord", {
					struct SerializeWith<'a>(&'a Vec<LandlordProperty>);
					...
					&SerializeWith(&self.r#landlord)
				})?;
			} else {
				serialize_struct.skip_field("landlord")?;
			}
			if !Vec::is_empty(&self.r#real_estate_agent) {
				serialize_struct.serialize_field("realEstateAgent", {
					struct SerializeWith<'a>(&'a Vec<RealEstateAgentProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#real_estate_agent)
				})?;
			} else {
				serialize_struct.skip_field("realEstateAgent")?;
			}
	...
	impl<'de> Deserialize<'de> for RentAction {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
				...
				{
					match value {
						"landlord" => Ok(Field::Landlord),
						"realEstateAgent" => Ok(Field::RealEstateAgent),
				...
				{
					match value {
						b"landlord" => Ok(Field::Landlord),
						b"realEstateAgent" => Ok(Field::RealEstateAgent),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = RentAction;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema RentAction")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#landlord_property = None;
					let mut r#real_estate_agent_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::Landlord => {
								if r#landlord_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"landlord",
									));
								}
								r#landlord_property = Some({
									struct DeserializeWith(Vec<LandlordProperty>);
							...
							Field::RealEstateAgent => {
								if r#real_estate_agent_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"realEstateAgent",
									));
								}
								r#real_estate_agent_property = Some({
									struct DeserializeWith(Vec<RealEstateAgentProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
					...
					Ok(RentAction {
						r#landlord: r#landlord_property.unwrap_or_default(),
						r#real_estate_agent: r#real_estate_agent_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("RentAction", FIELDS, ClassVisitor)
		}
	}
}
