...
mod serde {
	...
	impl Serialize for Protein {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Protein", len)?;
	...
	impl<'de> Deserialize<'de> for Protein {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Protein;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Protein")
				}
			...
			deserializer.deserialize_struct("Protein", FIELDS, ClassVisitor)
		}
	}
}
