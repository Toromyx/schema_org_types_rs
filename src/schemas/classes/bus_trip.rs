...
pub trait BusTripTrait {
	/// Get <https://schema.org/arrivalBusStop> from [`Self`] as borrowed slice.
	fn get_arrival_bus_stop(&self) -> &[ArrivalBusStopProperty];
	/// Take <https://schema.org/arrivalBusStop> from [`Self`] as owned vector.
	fn take_arrival_bus_stop(&mut self) -> Vec<ArrivalBusStopProperty>;
	/// Get <https://schema.org/busName> from [`Self`] as borrowed slice.
	fn get_bus_name(&self) -> &[BusNameProperty];
	/// Take <https://schema.org/busName> from [`Self`] as owned vector.
	fn take_bus_name(&mut self) -> Vec<BusNameProperty>;
	/// Get <https://schema.org/busNumber> from [`Self`] as borrowed slice.
	fn get_bus_number(&self) -> &[BusNumberProperty];
	/// Take <https://schema.org/busNumber> from [`Self`] as owned vector.
	fn take_bus_number(&mut self) -> Vec<BusNumberProperty>;
	/// Get <https://schema.org/departureBusStop> from [`Self`] as borrowed slice.
	fn get_departure_bus_stop(&self) -> &[DepartureBusStopProperty];
	/// Take <https://schema.org/departureBusStop> from [`Self`] as owned vector.
	fn take_departure_bus_stop(&mut self) -> Vec<DepartureBusStopProperty>;
}
impl BusTripTrait for BusTrip {
	fn get_arrival_bus_stop(&self) -> &[ArrivalBusStopProperty] {
		self.r#arrival_bus_stop.as_slice()
	}
	fn take_arrival_bus_stop(&mut self) -> Vec<ArrivalBusStopProperty> {
		std::mem::take(&mut self.r#arrival_bus_stop)
	}
	fn get_bus_name(&self) -> &[BusNameProperty] {
		self.r#bus_name.as_slice()
	}
	fn take_bus_name(&mut self) -> Vec<BusNameProperty> {
		std::mem::take(&mut self.r#bus_name)
	}
	fn get_bus_number(&self) -> &[BusNumberProperty] {
		self.r#bus_number.as_slice()
	}
	fn take_bus_number(&mut self) -> Vec<BusNumberProperty> {
		std::mem::take(&mut self.r#bus_number)
	}
	fn get_departure_bus_stop(&self) -> &[DepartureBusStopProperty] {
		self.r#departure_bus_stop.as_slice()
	}
	fn take_departure_bus_stop(&mut self) -> Vec<DepartureBusStopProperty> {
		std::mem::take(&mut self.r#departure_bus_stop)
	}
}
impl ThingTrait for BusTrip {
...
mod serde {
	...
	use super::*;
	impl Serialize for BusTrip {
		fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
		where
			S: Serializer,
		{
			let len: usize = [
				!Vec::is_empty(&self.r#arrival_bus_stop) as usize,
				!Vec::is_empty(&self.r#bus_name) as usize,
				!Vec::is_empty(&self.r#bus_number) as usize,
				!Vec::is_empty(&self.r#departure_bus_stop) as usize,
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "BusTrip", len)?;
			if !Vec::is_empty(&self.r#arrival_bus_stop) {
				serialize_struct.serialize_field("arrivalBusStop", {
					struct SerializeWith<'a>(&'a Vec<ArrivalBusStopProperty>);
					...
					&SerializeWith(&self.r#arrival_bus_stop)
				})?;
			} else {
				serialize_struct.skip_field("arrivalBusStop")?;
			}
			if !Vec::is_empty(&self.r#bus_name) {
				serialize_struct.serialize_field("busName", {
					struct SerializeWith<'a>(&'a Vec<BusNameProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#bus_name)
				})?;
			} else {
				serialize_struct.skip_field("busName")?;
			}
			if !Vec::is_empty(&self.r#bus_number) {
				serialize_struct.serialize_field("busNumber", {
					struct SerializeWith<'a>(&'a Vec<BusNumberProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#bus_number)
				})?;
			} else {
				serialize_struct.skip_field("busNumber")?;
			}
			if !Vec::is_empty(&self.r#departure_bus_stop) {
				serialize_struct.serialize_field("departureBusStop", {
					struct SerializeWith<'a>(&'a Vec<DepartureBusStopProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#departure_bus_stop)
				})?;
			} else {
				serialize_struct.skip_field("departureBusStop")?;
			}
	...
	impl<'de> Deserialize<'de> for BusTrip {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
				...
				{
					match value {
						"arrivalBusStop" => Ok(Field::ArrivalBusStop),
						"busName" => Ok(Field::BusName),
						"busNumber" => Ok(Field::BusNumber),
						"departureBusStop" => Ok(Field::DepartureBusStop),
				...
				{
					match value {
						b"arrivalBusStop" => Ok(Field::ArrivalBusStop),
						b"busName" => Ok(Field::BusName),
						b"busNumber" => Ok(Field::BusNumber),
						b"departureBusStop" => Ok(Field::DepartureBusStop),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = BusTrip;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema BusTrip")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#arrival_bus_stop_property = None;
					let mut r#bus_name_property = None;
					let mut r#bus_number_property = None;
					let mut r#departure_bus_stop_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::ArrivalBusStop => {
								if r#arrival_bus_stop_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"arrivalBusStop",
									));
								}
								r#arrival_bus_stop_property = Some({
									struct DeserializeWith(Vec<ArrivalBusStopProperty>);
							...
							Field::BusName => {
								if r#bus_name_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"busName",
									));
								}
								r#bus_name_property = Some({
									struct DeserializeWith(Vec<BusNameProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
							...
							Field::BusNumber => {
								if r#bus_number_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"busNumber",
									));
								}
								r#bus_number_property = Some({
									struct DeserializeWith(Vec<BusNumberProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
							...
							Field::DepartureBusStop => {
								if r#departure_bus_stop_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"departureBusStop",
									));
								}
								r#departure_bus_stop_property = Some({
									struct DeserializeWith(Vec<DepartureBusStopProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
					...
					Ok(BusTrip {
						r#arrival_bus_stop: r#arrival_bus_stop_property.unwrap_or_default(),
						r#bus_name: r#bus_name_property.unwrap_or_default(),
						r#bus_number: r#bus_number_property.unwrap_or_default(),
						r#departure_bus_stop: r#departure_bus_stop_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("BusTrip", FIELDS, ClassVisitor)
		}
	}
}
