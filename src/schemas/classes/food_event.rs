...
/// This trait is for properties from <https://schema.org/FoodEvent>.
...
mod serde {
	...
	impl Serialize for FoodEvent {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "FoodEvent", len)?;
	...
	impl<'de> Deserialize<'de> for FoodEvent {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = FoodEvent;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema FoodEvent")
				}
			...
			deserializer.deserialize_struct("FoodEvent", FIELDS, ClassVisitor)
		}
	}
}
