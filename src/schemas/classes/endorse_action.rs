...
pub struct EndorseAction {
	...
	pub r#endorsee: Vec<EndorseeProperty>,
...
impl EndorseActionTrait for EndorseAction {
	fn get_endorsee(&self) -> &[EndorseeProperty] {
		self.r#endorsee.as_slice()
	}
	fn take_endorsee(&mut self) -> Vec<EndorseeProperty> {
		std::mem::take(&mut self.r#endorsee)
	}
}
...
impl AssessActionTrait for EndorseAction {}
impl ReactActionTrait for EndorseAction {}
impl ThingTrait for EndorseAction {
...
mod serde {
	...
	use super::*;
	impl Serialize for EndorseAction {
		fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
		where
			S: Serializer,
		{
			let len: usize = [
				!Vec::is_empty(&self.r#endorsee) as usize,
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "EndorseAction", len)?;
			if !Vec::is_empty(&self.r#endorsee) {
				serialize_struct.serialize_field("endorsee", {
					struct SerializeWith<'a>(&'a Vec<EndorseeProperty>);
					...
					&SerializeWith(&self.r#endorsee)
				})?;
			} else {
				serialize_struct.skip_field("endorsee")?;
			}
	...
	impl<'de> Deserialize<'de> for EndorseAction {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = EndorseAction;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema EndorseAction")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#endorsee_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::Endorsee => {
								if r#endorsee_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"endorsee",
									));
								}
								r#endorsee_property = Some({
									struct DeserializeWith(Vec<EndorseeProperty>);
					...
					Ok(EndorseAction {
						r#endorsee: r#endorsee_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("EndorseAction", FIELDS, ClassVisitor)
		}
	}
}
