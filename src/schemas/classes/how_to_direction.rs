...
impl HowToDirectionTrait for HowToDirection {
	fn get_after_media(&self) -> &[AfterMediaProperty] {
		self.r#after_media.as_slice()
	}
	fn take_after_media(&mut self) -> Vec<AfterMediaProperty> {
		std::mem::take(&mut self.r#after_media)
	}
	fn get_before_media(&self) -> &[BeforeMediaProperty] {
		self.r#before_media.as_slice()
	}
	fn take_before_media(&mut self) -> Vec<BeforeMediaProperty> {
		std::mem::take(&mut self.r#before_media)
	}
	fn get_during_media(&self) -> &[DuringMediaProperty] {
		self.r#during_media.as_slice()
	}
	fn take_during_media(&mut self) -> Vec<DuringMediaProperty> {
		std::mem::take(&mut self.r#during_media)
	}
	fn get_perform_time(&self) -> &[PerformTimeProperty] {
...
impl CreativeWorkTrait for HowToDirection {
...
impl ListItemTrait for HowToDirection {
...
impl ThingTrait for HowToDirection {
...
mod serde {
	...
	use super::*;
	impl Serialize for HowToDirection {
		fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
		where
			S: Serializer,
		{
			let len: usize = [
				!Vec::is_empty(&self.r#after_media) as usize,
				!Vec::is_empty(&self.r#before_media) as usize,
				!Vec::is_empty(&self.r#during_media) as usize,
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "HowToDirection", len)?;
			if !Vec::is_empty(&self.r#after_media) {
				serialize_struct.serialize_field("afterMedia", {
					struct SerializeWith<'a>(&'a Vec<AfterMediaProperty>);
					...
					&SerializeWith(&self.r#after_media)
				})?;
			} else {
				serialize_struct.skip_field("afterMedia")?;
			}
			if !Vec::is_empty(&self.r#before_media) {
				serialize_struct.serialize_field("beforeMedia", {
					struct SerializeWith<'a>(&'a Vec<BeforeMediaProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#before_media)
				})?;
			} else {
				serialize_struct.skip_field("beforeMedia")?;
			}
			if !Vec::is_empty(&self.r#during_media) {
				serialize_struct.serialize_field("duringMedia", {
					struct SerializeWith<'a>(&'a Vec<DuringMediaProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#during_media)
				})?;
			} else {
				serialize_struct.skip_field("duringMedia")?;
			}
	...
	impl<'de> Deserialize<'de> for HowToDirection {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
				...
				{
					match value {
						"afterMedia" => Ok(Field::AfterMedia),
						"beforeMedia" => Ok(Field::BeforeMedia),
						"duringMedia" => Ok(Field::DuringMedia),
				...
				{
					match value {
						b"afterMedia" => Ok(Field::AfterMedia),
						b"beforeMedia" => Ok(Field::BeforeMedia),
						b"duringMedia" => Ok(Field::DuringMedia),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = HowToDirection;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema HowToDirection")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#after_media_property = None;
					let mut r#before_media_property = None;
					let mut r#during_media_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::AfterMedia => {
								if r#after_media_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"afterMedia",
									));
								}
								r#after_media_property = Some({
									struct DeserializeWith(Vec<AfterMediaProperty>);
							...
							Field::BeforeMedia => {
								if r#before_media_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"beforeMedia",
									));
								}
								r#before_media_property = Some({
									struct DeserializeWith(Vec<BeforeMediaProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
							...
							Field::DuringMedia => {
								if r#during_media_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"duringMedia",
									));
								}
								r#during_media_property = Some({
									struct DeserializeWith(Vec<DuringMediaProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
					...
					Ok(HowToDirection {
						r#after_media: r#after_media_property.unwrap_or_default(),
						r#before_media: r#before_media_property.unwrap_or_default(),
						r#during_media: r#during_media_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("HowToDirection", FIELDS, ClassVisitor)
		}
	}
}
