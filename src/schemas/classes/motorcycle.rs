...
/// This trait is for properties from <https://schema.org/Motorcycle>.
pub trait MotorcycleTrait {}
impl MotorcycleTrait for Motorcycle {}
...
mod serde {
	...
	impl Serialize for Motorcycle {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Motorcycle", len)?;
	...
	impl<'de> Deserialize<'de> for Motorcycle {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				...
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Motorcycle")
				}
			...
			deserializer.deserialize_struct("Motorcycle", FIELDS, ClassVisitor)
		}
	}
}
