...
/// This trait is for properties from <https://schema.org/Country>.
pub trait CountryTrait {}
impl CountryTrait for Country {}
impl AdministrativeAreaTrait for Country {}
...
mod serde {
	...
	impl Serialize for Country {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Country", len)?;
	...
	impl<'de> Deserialize<'de> for Country {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Country;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Country")
				}
			...
			deserializer.deserialize_struct("Country", FIELDS, ClassVisitor)
		}
	}
}
