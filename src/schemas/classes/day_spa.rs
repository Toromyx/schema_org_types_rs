...
/// This trait is for properties from <https://schema.org/DaySpa>.
pub trait DaySpaTrait {}
impl DaySpaTrait for DaySpa {}
impl HealthAndBeautyBusinessTrait for DaySpa {}
impl LocalBusinessTrait for DaySpa {
...
mod serde {
	...
	impl Serialize for DaySpa {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "DaySpa", len)?;
	...
	impl<'de> Deserialize<'de> for DaySpa {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = DaySpa;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema DaySpa")
				}
			...
			deserializer.deserialize_struct("DaySpa", FIELDS, ClassVisitor)
		}
	}
}
