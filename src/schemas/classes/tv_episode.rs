...
impl TvEpisodeTrait for TvEpisode {
	fn get_country_of_origin(&self) -> &[CountryOfOriginProperty] {
...
impl CreativeWorkTrait for TvEpisode {
...
impl EpisodeTrait for TvEpisode {
...
mod serde {
	...
	impl Serialize for TvEpisode {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "TvEpisode", len)?;
	...
	impl<'de> Deserialize<'de> for TvEpisode {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = TvEpisode;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema TVEpisode")
				}
			...
			deserializer.deserialize_struct("TvEpisode", FIELDS, ClassVisitor)
		}
	}
}
