...
/// This trait is for properties from <https://schema.org/City>.
pub trait CityTrait {}
impl CityTrait for City {}
impl AdministrativeAreaTrait for City {}
...
mod serde {
	...
	impl Serialize for City {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "City", len)?;
	...
	impl<'de> Deserialize<'de> for City {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = City;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema City")
				}
			...
			deserializer.deserialize_struct("City", FIELDS, ClassVisitor)
		}
	}
}
