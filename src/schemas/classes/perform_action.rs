...
pub struct PerformAction {
	...
	pub r#entertainment_business: Vec<EntertainmentBusinessProperty>,
...
pub trait PerformActionTrait {
	/// Get <https://schema.org/entertainmentBusiness> from [`Self`] as borrowed slice.
	fn get_entertainment_business(&self) -> &[EntertainmentBusinessProperty];
	/// Take <https://schema.org/entertainmentBusiness> from [`Self`] as owned vector.
	fn take_entertainment_business(&mut self) -> Vec<EntertainmentBusinessProperty>;
}
impl PerformActionTrait for PerformAction {
	fn get_entertainment_business(&self) -> &[EntertainmentBusinessProperty] {
		self.r#entertainment_business.as_slice()
	}
	fn take_entertainment_business(&mut self) -> Vec<EntertainmentBusinessProperty> {
		std::mem::take(&mut self.r#entertainment_business)
	}
}
...
impl PlayActionTrait for PerformAction {
...
impl ThingTrait for PerformAction {
...
mod serde {
	...
	impl Serialize for PerformAction {
		...
		{
			let len: usize = [
				!Vec::is_empty(&self.r#entertainment_business) as usize,
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "PerformAction", len)?;
			if !Vec::is_empty(&self.r#entertainment_business) {
				serialize_struct.serialize_field("entertainmentBusiness", {
					struct SerializeWith<'a>(&'a Vec<EntertainmentBusinessProperty>);
					...
					&SerializeWith(&self.r#entertainment_business)
				})?;
			} else {
				serialize_struct.skip_field("entertainmentBusiness")?;
			}
	...
	impl<'de> Deserialize<'de> for PerformAction {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
				...
				{
					match value {
						"entertainmentBusiness" => Ok(Field::EntertainmentBusiness),
				...
				{
					match value {
						b"entertainmentBusiness" => Ok(Field::EntertainmentBusiness),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = PerformAction;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema PerformAction")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#entertainment_business_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::EntertainmentBusiness => {
								if r#entertainment_business_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"entertainmentBusiness",
									));
								}
								r#entertainment_business_property = Some({
									struct DeserializeWith(Vec<EntertainmentBusinessProperty>);
					...
					Ok(PerformAction {
						r#entertainment_business: r#entertainment_business_property
							.unwrap_or_default(),
			...
			deserializer.deserialize_struct("PerformAction", FIELDS, ClassVisitor)
		}
	}
}
