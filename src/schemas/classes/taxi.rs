...
mod serde {
	...
	impl Serialize for Taxi {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Taxi", len)?;
	...
	impl<'de> Deserialize<'de> for Taxi {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Taxi;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Taxi")
				}
			...
			deserializer.deserialize_struct("Taxi", FIELDS, ClassVisitor)
		}
	}
}
