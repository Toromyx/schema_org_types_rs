...
/// This trait is for properties from <https://schema.org/Library>.
pub trait LibraryTrait {}
impl LibraryTrait for Library {}
impl LocalBusinessTrait for Library {
...
mod serde {
	...
	impl Serialize for Library {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Library", len)?;
	...
	impl<'de> Deserialize<'de> for Library {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Library;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Library")
				}
			...
			deserializer.deserialize_struct("Library", FIELDS, ClassVisitor)
		}
	}
}
