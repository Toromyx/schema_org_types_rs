...
pub trait MathSolverTrait {
	/// Get <https://schema.org/mathExpression> from [`Self`] as borrowed slice.
	fn get_math_expression(&self) -> &[MathExpressionProperty];
	/// Take <https://schema.org/mathExpression> from [`Self`] as owned vector.
	fn take_math_expression(&mut self) -> Vec<MathExpressionProperty>;
}
impl MathSolverTrait for MathSolver {
	fn get_math_expression(&self) -> &[MathExpressionProperty] {
		self.r#math_expression.as_slice()
	}
	fn take_math_expression(&mut self) -> Vec<MathExpressionProperty> {
		std::mem::take(&mut self.r#math_expression)
	}
}
impl CreativeWorkTrait for MathSolver {
...
impl ThingTrait for MathSolver {
...
mod serde {
	...
	use super::*;
	impl Serialize for MathSolver {
		fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
		where
			S: Serializer,
		{
			let len: usize = [
				!Vec::is_empty(&self.r#math_expression) as usize,
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "MathSolver", len)?;
			if !Vec::is_empty(&self.r#math_expression) {
				serialize_struct.serialize_field("mathExpression", {
					struct SerializeWith<'a>(&'a Vec<MathExpressionProperty>);
					...
					&SerializeWith(&self.r#math_expression)
				})?;
			} else {
				serialize_struct.skip_field("mathExpression")?;
			}
	...
	impl<'de> Deserialize<'de> for MathSolver {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
				...
				{
					match value {
						"mathExpression" => Ok(Field::MathExpression),
				...
				{
					match value {
						b"mathExpression" => Ok(Field::MathExpression),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = MathSolver;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema MathSolver")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#math_expression_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::MathExpression => {
								if r#math_expression_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"mathExpression",
									));
								}
								r#math_expression_property = Some({
									struct DeserializeWith(Vec<MathExpressionProperty>);
					...
					Ok(MathSolver {
						r#math_expression: r#math_expression_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("MathSolver", FIELDS, ClassVisitor)
		}
	}
}
