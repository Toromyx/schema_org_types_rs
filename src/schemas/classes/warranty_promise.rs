...
pub trait WarrantyPromiseTrait {
	/// Get <https://schema.org/durationOfWarranty> from [`Self`] as borrowed slice.
	fn get_duration_of_warranty(&self) -> &[DurationOfWarrantyProperty];
	/// Take <https://schema.org/durationOfWarranty> from [`Self`] as owned vector.
	fn take_duration_of_warranty(&mut self) -> Vec<DurationOfWarrantyProperty>;
	/// Get <https://schema.org/warrantyScope> from [`Self`] as borrowed slice.
	fn get_warranty_scope(&self) -> &[WarrantyScopeProperty];
	/// Take <https://schema.org/warrantyScope> from [`Self`] as owned vector.
...
mod serde {
	...
	use super::*;
	impl Serialize for WarrantyPromise {
	...
	impl<'de> Deserialize<'de> for WarrantyPromise {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = WarrantyPromise;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema WarrantyPromise")
				}
