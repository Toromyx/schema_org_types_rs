...
pub trait MedicalScholarlyArticleTrait {
	/// Get <https://schema.org/publicationType> from [`Self`] as borrowed slice.
	fn get_publication_type(&self) -> &[PublicationTypeProperty];
	/// Take <https://schema.org/publicationType> from [`Self`] as owned vector.
	fn take_publication_type(&mut self) -> Vec<PublicationTypeProperty>;
}
impl MedicalScholarlyArticleTrait for MedicalScholarlyArticle {
	fn get_publication_type(&self) -> &[PublicationTypeProperty] {
		self.r#publication_type.as_slice()
	}
	fn take_publication_type(&mut self) -> Vec<PublicationTypeProperty> {
		std::mem::take(&mut self.r#publication_type)
	}
}
impl ArticleTrait for MedicalScholarlyArticle {
...
impl CreativeWorkTrait for MedicalScholarlyArticle {
...
impl ScholarlyArticleTrait for MedicalScholarlyArticle {}
impl ThingTrait for MedicalScholarlyArticle {
...
mod serde {
	...
	use super::*;
	impl Serialize for MedicalScholarlyArticle {
		fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
		where
			S: Serializer,
		{
			let len: usize = [
				!Vec::is_empty(&self.r#publication_type) as usize,
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "MedicalScholarlyArticle", len)?;
			if !Vec::is_empty(&self.r#publication_type) {
				serialize_struct.serialize_field("publicationType", {
					struct SerializeWith<'a>(&'a Vec<PublicationTypeProperty>);
					...
					&SerializeWith(&self.r#publication_type)
				})?;
			} else {
				serialize_struct.skip_field("publicationType")?;
			}
	...
	impl<'de> Deserialize<'de> for MedicalScholarlyArticle {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
				...
				{
					match value {
						"publicationType" => Ok(Field::PublicationType),
				...
				{
					match value {
						b"publicationType" => Ok(Field::PublicationType),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = MedicalScholarlyArticle;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema MedicalScholarlyArticle")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#publication_type_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::PublicationType => {
								if r#publication_type_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"publicationType",
									));
								}
								r#publication_type_property = Some({
									struct DeserializeWith(Vec<PublicationTypeProperty>);
					...
					Ok(MedicalScholarlyArticle {
						r#publication_type: r#publication_type_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("MedicalScholarlyArticle", FIELDS, ClassVisitor)
		}
	}
}
