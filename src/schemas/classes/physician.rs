...
pub trait PhysicianTrait {
	...
	/// Get <https://schema.org/hospitalAffiliation> from [`Self`] as borrowed slice.
	fn get_hospital_affiliation(&self) -> &[HospitalAffiliationProperty];
	/// Take <https://schema.org/hospitalAffiliation> from [`Self`] as owned vector.
...
impl PhysicianTrait for Physician {
	...
	fn get_hospital_affiliation(&self) -> &[HospitalAffiliationProperty] {
		self.r#hospital_affiliation.as_slice()
	}
	fn take_hospital_affiliation(&mut self) -> Vec<HospitalAffiliationProperty> {
		std::mem::take(&mut self.r#hospital_affiliation)
	}
	fn get_medical_specialty(&self) -> &[MedicalSpecialtyProperty] {
...
impl MedicalBusinessTrait for Physician {}
impl MedicalOrganizationTrait for Physician {
...
mod serde {
	...
	impl Serialize for Physician {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Physician", len)?;
			...
			if !Vec::is_empty(&self.r#hospital_affiliation) {
				serialize_struct.serialize_field("hospitalAffiliation", {
					struct SerializeWith<'a>(&'a Vec<HospitalAffiliationProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#hospital_affiliation)
				})?;
			} else {
				serialize_struct.skip_field("hospitalAffiliation")?;
			}
	...
	impl<'de> Deserialize<'de> for Physician {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Physician;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Physician")
				}
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							...
							Field::HospitalAffiliation => {
								if r#hospital_affiliation_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"hospitalAffiliation",
									));
								}
								r#hospital_affiliation_property = Some({
									struct DeserializeWith(Vec<HospitalAffiliationProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
			...
			deserializer.deserialize_struct("Physician", FIELDS, ClassVisitor)
		}
	}
}
