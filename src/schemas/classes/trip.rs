...
mod serde {
	...
	impl Serialize for Trip {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Trip", len)?;
			#[cfg(any(
				any(
					feature = "additional-type-property-schema",
					feature = "general-schema-section"
	...
	impl<'de> Deserialize<'de> for Trip {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Trip;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Trip")
				}
			...
			deserializer.deserialize_struct("Trip", FIELDS, ClassVisitor)
		}
	}
}
