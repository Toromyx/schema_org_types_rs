...
/// This trait is for properties from <https://schema.org/Bridge>.
pub trait BridgeTrait {}
impl BridgeTrait for Bridge {}
impl CivicStructureTrait for Bridge {
...
mod serde {
	...
	impl Serialize for Bridge {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Bridge", len)?;
	...
	impl<'de> Deserialize<'de> for Bridge {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Bridge;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Bridge")
				}
			...
			deserializer.deserialize_struct("Bridge", FIELDS, ClassVisitor)
		}
	}
}
