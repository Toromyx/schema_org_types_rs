...
/// This trait is for properties from <https://schema.org/Embassy>.
pub trait EmbassyTrait {}
impl EmbassyTrait for Embassy {}
impl CivicStructureTrait for Embassy {
...
impl GovernmentBuildingTrait for Embassy {}
...
mod serde {
	...
	impl Serialize for Embassy {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Embassy", len)?;
	...
	impl<'de> Deserialize<'de> for Embassy {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Embassy;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Embassy")
				}
			...
			deserializer.deserialize_struct("Embassy", FIELDS, ClassVisitor)
		}
	}
}
