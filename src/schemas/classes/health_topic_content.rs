...
pub trait HealthTopicContentTrait {
	/// Get <https://schema.org/hasHealthAspect> from [`Self`] as borrowed slice.
	fn get_has_health_aspect(&self) -> &[HasHealthAspectProperty];
	/// Take <https://schema.org/hasHealthAspect> from [`Self`] as owned vector.
	fn take_has_health_aspect(&mut self) -> Vec<HasHealthAspectProperty>;
}
impl HealthTopicContentTrait for HealthTopicContent {
	fn get_has_health_aspect(&self) -> &[HasHealthAspectProperty] {
		self.r#has_health_aspect.as_slice()
	}
	fn take_has_health_aspect(&mut self) -> Vec<HasHealthAspectProperty> {
		std::mem::take(&mut self.r#has_health_aspect)
	}
}
impl CreativeWorkTrait for HealthTopicContent {
...
impl ThingTrait for HealthTopicContent {
...
impl WebContentTrait for HealthTopicContent {}
#[cfg(feature = "serde")]
mod serde {
	...
	use super::*;
	impl Serialize for HealthTopicContent {
		fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
		where
			S: Serializer,
		{
			let len: usize = [
				!Vec::is_empty(&self.r#has_health_aspect) as usize,
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "HealthTopicContent", len)?;
			if !Vec::is_empty(&self.r#has_health_aspect) {
				serialize_struct.serialize_field("hasHealthAspect", {
					struct SerializeWith<'a>(&'a Vec<HasHealthAspectProperty>);
					...
					&SerializeWith(&self.r#has_health_aspect)
				})?;
			} else {
				serialize_struct.skip_field("hasHealthAspect")?;
			}
	...
	impl<'de> Deserialize<'de> for HealthTopicContent {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
				...
				{
					match value {
						"hasHealthAspect" => Ok(Field::HasHealthAspect),
				...
				{
					match value {
						b"hasHealthAspect" => Ok(Field::HasHealthAspect),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = HealthTopicContent;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema HealthTopicContent")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#has_health_aspect_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::HasHealthAspect => {
								if r#has_health_aspect_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"hasHealthAspect",
									));
								}
								r#has_health_aspect_property = Some({
									struct DeserializeWith(Vec<HasHealthAspectProperty>);
					...
					Ok(HealthTopicContent {
						r#has_health_aspect: r#has_health_aspect_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("HealthTopicContent", FIELDS, ClassVisitor)
		}
	}
}
