...
impl MonetaryAmountTrait for MonetaryAmount {
	fn get_currency(&self) -> &[CurrencyProperty] {
...
impl StructuredValueTrait for MonetaryAmount {}
impl ThingTrait for MonetaryAmount {
	...
	impl Serialize for MonetaryAmount {
			...
			S: Serializer,
		{
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "MonetaryAmount", len)?;
	...
	impl<'de> Deserialize<'de> for MonetaryAmount {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = MonetaryAmount;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema MonetaryAmount")
				}
			...
			deserializer.deserialize_struct("MonetaryAmount", FIELDS, ClassVisitor)
		}
	}
}
