...
pub trait HyperTocEntryTrait {
	...
	/// Get <https://schema.org/tocContinuation> from [`Self`] as borrowed slice.
	fn get_toc_continuation(&self) -> &[TocContinuationProperty];
	/// Take <https://schema.org/tocContinuation> from [`Self`] as owned vector.
	fn take_toc_continuation(&mut self) -> Vec<TocContinuationProperty>;
	/// Get <https://schema.org/utterances> from [`Self`] as borrowed slice.
	fn get_utterances(&self) -> &[UtterancesProperty];
	/// Take <https://schema.org/utterances> from [`Self`] as owned vector.
	fn take_utterances(&mut self) -> Vec<UtterancesProperty>;
}
impl HyperTocEntryTrait for HyperTocEntry {
	fn get_associated_media(&self) -> &[AssociatedMediaProperty] {
	...
	fn get_toc_continuation(&self) -> &[TocContinuationProperty] {
		self.r#toc_continuation.as_slice()
	}
	fn take_toc_continuation(&mut self) -> Vec<TocContinuationProperty> {
		std::mem::take(&mut self.r#toc_continuation)
	}
	fn get_utterances(&self) -> &[UtterancesProperty] {
		self.r#utterances.as_slice()
	}
	fn take_utterances(&mut self) -> Vec<UtterancesProperty> {
		std::mem::take(&mut self.r#utterances)
	}
}
impl CreativeWorkTrait for HyperTocEntry {
...
impl ThingTrait for HyperTocEntry {
...
mod serde {
	...
	use super::*;
	impl Serialize for HyperTocEntry {
			...
			S: Serializer,
		{
			let len: usize = [
				...
				!Vec::is_empty(&self.r#toc_continuation) as usize,
				!Vec::is_empty(&self.r#utterances) as usize,
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "HyperTocEntry", len)?;
			...
			if !Vec::is_empty(&self.r#toc_continuation) {
				serialize_struct.serialize_field("tocContinuation", {
					struct SerializeWith<'a>(&'a Vec<TocContinuationProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#toc_continuation)
				})?;
			} else {
				serialize_struct.skip_field("tocContinuation")?;
			}
			if !Vec::is_empty(&self.r#utterances) {
				serialize_struct.serialize_field("utterances", {
					struct SerializeWith<'a>(&'a Vec<UtterancesProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#utterances)
				})?;
			} else {
				serialize_struct.skip_field("utterances")?;
			}
	...
	impl<'de> Deserialize<'de> for HyperTocEntry {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
					...
					E: de::Error,
				{
					match value {
						...
						"tocContinuation" => Ok(Field::TocContinuation),
						"utterances" => Ok(Field::Utterances),
					...
					E: de::Error,
				{
					match value {
						...
						b"tocContinuation" => Ok(Field::TocContinuation),
						b"utterances" => Ok(Field::Utterances),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = HyperTocEntry;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema HyperTocEntry")
				}
					...
					A: de::MapAccess<'de>,
				{
					...
					let mut r#toc_continuation_property = None;
					let mut r#utterances_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							...
							Field::TocContinuation => {
								if r#toc_continuation_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"tocContinuation",
									));
								}
								r#toc_continuation_property = Some({
									struct DeserializeWith(Vec<TocContinuationProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
							...
							Field::Utterances => {
								if r#utterances_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"utterances",
									));
								}
								r#utterances_property = Some({
									struct DeserializeWith(Vec<UtterancesProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
					...
					Ok(HyperTocEntry {
						...
						r#toc_continuation: r#toc_continuation_property.unwrap_or_default(),
						r#utterances: r#utterances_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("HyperTocEntry", FIELDS, ClassVisitor)
		}
	}
}
