...
/// This trait is for properties from <https://schema.org/MedicalGuidelineContraindication>.
pub trait MedicalGuidelineContraindicationTrait {}
impl MedicalGuidelineContraindicationTrait for MedicalGuidelineContraindication {}
impl MedicalEntityTrait for MedicalGuidelineContraindication {
...
impl MedicalGuidelineTrait for MedicalGuidelineContraindication {
...
impl ThingTrait for MedicalGuidelineContraindication {
...
mod serde {
	...
	use super::*;
	impl Serialize for MedicalGuidelineContraindication {
			...
			S: Serializer,
		{
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "MedicalGuidelineContraindication", len)?;
			...
			if !Vec::is_empty(&self.r#guideline_date) {
				serialize_struct.serialize_field("guidelineDate", {
					...
					&SerializeWith(&self.r#guideline_date)
				})?;
			} else {
				serialize_struct.skip_field("guidelineDate")?;
			}
			if !Vec::is_empty(&self.r#guideline_subject) {
				serialize_struct.serialize_field("guidelineSubject", {
					struct SerializeWith<'a>(&'a Vec<GuidelineSubjectProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#guideline_subject)
				})?;
			} else {
	...
	impl<'de> Deserialize<'de> for MedicalGuidelineContraindication {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = MedicalGuidelineContraindication;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema MedicalGuidelineContraindication")
				}
					...
					A: de::MapAccess<'de>,
				{
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							...
							Field::GuidelineDate => {
								...
								r#guideline_date_property = Some({
									struct DeserializeWith(Vec<GuidelineDateProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
							...
							Field::GuidelineSubject => {
								if r#guideline_subject_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"guidelineSubject",
									));
								}
					...
					Ok(MedicalGuidelineContraindication {
