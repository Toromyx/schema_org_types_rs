...
mod serde {
	...
	impl Serialize for Event {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Event", len)?;
	...
	impl<'de> Deserialize<'de> for Event {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Event;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Event")
				}
			...
			deserializer.deserialize_struct("Event", FIELDS, ClassVisitor)
		}
	}
}
