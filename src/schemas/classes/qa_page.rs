...
/// This trait is for properties from <https://schema.org/QAPage>.
pub trait QaPageTrait {}
impl QaPageTrait for QaPage {}
impl CreativeWorkTrait for QaPage {
...
mod serde {
	...
	impl Serialize for QaPage {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "QaPage", len)?;
	...
	impl<'de> Deserialize<'de> for QaPage {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = QaPage;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema QAPage")
				}
			...
			deserializer.deserialize_struct("QaPage", FIELDS, ClassVisitor)
		}
	}
}
