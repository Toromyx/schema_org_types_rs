...
/// This trait is for properties from <https://schema.org/BookStore>.
pub trait BookStoreTrait {}
impl BookStoreTrait for BookStore {}
impl LocalBusinessTrait for BookStore {
...
impl StoreTrait for BookStore {}
impl ThingTrait for BookStore {
...
mod serde {
	...
	impl Serialize for BookStore {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "BookStore", len)?;
	...
	impl<'de> Deserialize<'de> for BookStore {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = BookStore;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema BookStore")
				}
			...
			deserializer.deserialize_struct("BookStore", FIELDS, ClassVisitor)
		}
	}
}
