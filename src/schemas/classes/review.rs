...
pub trait ReviewTrait {
	...
	/// Take <https://schema.org/associatedClaimReview> from [`Self`] as owned vector.
	...
	/// Take <https://schema.org/associatedMediaReview> from [`Self`] as owned vector.
...
impl CreativeWorkTrait for Review {
...
mod serde {
	...
	impl Serialize for Review {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Review", len)?;
	...
	impl<'de> Deserialize<'de> for Review {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Review;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Review")
				}
			...
			deserializer.deserialize_struct("Review", FIELDS, ClassVisitor)
		}
	}
}
