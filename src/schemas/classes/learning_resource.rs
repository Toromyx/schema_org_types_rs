...
impl LearningResourceTrait for LearningResource {
	fn get_assesses(&self) -> &[AssessesProperty] {
...
impl CreativeWorkTrait for LearningResource {
...
impl ThingTrait for LearningResource {
...
mod serde {
	...
	use super::*;
	impl Serialize for LearningResource {
			...
			S: Serializer,
		{
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "LearningResource", len)?;
	...
	impl<'de> Deserialize<'de> for LearningResource {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = LearningResource;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema LearningResource")
				}
			...
			deserializer.deserialize_struct("LearningResource", FIELDS, ClassVisitor)
		}
	}
}
