...
pub trait DatasetTrait {
	...
	/// Take <https://schema.org/includedInDataCatalog> from [`Self`] as owned vector.
...
impl CreativeWorkTrait for Dataset {
...
mod serde {
	...
	impl Serialize for Dataset {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Dataset", len)?;
	...
	impl<'de> Deserialize<'de> for Dataset {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Dataset;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Dataset")
				}
			...
			deserializer.deserialize_struct("Dataset", FIELDS, ClassVisitor)
		}
	}
}
