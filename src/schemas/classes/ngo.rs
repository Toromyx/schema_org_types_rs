...
/// This trait is for properties from <https://schema.org/NGO>.
pub trait NgoTrait {}
impl NgoTrait for Ngo {}
...
mod serde {
	...
	impl Serialize for Ngo {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Ngo", len)?;
	...
	impl<'de> Deserialize<'de> for Ngo {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Ngo;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema NGO")
				}
			...
			deserializer.deserialize_struct("Ngo", FIELDS, ClassVisitor)
		}
	}
}
