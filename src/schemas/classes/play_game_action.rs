...
pub struct PlayGameAction {
	...
	pub r#game_availability_type: Vec<GameAvailabilityTypeProperty>,
...
pub trait PlayGameActionTrait {
	/// Get <https://schema.org/gameAvailabilityType> from [`Self`] as borrowed slice.
	fn get_game_availability_type(&self) -> &[GameAvailabilityTypeProperty];
	/// Take <https://schema.org/gameAvailabilityType> from [`Self`] as owned vector.
	fn take_game_availability_type(&mut self) -> Vec<GameAvailabilityTypeProperty>;
}
impl PlayGameActionTrait for PlayGameAction {
	fn get_game_availability_type(&self) -> &[GameAvailabilityTypeProperty] {
		self.r#game_availability_type.as_slice()
	}
	fn take_game_availability_type(&mut self) -> Vec<GameAvailabilityTypeProperty> {
		std::mem::take(&mut self.r#game_availability_type)
	}
}
...
impl ThingTrait for PlayGameAction {
...
mod serde {
	...
	use super::*;
	impl Serialize for PlayGameAction {
		fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
		where
			S: Serializer,
		{
			let len: usize = [
				!Vec::is_empty(&self.r#game_availability_type) as usize,
				...
				!Vec::is_empty(&self.r#action_accessibility_requirement) as usize,
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "PlayGameAction", len)?;
			if !Vec::is_empty(&self.r#game_availability_type) {
				serialize_struct.serialize_field("gameAvailabilityType", {
					struct SerializeWith<'a>(&'a Vec<GameAvailabilityTypeProperty>);
					...
					&SerializeWith(&self.r#game_availability_type)
				})?;
			} else {
				serialize_struct.skip_field("gameAvailabilityType")?;
			}
			...
			if !Vec::is_empty(&self.r#action_accessibility_requirement) {
				serialize_struct.serialize_field("actionAccessibilityRequirement", {
					struct SerializeWith<'a>(&'a Vec<ActionAccessibilityRequirementProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#action_accessibility_requirement)
				})?;
			} else {
				serialize_struct.skip_field("actionAccessibilityRequirement")?;
			}
	...
	impl<'de> Deserialize<'de> for PlayGameAction {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
				...
				{
					match value {
						"gameAvailabilityType" => Ok(Field::GameAvailabilityType),
						...
						"actionAccessibilityRequirement" => {
							Ok(Field::ActionAccessibilityRequirement)
						}
				...
				{
					match value {
						b"gameAvailabilityType" => Ok(Field::GameAvailabilityType),
						...
						b"actionAccessibilityRequirement" => {
							Ok(Field::ActionAccessibilityRequirement)
						}
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = PlayGameAction;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema PlayGameAction")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#game_availability_type_property = None;
					...
					let mut r#action_accessibility_requirement_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::GameAvailabilityType => {
								if r#game_availability_type_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"gameAvailabilityType",
									));
								}
								r#game_availability_type_property = Some({
									struct DeserializeWith(Vec<GameAvailabilityTypeProperty>);
							...
							Field::ActionAccessibilityRequirement => {
								if r#action_accessibility_requirement_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"actionAccessibilityRequirement",
									));
								}
								r#action_accessibility_requirement_property = Some({
					...
					Ok(PlayGameAction {
						r#game_availability_type: r#game_availability_type_property
							.unwrap_or_default(),
						...
						r#action_accessibility_requirement:
							r#action_accessibility_requirement_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("PlayGameAction", FIELDS, ClassVisitor)
		}
	}
}
