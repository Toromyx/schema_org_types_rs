...
pub trait ArchiveOrganizationTrait {
	/// Get <https://schema.org/archiveHeld> from [`Self`] as borrowed slice.
	fn get_archive_held(&self) -> &[ArchiveHeldProperty];
	/// Take <https://schema.org/archiveHeld> from [`Self`] as owned vector.
	fn take_archive_held(&mut self) -> Vec<ArchiveHeldProperty>;
}
impl ArchiveOrganizationTrait for ArchiveOrganization {
	fn get_archive_held(&self) -> &[ArchiveHeldProperty] {
		self.r#archive_held.as_slice()
	}
	fn take_archive_held(&mut self) -> Vec<ArchiveHeldProperty> {
		std::mem::take(&mut self.r#archive_held)
	}
}
impl LocalBusinessTrait for ArchiveOrganization {
...
impl ThingTrait for ArchiveOrganization {
...
mod serde {
	...
	use super::*;
	impl Serialize for ArchiveOrganization {
		fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
		where
			S: Serializer,
		{
			let len: usize = [
				!Vec::is_empty(&self.r#archive_held) as usize,
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "ArchiveOrganization", len)?;
			if !Vec::is_empty(&self.r#archive_held) {
				serialize_struct.serialize_field("archiveHeld", {
					struct SerializeWith<'a>(&'a Vec<ArchiveHeldProperty>);
					...
					&SerializeWith(&self.r#archive_held)
				})?;
			} else {
				serialize_struct.skip_field("archiveHeld")?;
			}
	...
	impl<'de> Deserialize<'de> for ArchiveOrganization {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
				...
				{
					match value {
						"archiveHeld" => Ok(Field::ArchiveHeld),
				...
				{
					match value {
						b"archiveHeld" => Ok(Field::ArchiveHeld),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = ArchiveOrganization;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema ArchiveOrganization")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#archive_held_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::ArchiveHeld => {
								if r#archive_held_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"archiveHeld",
									));
								}
								r#archive_held_property = Some({
									struct DeserializeWith(Vec<ArchiveHeldProperty>);
					...
					Ok(ArchiveOrganization {
						r#archive_held: r#archive_held_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("ArchiveOrganization", FIELDS, ClassVisitor)
		}
	}
}
