...
/// This trait is for properties from <https://schema.org/CivicStructure>.
pub trait CivicStructureTrait {
...
impl CivicStructureTrait for CivicStructure {
	fn get_opening_hours(&self) -> &[OpeningHoursProperty] {
...
impl ThingTrait for CivicStructure {
...
mod serde {
	...
	use super::*;
	impl Serialize for CivicStructure {
			...
			S: Serializer,
		{
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "CivicStructure", len)?;
	...
	impl<'de> Deserialize<'de> for CivicStructure {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = CivicStructure;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema CivicStructure")
				}
			...
			deserializer.deserialize_struct("CivicStructure", FIELDS, ClassVisitor)
		}
	}
}
