...
/// This trait is for properties from <https://schema.org/LegislationObject>.
pub trait LegislationObjectTrait {
	/// Get <https://schema.org/legislationLegalValue> from [`Self`] as borrowed slice.
	fn get_legislation_legal_value(&self) -> &[LegislationLegalValueProperty];
	/// Take <https://schema.org/legislationLegalValue> from [`Self`] as owned vector.
	fn take_legislation_legal_value(&mut self) -> Vec<LegislationLegalValueProperty>;
}
impl LegislationObjectTrait for LegislationObject {
	fn get_legislation_legal_value(&self) -> &[LegislationLegalValueProperty] {
		self.r#legislation_legal_value.as_slice()
	}
	fn take_legislation_legal_value(&mut self) -> Vec<LegislationLegalValueProperty> {
		std::mem::take(&mut self.r#legislation_legal_value)
	}
}
impl CreativeWorkTrait for LegislationObject {
...
impl LegislationTrait for LegislationObject {
	fn get_jurisdiction(&self) -> &[JurisdictionProperty] {
		self.r#jurisdiction.as_slice()
	}
	...
	fn take_legislation_applies(&mut self) -> Vec<LegislationAppliesProperty> {
		std::mem::take(&mut self.r#legislation_applies)
	}
	fn get_legislation_changes(&self) -> &[LegislationChangesProperty] {
		self.r#legislation_changes.as_slice()
	}
	...
	fn take_legislation_identifier(&mut self) -> Vec<LegislationIdentifierProperty> {
		std::mem::take(&mut self.r#legislation_identifier)
	}
	fn get_legislation_jurisdiction(&self) -> &[LegislationJurisdictionProperty] {
		self.r#legislation_jurisdiction.as_slice()
	}
	fn take_legislation_jurisdiction(&mut self) -> Vec<LegislationJurisdictionProperty> {
		std::mem::take(&mut self.r#legislation_jurisdiction)
	}
	fn get_legislation_legal_force(&self) -> &[LegislationLegalForceProperty] {
		self.r#legislation_legal_force.as_slice()
	}
	fn take_legislation_legal_force(&mut self) -> Vec<LegislationLegalForceProperty> {
		std::mem::take(&mut self.r#legislation_legal_force)
	}
	fn get_legislation_passed_by(&self) -> &[LegislationPassedByProperty] {
		self.r#legislation_passed_by.as_slice()
	}
...
impl ThingTrait for LegislationObject {
...
mod serde {
	...
	use super::*;
	impl Serialize for LegislationObject {
		fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
		where
			S: Serializer,
		{
			let len: usize = [
				!Vec::is_empty(&self.r#legislation_legal_value) as usize,
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "LegislationObject", len)?;
			if !Vec::is_empty(&self.r#legislation_legal_value) {
				serialize_struct.serialize_field("legislationLegalValue", {
					struct SerializeWith<'a>(&'a Vec<LegislationLegalValueProperty>);
					...
					&SerializeWith(&self.r#legislation_legal_value)
				})?;
			} else {
				serialize_struct.skip_field("legislationLegalValue")?;
			}
	...
	impl<'de> Deserialize<'de> for LegislationObject {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
				...
				{
					match value {
						"legislationLegalValue" => Ok(Field::LegislationLegalValue),
				...
				{
					match value {
						b"legislationLegalValue" => Ok(Field::LegislationLegalValue),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = LegislationObject;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema LegislationObject")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#legislation_legal_value_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::LegislationLegalValue => {
								if r#legislation_legal_value_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"legislationLegalValue",
									));
								}
								r#legislation_legal_value_property = Some({
									struct DeserializeWith(Vec<LegislationLegalValueProperty>);
					...
					Ok(LegislationObject {
						r#legislation_legal_value: r#legislation_legal_value_property
							.unwrap_or_default(),
			...
			deserializer.deserialize_struct("LegislationObject", FIELDS, ClassVisitor)
		}
	}
}
