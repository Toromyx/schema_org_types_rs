...
pub struct BorrowAction {
	...
	pub r#lender: Vec<LenderProperty>,
...
/// This trait is for properties from <https://schema.org/BorrowAction>.
pub trait BorrowActionTrait {
	/// Get <https://schema.org/lender> from [`Self`] as borrowed slice.
	fn get_lender(&self) -> &[LenderProperty];
	/// Take <https://schema.org/lender> from [`Self`] as owned vector.
	fn take_lender(&mut self) -> Vec<LenderProperty>;
}
impl BorrowActionTrait for BorrowAction {
	fn get_lender(&self) -> &[LenderProperty] {
		self.r#lender.as_slice()
	}
	fn take_lender(&mut self) -> Vec<LenderProperty> {
		std::mem::take(&mut self.r#lender)
	}
}
...
impl ThingTrait for BorrowAction {
...
impl TransferActionTrait for BorrowAction {
...
mod serde {
	...
	use super::*;
	impl Serialize for BorrowAction {
		fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
		where
			S: Serializer,
		{
			let len: usize = [
				!Vec::is_empty(&self.r#lender) as usize,
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "BorrowAction", len)?;
			if !Vec::is_empty(&self.r#lender) {
				serialize_struct.serialize_field("lender", {
					struct SerializeWith<'a>(&'a Vec<LenderProperty>);
					...
					&SerializeWith(&self.r#lender)
				})?;
			} else {
				serialize_struct.skip_field("lender")?;
			}
	...
	impl<'de> Deserialize<'de> for BorrowAction {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = BorrowAction;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema BorrowAction")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#lender_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::Lender => {
								if r#lender_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field("lender"));
								}
								r#lender_property = Some({
									struct DeserializeWith(Vec<LenderProperty>);
					...
					Ok(BorrowAction {
						r#lender: r#lender_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("BorrowAction", FIELDS, ClassVisitor)
		}
	}
}
