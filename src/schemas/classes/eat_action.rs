...
/// This trait is for properties from <https://schema.org/EatAction>.
pub trait EatActionTrait {}
impl EatActionTrait for EatAction {}
...
impl ConsumeActionTrait for EatAction {
...
mod serde {
	...
	impl Serialize for EatAction {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "EatAction", len)?;
	...
	impl<'de> Deserialize<'de> for EatAction {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = EatAction;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema EatAction")
				}
			...
			deserializer.deserialize_struct("EatAction", FIELDS, ClassVisitor)
		}
	}
}
