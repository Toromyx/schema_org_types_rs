...
pub struct ReviewAction {
	...
	pub r#result_review: Vec<ResultReviewProperty>,
...
/// This trait is for properties from <https://schema.org/ReviewAction>.
pub trait ReviewActionTrait {
	/// Get <https://schema.org/resultReview> from [`Self`] as borrowed slice.
	fn get_result_review(&self) -> &[ResultReviewProperty];
	/// Take <https://schema.org/resultReview> from [`Self`] as owned vector.
	fn take_result_review(&mut self) -> Vec<ResultReviewProperty>;
}
impl ReviewActionTrait for ReviewAction {
	fn get_result_review(&self) -> &[ResultReviewProperty] {
		self.r#result_review.as_slice()
	}
	fn take_result_review(&mut self) -> Vec<ResultReviewProperty> {
		std::mem::take(&mut self.r#result_review)
	}
}
...
impl AssessActionTrait for ReviewAction {}
impl ThingTrait for ReviewAction {
...
mod serde {
	...
	use super::*;
	impl Serialize for ReviewAction {
		fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
		where
			S: Serializer,
		{
			let len: usize = [
				!Vec::is_empty(&self.r#result_review) as usize,
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "ReviewAction", len)?;
			if !Vec::is_empty(&self.r#result_review) {
				serialize_struct.serialize_field("resultReview", {
					struct SerializeWith<'a>(&'a Vec<ResultReviewProperty>);
					...
					&SerializeWith(&self.r#result_review)
				})?;
			} else {
				serialize_struct.skip_field("resultReview")?;
			}
	...
	impl<'de> Deserialize<'de> for ReviewAction {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
				...
				{
					match value {
						"resultReview" => Ok(Field::ResultReview),
				...
				{
					match value {
						b"resultReview" => Ok(Field::ResultReview),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = ReviewAction;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema ReviewAction")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#result_review_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::ResultReview => {
								if r#result_review_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"resultReview",
									));
								}
								r#result_review_property = Some({
									struct DeserializeWith(Vec<ResultReviewProperty>);
					...
					Ok(ReviewAction {
						r#result_review: r#result_review_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("ReviewAction", FIELDS, ClassVisitor)
		}
	}
}
