...
impl CreativeWorkTrait for Chapter {
...
mod serde {
	...
	impl Serialize for Chapter {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Chapter", len)?;
	...
	impl<'de> Deserialize<'de> for Chapter {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Chapter;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Chapter")
				}
			...
			deserializer.deserialize_struct("Chapter", FIELDS, ClassVisitor)
		}
	}
}
