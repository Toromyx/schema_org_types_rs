...
/// This trait is for properties from <https://schema.org/WPAdBlock>.
pub trait WpAdBlockTrait {}
impl WpAdBlockTrait for WpAdBlock {}
impl CreativeWorkTrait for WpAdBlock {
...
impl WebPageElementTrait for WpAdBlock {
...
mod serde {
	...
	impl Serialize for WpAdBlock {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "WpAdBlock", len)?;
	...
	impl<'de> Deserialize<'de> for WpAdBlock {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = WpAdBlock;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema WPAdBlock")
				}
			...
			deserializer.deserialize_struct("WpAdBlock", FIELDS, ClassVisitor)
		}
	}
}
