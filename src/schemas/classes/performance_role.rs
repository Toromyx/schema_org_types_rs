...
pub struct PerformanceRole {
	...
	pub r#character_name: Vec<CharacterNameProperty>,
...
pub trait PerformanceRoleTrait {
	/// Get <https://schema.org/characterName> from [`Self`] as borrowed slice.
	fn get_character_name(&self) -> &[CharacterNameProperty];
	/// Take <https://schema.org/characterName> from [`Self`] as owned vector.
	fn take_character_name(&mut self) -> Vec<CharacterNameProperty>;
}
impl PerformanceRoleTrait for PerformanceRole {
	fn get_character_name(&self) -> &[CharacterNameProperty] {
		self.r#character_name.as_slice()
	}
	fn take_character_name(&mut self) -> Vec<CharacterNameProperty> {
		std::mem::take(&mut self.r#character_name)
	}
}
impl RoleTrait for PerformanceRole {
...
impl ThingTrait for PerformanceRole {
...
mod serde {
	...
	use super::*;
	impl Serialize for PerformanceRole {
		fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
		where
			S: Serializer,
		{
			let len: usize = [
				!Vec::is_empty(&self.r#character_name) as usize,
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "PerformanceRole", len)?;
			if !Vec::is_empty(&self.r#character_name) {
				serialize_struct.serialize_field("characterName", {
					struct SerializeWith<'a>(&'a Vec<CharacterNameProperty>);
					...
					&SerializeWith(&self.r#character_name)
				})?;
			} else {
				serialize_struct.skip_field("characterName")?;
			}
	...
	impl<'de> Deserialize<'de> for PerformanceRole {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
				...
				{
					match value {
						"characterName" => Ok(Field::CharacterName),
				...
				{
					match value {
						b"characterName" => Ok(Field::CharacterName),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = PerformanceRole;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema PerformanceRole")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#character_name_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::CharacterName => {
								if r#character_name_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"characterName",
									));
								}
								r#character_name_property = Some({
									struct DeserializeWith(Vec<CharacterNameProperty>);
					...
					Ok(PerformanceRole {
						r#character_name: r#character_name_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("PerformanceRole", FIELDS, ClassVisitor)
		}
	}
}
