...
impl ScreeningEventTrait for ScreeningEvent {
	...
	fn get_work_presented(&self) -> &[WorkPresentedProperty] {
		self.r#work_presented.as_slice()
	}
	fn take_work_presented(&mut self) -> Vec<WorkPresentedProperty> {
		std::mem::take(&mut self.r#work_presented)
	}
}
impl EventTrait for ScreeningEvent {
...
impl ThingTrait for ScreeningEvent {
...
mod serde {
	...
	use super::*;
	impl Serialize for ScreeningEvent {
			...
			S: Serializer,
		{
			let len: usize = [
				...
				!Vec::is_empty(&self.r#work_presented) as usize,
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "ScreeningEvent", len)?;
			...
			if !Vec::is_empty(&self.r#work_presented) {
				serialize_struct.serialize_field("workPresented", {
					struct SerializeWith<'a>(&'a Vec<WorkPresentedProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#work_presented)
				})?;
			} else {
				serialize_struct.skip_field("workPresented")?;
			}
	...
	impl<'de> Deserialize<'de> for ScreeningEvent {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
					...
					E: de::Error,
				{
					match value {
						...
						"workPresented" => Ok(Field::WorkPresented),
					...
					E: de::Error,
				{
					match value {
						...
						b"workPresented" => Ok(Field::WorkPresented),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = ScreeningEvent;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema ScreeningEvent")
				}
					...
					A: de::MapAccess<'de>,
				{
					...
					let mut r#work_presented_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							...
							Field::WorkPresented => {
								if r#work_presented_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"workPresented",
									));
								}
								r#work_presented_property = Some({
									struct DeserializeWith(Vec<WorkPresentedProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
					...
					Ok(ScreeningEvent {
						...
						r#work_presented: r#work_presented_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("ScreeningEvent", FIELDS, ClassVisitor)
		}
	}
}
