...
pub trait ImageObjectTrait {
	...
	/// Get <https://schema.org/exifData> from [`Self`] as borrowed slice.
	fn get_exif_data(&self) -> &[ExifDataProperty];
	/// Take <https://schema.org/exifData> from [`Self`] as owned vector.
	fn take_exif_data(&mut self) -> Vec<ExifDataProperty>;
	/// Get <https://schema.org/representativeOfPage> from [`Self`] as borrowed slice.
	fn get_representative_of_page(&self) -> &[RepresentativeOfPageProperty];
	/// Take <https://schema.org/representativeOfPage> from [`Self`] as owned vector.
	fn take_representative_of_page(&mut self) -> Vec<RepresentativeOfPageProperty>;
}
impl ImageObjectTrait for ImageObject {
	fn get_caption(&self) -> &[CaptionProperty] {
...
impl CreativeWorkTrait for ImageObject {
...
impl MediaObjectTrait for ImageObject {
...
impl ThingTrait for ImageObject {
...
mod serde {
	...
	use super::*;
	impl Serialize for ImageObject {
			...
			S: Serializer,
		{
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "ImageObject", len)?;
	...
	impl<'de> Deserialize<'de> for ImageObject {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				...
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema ImageObject")
				}
			...
			deserializer.deserialize_struct("ImageObject", FIELDS, ClassVisitor)
		}
	}
}
