...
/// This trait is for properties from <https://schema.org/JoinAction>.
pub trait JoinActionTrait {
...
impl JoinActionTrait for JoinAction {
	fn get_event(&self) -> &[EventProperty] {
...
impl InteractActionTrait for JoinAction {}
impl ThingTrait for JoinAction {
...
mod serde {
	...
	impl Serialize for JoinAction {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "JoinAction", len)?;
	...
	impl<'de> Deserialize<'de> for JoinAction {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = JoinAction;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema JoinAction")
				}
			...
			deserializer.deserialize_struct("JoinAction", FIELDS, ClassVisitor)
		}
	}
}
