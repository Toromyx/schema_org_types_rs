...
/// This trait is for properties from <https://schema.org/Winery>.
pub trait WineryTrait {}
impl WineryTrait for Winery {}
impl FoodEstablishmentTrait for Winery {
...
impl LocalBusinessTrait for Winery {
...
mod serde {
	...
	impl Serialize for Winery {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Winery", len)?;
	...
	impl<'de> Deserialize<'de> for Winery {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Winery;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Winery")
				}
			...
			deserializer.deserialize_struct("Winery", FIELDS, ClassVisitor)
		}
	}
}
