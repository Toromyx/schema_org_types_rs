...
pub struct BoatTrip {
	...
	pub r#arrival_boat_terminal: Vec<ArrivalBoatTerminalProperty>,
	/// <https://schema.org/departureBoatTerminal>
	pub r#departure_boat_terminal: Vec<DepartureBoatTerminalProperty>,
...
pub trait BoatTripTrait {
	/// Get <https://schema.org/arrivalBoatTerminal> from [`Self`] as borrowed slice.
	fn get_arrival_boat_terminal(&self) -> &[ArrivalBoatTerminalProperty];
	/// Take <https://schema.org/arrivalBoatTerminal> from [`Self`] as owned vector.
	fn take_arrival_boat_terminal(&mut self) -> Vec<ArrivalBoatTerminalProperty>;
	/// Get <https://schema.org/departureBoatTerminal> from [`Self`] as borrowed slice.
	fn get_departure_boat_terminal(&self) -> &[DepartureBoatTerminalProperty];
	/// Take <https://schema.org/departureBoatTerminal> from [`Self`] as owned vector.
	fn take_departure_boat_terminal(&mut self) -> Vec<DepartureBoatTerminalProperty>;
}
impl BoatTripTrait for BoatTrip {
	fn get_arrival_boat_terminal(&self) -> &[ArrivalBoatTerminalProperty] {
		self.r#arrival_boat_terminal.as_slice()
	}
	fn take_arrival_boat_terminal(&mut self) -> Vec<ArrivalBoatTerminalProperty> {
		std::mem::take(&mut self.r#arrival_boat_terminal)
	}
	fn get_departure_boat_terminal(&self) -> &[DepartureBoatTerminalProperty] {
		self.r#departure_boat_terminal.as_slice()
	}
	fn take_departure_boat_terminal(&mut self) -> Vec<DepartureBoatTerminalProperty> {
		std::mem::take(&mut self.r#departure_boat_terminal)
	}
}
...
mod serde {
	...
	use super::*;
	impl Serialize for BoatTrip {
		fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
		where
			S: Serializer,
		{
			let len: usize = [
				!Vec::is_empty(&self.r#arrival_boat_terminal) as usize,
				!Vec::is_empty(&self.r#departure_boat_terminal) as usize,
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "BoatTrip", len)?;
			if !Vec::is_empty(&self.r#arrival_boat_terminal) {
				serialize_struct.serialize_field("arrivalBoatTerminal", {
					struct SerializeWith<'a>(&'a Vec<ArrivalBoatTerminalProperty>);
					...
					&SerializeWith(&self.r#arrival_boat_terminal)
				})?;
			} else {
				serialize_struct.skip_field("arrivalBoatTerminal")?;
			}
			if !Vec::is_empty(&self.r#departure_boat_terminal) {
				serialize_struct.serialize_field("departureBoatTerminal", {
					struct SerializeWith<'a>(&'a Vec<DepartureBoatTerminalProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#departure_boat_terminal)
				})?;
			} else {
				serialize_struct.skip_field("departureBoatTerminal")?;
			}
	...
	impl<'de> Deserialize<'de> for BoatTrip {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
				...
				{
					match value {
						"arrivalBoatTerminal" => Ok(Field::ArrivalBoatTerminal),
						"departureBoatTerminal" => Ok(Field::DepartureBoatTerminal),
				...
				{
					match value {
						b"arrivalBoatTerminal" => Ok(Field::ArrivalBoatTerminal),
						b"departureBoatTerminal" => Ok(Field::DepartureBoatTerminal),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = BoatTrip;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema BoatTrip")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#arrival_boat_terminal_property = None;
					let mut r#departure_boat_terminal_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::ArrivalBoatTerminal => {
								if r#arrival_boat_terminal_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"arrivalBoatTerminal",
									));
								}
								r#arrival_boat_terminal_property = Some({
									struct DeserializeWith(Vec<ArrivalBoatTerminalProperty>);
							...
							Field::DepartureBoatTerminal => {
								if r#departure_boat_terminal_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"departureBoatTerminal",
									));
								}
								r#departure_boat_terminal_property = Some({
									struct DeserializeWith(Vec<DepartureBoatTerminalProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
					...
					Ok(BoatTrip {
						r#arrival_boat_terminal: r#arrival_boat_terminal_property
							.unwrap_or_default(),
						r#departure_boat_terminal: r#departure_boat_terminal_property
							.unwrap_or_default(),
			...
			deserializer.deserialize_struct("BoatTrip", FIELDS, ClassVisitor)
		}
	}
}
