...
mod serde {
	...
	impl Serialize for ItemList {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "ItemList", len)?;
	...
	impl<'de> Deserialize<'de> for ItemList {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = ItemList;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema ItemList")
				}
			...
			deserializer.deserialize_struct("ItemList", FIELDS, ClassVisitor)
		}
	}
}
