...
impl LinkRoleTrait for LinkRole {
	...
	fn get_link_relationship(&self) -> &[LinkRelationshipProperty] {
		self.r#link_relationship.as_slice()
	}
	fn take_link_relationship(&mut self) -> Vec<LinkRelationshipProperty> {
		std::mem::take(&mut self.r#link_relationship)
	}
}
...
mod serde {
	...
	impl Serialize for LinkRole {
			...
			S: Serializer,
		{
			let len: usize = [
				...
				!Vec::is_empty(&self.r#link_relationship) as usize,
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "LinkRole", len)?;
			...
			if !Vec::is_empty(&self.r#link_relationship) {
				serialize_struct.serialize_field("linkRelationship", {
					struct SerializeWith<'a>(&'a Vec<LinkRelationshipProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#link_relationship)
				})?;
			} else {
				serialize_struct.skip_field("linkRelationship")?;
			}
	...
	impl<'de> Deserialize<'de> for LinkRole {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
					...
					E: de::Error,
				{
					match value {
						...
						"linkRelationship" => Ok(Field::LinkRelationship),
					...
					E: de::Error,
				{
					match value {
						...
						b"linkRelationship" => Ok(Field::LinkRelationship),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = LinkRole;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema LinkRole")
				}
					...
					A: de::MapAccess<'de>,
				{
					...
					let mut r#link_relationship_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							...
							Field::LinkRelationship => {
								if r#link_relationship_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"linkRelationship",
									));
								}
								r#link_relationship_property = Some({
									struct DeserializeWith(Vec<LinkRelationshipProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
					...
					Ok(LinkRole {
						...
						r#link_relationship: r#link_relationship_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("LinkRole", FIELDS, ClassVisitor)
		}
	}
}
