...
impl DrugClassTrait for DrugClass {
	fn get_drug(&self) -> &[DrugProperty] {
...
impl MedicalEntityTrait for DrugClass {
...
mod serde {
	...
	impl Serialize for DrugClass {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "DrugClass", len)?;
	...
	impl<'de> Deserialize<'de> for DrugClass {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = DrugClass;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema DrugClass")
				}
			...
			deserializer.deserialize_struct("DrugClass", FIELDS, ClassVisitor)
		}
	}
}
