...
pub struct GovernmentService {
	...
	pub r#service_operator: Vec<ServiceOperatorProperty>,
...
pub trait GovernmentServiceTrait {
	...
	/// Get <https://schema.org/serviceOperator> from [`Self`] as borrowed slice.
	fn get_service_operator(&self) -> &[ServiceOperatorProperty];
	/// Take <https://schema.org/serviceOperator> from [`Self`] as owned vector.
	fn take_service_operator(&mut self) -> Vec<ServiceOperatorProperty>;
}
impl GovernmentServiceTrait for GovernmentService {
	fn get_jurisdiction(&self) -> &[JurisdictionProperty] {
	...
	fn get_service_operator(&self) -> &[ServiceOperatorProperty] {
		self.r#service_operator.as_slice()
	}
	fn take_service_operator(&mut self) -> Vec<ServiceOperatorProperty> {
		std::mem::take(&mut self.r#service_operator)
	}
}
impl ServiceTrait for GovernmentService {
...
impl ThingTrait for GovernmentService {
...
mod serde {
	...
	use super::*;
	impl Serialize for GovernmentService {
			...
			S: Serializer,
		{
			let len: usize = [
				...
				!Vec::is_empty(&self.r#service_operator) as usize,
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "GovernmentService", len)?;
			...
			if !Vec::is_empty(&self.r#service_operator) {
				serialize_struct.serialize_field("serviceOperator", {
					struct SerializeWith<'a>(&'a Vec<ServiceOperatorProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#service_operator)
				})?;
			} else {
				serialize_struct.skip_field("serviceOperator")?;
			}
	...
	impl<'de> Deserialize<'de> for GovernmentService {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
					...
					E: de::Error,
				{
					match value {
						...
						"serviceOperator" => Ok(Field::ServiceOperator),
					...
					E: de::Error,
				{
					match value {
						...
						b"serviceOperator" => Ok(Field::ServiceOperator),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = GovernmentService;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema GovernmentService")
				}
					...
					A: de::MapAccess<'de>,
				{
					...
					let mut r#service_operator_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							...
							Field::ServiceOperator => {
								if r#service_operator_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"serviceOperator",
									));
								}
								r#service_operator_property = Some({
									struct DeserializeWith(Vec<ServiceOperatorProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
					...
					Ok(GovernmentService {
						...
						r#service_operator: r#service_operator_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("GovernmentService", FIELDS, ClassVisitor)
		}
	}
}
