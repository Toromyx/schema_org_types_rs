...
impl ThingTrait for Organization {
	...
	impl Serialize for Organization {
			...
			S: Serializer,
		{
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "Organization", len)?;
	...
	impl<'de> Deserialize<'de> for Organization {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				...
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Organization")
				}
			...
			deserializer.deserialize_struct("Organization", FIELDS, ClassVisitor)
		}
	}
}
