...
mod serde {
	...
	impl Serialize for Brand {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Brand", len)?;
	...
	impl<'de> Deserialize<'de> for Brand {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Brand;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Brand")
				}
			...
			deserializer.deserialize_struct("Brand", FIELDS, ClassVisitor)
		}
	}
}
