...
pub trait MusicPlaylistTrait {
	/// Get <https://schema.org/numTracks> from [`Self`] as borrowed slice.
	fn get_num_tracks(&self) -> &[NumTracksProperty];
	/// Take <https://schema.org/numTracks> from [`Self`] as owned vector.
...
impl MusicPlaylistTrait for MusicPlaylist {
	fn get_num_tracks(&self) -> &[NumTracksProperty] {
...
impl CreativeWorkTrait for MusicPlaylist {
...
impl ThingTrait for MusicPlaylist {
...
mod serde {
	...
	use super::*;
	impl Serialize for MusicPlaylist {
			...
			S: Serializer,
		{
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "MusicPlaylist", len)?;
	...
	impl<'de> Deserialize<'de> for MusicPlaylist {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = MusicPlaylist;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema MusicPlaylist")
				}
			...
			deserializer.deserialize_struct("MusicPlaylist", FIELDS, ClassVisitor)
		}
	}
}
