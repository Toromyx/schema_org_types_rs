...
impl MoveActionTrait for MoveAction {
	fn get_from_location(&self) -> &[FromLocationProperty] {
...
impl ThingTrait for MoveAction {
...
mod serde {
	...
	impl Serialize for MoveAction {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "MoveAction", len)?;
	...
	impl<'de> Deserialize<'de> for MoveAction {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = MoveAction;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema MoveAction")
				}
			...
			deserializer.deserialize_struct("MoveAction", FIELDS, ClassVisitor)
		}
	}
}
