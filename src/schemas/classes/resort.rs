...
/// This trait is for properties from <https://schema.org/Resort>.
pub trait ResortTrait {}
impl ResortTrait for Resort {}
impl LocalBusinessTrait for Resort {
...
impl LodgingBusinessTrait for Resort {
...
mod serde {
	...
	impl Serialize for Resort {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Resort", len)?;
	...
	impl<'de> Deserialize<'de> for Resort {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Resort;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Resort")
				}
			...
			deserializer.deserialize_struct("Resort", FIELDS, ClassVisitor)
		}
	}
}
