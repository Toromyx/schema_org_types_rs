...
pub trait TechArticleTrait {
	/// Get <https://schema.org/dependencies> from [`Self`] as borrowed slice.
	fn get_dependencies(&self) -> &[DependenciesProperty];
	/// Take <https://schema.org/dependencies> from [`Self`] as owned vector.
	fn take_dependencies(&mut self) -> Vec<DependenciesProperty>;
	/// Get <https://schema.org/proficiencyLevel> from [`Self`] as borrowed slice.
	fn get_proficiency_level(&self) -> &[ProficiencyLevelProperty];
	/// Take <https://schema.org/proficiencyLevel> from [`Self`] as owned vector.
	fn take_proficiency_level(&mut self) -> Vec<ProficiencyLevelProperty>;
}
impl TechArticleTrait for TechArticle {
	fn get_dependencies(&self) -> &[DependenciesProperty] {
...
impl ArticleTrait for TechArticle {
...
impl CreativeWorkTrait for TechArticle {
...
impl ThingTrait for TechArticle {
...
mod serde {
	...
	use super::*;
	impl Serialize for TechArticle {
			...
			S: Serializer,
		{
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "TechArticle", len)?;
	...
	impl<'de> Deserialize<'de> for TechArticle {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = TechArticle;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema TechArticle")
				}
			...
			deserializer.deserialize_struct("TechArticle", FIELDS, ClassVisitor)
		}
	}
}
