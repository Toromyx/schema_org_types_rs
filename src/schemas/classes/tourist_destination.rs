...
pub trait TouristDestinationTrait {
	/// Get <https://schema.org/includesAttraction> from [`Self`] as borrowed slice.
	fn get_includes_attraction(&self) -> &[IncludesAttractionProperty];
	/// Take <https://schema.org/includesAttraction> from [`Self`] as owned vector.
	fn take_includes_attraction(&mut self) -> Vec<IncludesAttractionProperty>;
	/// Get <https://schema.org/touristType> from [`Self`] as borrowed slice.
	fn get_tourist_type(&self) -> &[TouristTypeProperty];
	/// Take <https://schema.org/touristType> from [`Self`] as owned vector.
	fn take_tourist_type(&mut self) -> Vec<TouristTypeProperty>;
}
impl TouristDestinationTrait for TouristDestination {
	fn get_includes_attraction(&self) -> &[IncludesAttractionProperty] {
		self.r#includes_attraction.as_slice()
	}
	fn take_includes_attraction(&mut self) -> Vec<IncludesAttractionProperty> {
		std::mem::take(&mut self.r#includes_attraction)
	}
	fn get_tourist_type(&self) -> &[TouristTypeProperty] {
		self.r#tourist_type.as_slice()
	}
	fn take_tourist_type(&mut self) -> Vec<TouristTypeProperty> {
		std::mem::take(&mut self.r#tourist_type)
	}
}
...
impl ThingTrait for TouristDestination {
...
mod serde {
	...
	use super::*;
	impl Serialize for TouristDestination {
		fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
		where
			S: Serializer,
		{
			let len: usize = [
				!Vec::is_empty(&self.r#includes_attraction) as usize,
				!Vec::is_empty(&self.r#tourist_type) as usize,
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "TouristDestination", len)?;
			if !Vec::is_empty(&self.r#includes_attraction) {
				serialize_struct.serialize_field("includesAttraction", {
					struct SerializeWith<'a>(&'a Vec<IncludesAttractionProperty>);
					...
					&SerializeWith(&self.r#includes_attraction)
				})?;
			} else {
				serialize_struct.skip_field("includesAttraction")?;
			}
			if !Vec::is_empty(&self.r#tourist_type) {
				serialize_struct.serialize_field("touristType", {
					struct SerializeWith<'a>(&'a Vec<TouristTypeProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#tourist_type)
				})?;
			} else {
				serialize_struct.skip_field("touristType")?;
			}
	...
	impl<'de> Deserialize<'de> for TouristDestination {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
				...
				{
					match value {
						"includesAttraction" => Ok(Field::IncludesAttraction),
						"touristType" => Ok(Field::TouristType),
				...
				{
					match value {
						b"includesAttraction" => Ok(Field::IncludesAttraction),
						b"touristType" => Ok(Field::TouristType),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = TouristDestination;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema TouristDestination")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#includes_attraction_property = None;
					let mut r#tourist_type_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::IncludesAttraction => {
								if r#includes_attraction_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"includesAttraction",
									));
								}
								r#includes_attraction_property = Some({
									struct DeserializeWith(Vec<IncludesAttractionProperty>);
							...
							Field::TouristType => {
								if r#tourist_type_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"touristType",
									));
								}
								r#tourist_type_property = Some({
									struct DeserializeWith(Vec<TouristTypeProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
					...
					Ok(TouristDestination {
						r#includes_attraction: r#includes_attraction_property.unwrap_or_default(),
						r#tourist_type: r#tourist_type_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("TouristDestination", FIELDS, ClassVisitor)
		}
	}
}
