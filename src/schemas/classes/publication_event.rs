...
pub trait PublicationEventTrait {
	...
	/// Take <https://schema.org/publishedBy> from [`Self`] as owned vector.
	fn take_published_by(&mut self) -> Vec<PublishedByProperty>;
	/// Get <https://schema.org/publishedOn> from [`Self`] as borrowed slice.
	fn get_published_on(&self) -> &[PublishedOnProperty];
	/// Take <https://schema.org/publishedOn> from [`Self`] as owned vector.
	fn take_published_on(&mut self) -> Vec<PublishedOnProperty>;
}
impl PublicationEventTrait for PublicationEvent {
	fn get_free(&self) -> &[FreeProperty] {
...
impl EventTrait for PublicationEvent {
...
impl ThingTrait for PublicationEvent {
...
mod serde {
	...
	use super::*;
	impl Serialize for PublicationEvent {
			...
			S: Serializer,
		{
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "PublicationEvent", len)?;
	...
	impl<'de> Deserialize<'de> for PublicationEvent {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = PublicationEvent;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema PublicationEvent")
				}
			...
			deserializer.deserialize_struct("PublicationEvent", FIELDS, ClassVisitor)
		}
	}
}
