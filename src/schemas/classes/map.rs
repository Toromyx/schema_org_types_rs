...
impl MapTrait for Map {
	fn get_map_type(&self) -> &[MapTypeProperty] {
		self.r#map_type.as_slice()
	}
	fn take_map_type(&mut self) -> Vec<MapTypeProperty> {
		std::mem::take(&mut self.r#map_type)
	}
}
impl CreativeWorkTrait for Map {
...
mod serde {
	...
	impl Serialize for Map {
		...
		{
			let len: usize = [
				!Vec::is_empty(&self.r#map_type) as usize,
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Map", len)?;
			if !Vec::is_empty(&self.r#map_type) {
				serialize_struct.serialize_field("mapType", {
					struct SerializeWith<'a>(&'a Vec<MapTypeProperty>);
					...
					&SerializeWith(&self.r#map_type)
				})?;
			} else {
				serialize_struct.skip_field("mapType")?;
			}
	...
	impl<'de> Deserialize<'de> for Map {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Map;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Map")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#map_type_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::MapType => {
								if r#map_type_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"mapType",
									));
								}
								r#map_type_property = Some({
									struct DeserializeWith(Vec<MapTypeProperty>);
					...
					Ok(Map {
						r#map_type: r#map_type_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("Map", FIELDS, ClassVisitor)
		}
	}
}
