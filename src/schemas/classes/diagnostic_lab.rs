...
pub trait DiagnosticLabTrait {
	/// Get <https://schema.org/availableTest> from [`Self`] as borrowed slice.
	fn get_available_test(&self) -> &[AvailableTestProperty];
	/// Take <https://schema.org/availableTest> from [`Self`] as owned vector.
	fn take_available_test(&mut self) -> Vec<AvailableTestProperty>;
}
impl DiagnosticLabTrait for DiagnosticLab {
	fn get_available_test(&self) -> &[AvailableTestProperty] {
		self.r#available_test.as_slice()
	}
	fn take_available_test(&mut self) -> Vec<AvailableTestProperty> {
		std::mem::take(&mut self.r#available_test)
	}
}
impl MedicalOrganizationTrait for DiagnosticLab {
...
impl ThingTrait for DiagnosticLab {
...
mod serde {
	...
	use super::*;
	impl Serialize for DiagnosticLab {
		fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
		where
			S: Serializer,
		{
			let len: usize = [
				!Vec::is_empty(&self.r#available_test) as usize,
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "DiagnosticLab", len)?;
			if !Vec::is_empty(&self.r#available_test) {
				serialize_struct.serialize_field("availableTest", {
					struct SerializeWith<'a>(&'a Vec<AvailableTestProperty>);
					...
					&SerializeWith(&self.r#available_test)
				})?;
			} else {
				serialize_struct.skip_field("availableTest")?;
			}
	...
	impl<'de> Deserialize<'de> for DiagnosticLab {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
				...
				{
					match value {
						"availableTest" => Ok(Field::AvailableTest),
				...
				{
					match value {
						b"availableTest" => Ok(Field::AvailableTest),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = DiagnosticLab;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema DiagnosticLab")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#available_test_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::AvailableTest => {
								if r#available_test_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"availableTest",
									));
								}
								r#available_test_property = Some({
									struct DeserializeWith(Vec<AvailableTestProperty>);
					...
					Ok(DiagnosticLab {
						r#available_test: r#available_test_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("DiagnosticLab", FIELDS, ClassVisitor)
		}
	}
}
