...
/// This trait is for properties from <https://schema.org/Drawing>.
pub trait DrawingTrait {}
impl DrawingTrait for Drawing {}
impl CreativeWorkTrait for Drawing {
...
mod serde {
	...
	impl Serialize for Drawing {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Drawing", len)?;
	...
	impl<'de> Deserialize<'de> for Drawing {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Drawing;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Drawing")
				}
			...
			deserializer.deserialize_struct("Drawing", FIELDS, ClassVisitor)
		}
	}
}
