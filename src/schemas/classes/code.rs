...
/// This trait is for properties from <https://schema.org/Code>.
#[deprecated = "This schema is superseded by <https://schema.org/SoftwareSourceCode>."]
pub trait CodeTrait {}
impl CodeTrait for Code {}
impl CreativeWorkTrait for Code {
...
mod serde {
	...
	impl Serialize for Code {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Code", len)?;
	...
	impl<'de> Deserialize<'de> for Code {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Code;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Code")
				}
			...
			deserializer.deserialize_struct("Code", FIELDS, ClassVisitor)
		}
	}
}
