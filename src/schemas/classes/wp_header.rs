...
/// This trait is for properties from <https://schema.org/WPHeader>.
pub trait WpHeaderTrait {}
impl WpHeaderTrait for WpHeader {}
impl CreativeWorkTrait for WpHeader {
...
impl WebPageElementTrait for WpHeader {
...
mod serde {
	...
	impl Serialize for WpHeader {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "WpHeader", len)?;
	...
	impl<'de> Deserialize<'de> for WpHeader {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = WpHeader;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema WPHeader")
				}
			...
			deserializer.deserialize_struct("WpHeader", FIELDS, ClassVisitor)
		}
	}
}
