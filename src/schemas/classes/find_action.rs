...
/// This trait is for properties from <https://schema.org/FindAction>.
pub trait FindActionTrait {}
impl FindActionTrait for FindAction {}
...
impl ThingTrait for FindAction {
...
mod serde {
	...
	impl Serialize for FindAction {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "FindAction", len)?;
	...
	impl<'de> Deserialize<'de> for FindAction {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = FindAction;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema FindAction")
				}
			...
			deserializer.deserialize_struct("FindAction", FIELDS, ClassVisitor)
		}
	}
}
