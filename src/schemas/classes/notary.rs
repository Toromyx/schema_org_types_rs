...
/// This trait is for properties from <https://schema.org/Notary>.
pub trait NotaryTrait {}
impl NotaryTrait for Notary {}
impl LegalServiceTrait for Notary {}
...
mod serde {
	...
	impl Serialize for Notary {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Notary", len)?;
	...
	impl<'de> Deserialize<'de> for Notary {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Notary;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Notary")
				}
			...
			deserializer.deserialize_struct("Notary", FIELDS, ClassVisitor)
		}
	}
}
