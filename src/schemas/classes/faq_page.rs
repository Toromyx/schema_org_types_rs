...
/// This trait is for properties from <https://schema.org/FAQPage>.
pub trait FaqPageTrait {}
impl FaqPageTrait for FaqPage {}
impl CreativeWorkTrait for FaqPage {
...
mod serde {
	...
	impl Serialize for FaqPage {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "FaqPage", len)?;
	...
	impl<'de> Deserialize<'de> for FaqPage {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = FaqPage;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema FAQPage")
				}
			...
			deserializer.deserialize_struct("FaqPage", FIELDS, ClassVisitor)
		}
	}
}
