...
/// This trait is for properties from <https://schema.org/Cemetery>.
pub trait CemeteryTrait {}
impl CemeteryTrait for Cemetery {}
impl CivicStructureTrait for Cemetery {
...
mod serde {
	...
	impl Serialize for Cemetery {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Cemetery", len)?;
	...
	impl<'de> Deserialize<'de> for Cemetery {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Cemetery;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Cemetery")
				}
			...
			deserializer.deserialize_struct("Cemetery", FIELDS, ClassVisitor)
		}
	}
}
