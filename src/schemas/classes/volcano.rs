...
/// This trait is for properties from <https://schema.org/Volcano>.
pub trait VolcanoTrait {}
impl VolcanoTrait for Volcano {}
impl LandformTrait for Volcano {}
...
mod serde {
	...
	impl Serialize for Volcano {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Volcano", len)?;
	...
	impl<'de> Deserialize<'de> for Volcano {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Volcano;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Volcano")
				}
			...
			deserializer.deserialize_struct("Volcano", FIELDS, ClassVisitor)
		}
	}
}
