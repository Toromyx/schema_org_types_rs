...
pub trait CommentTrait {
	/// Get <https://schema.org/downvoteCount> from [`Self`] as borrowed slice.
	fn get_downvote_count(&self) -> &[DownvoteCountProperty];
	/// Take <https://schema.org/downvoteCount> from [`Self`] as owned vector.
	...
	/// Get <https://schema.org/upvoteCount> from [`Self`] as borrowed slice.
	fn get_upvote_count(&self) -> &[UpvoteCountProperty];
	/// Take <https://schema.org/upvoteCount> from [`Self`] as owned vector.
...
impl CreativeWorkTrait for Comment {
...
mod serde {
	...
	impl Serialize for Comment {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Comment", len)?;
	...
	impl<'de> Deserialize<'de> for Comment {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Comment;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Comment")
				}
			...
			deserializer.deserialize_struct("Comment", FIELDS, ClassVisitor)
		}
	}
}
