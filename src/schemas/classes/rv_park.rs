...
/// This trait is for properties from <https://schema.org/RVPark>.
pub trait RvParkTrait {}
impl RvParkTrait for RvPark {}
impl CivicStructureTrait for RvPark {
...
mod serde {
	...
	impl Serialize for RvPark {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "RvPark", len)?;
	...
	impl<'de> Deserialize<'de> for RvPark {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = RvPark;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema RVPark")
				}
			...
			deserializer.deserialize_struct("RvPark", FIELDS, ClassVisitor)
		}
	}
}
