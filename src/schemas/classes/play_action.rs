...
impl PlayActionTrait for PlayAction {
	fn get_audience(&self) -> &[AudienceProperty] {
...
impl ThingTrait for PlayAction {
...
mod serde {
	...
	impl Serialize for PlayAction {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "PlayAction", len)?;
	...
	impl<'de> Deserialize<'de> for PlayAction {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				...
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema PlayAction")
				}
			...
			deserializer.deserialize_struct("PlayAction", FIELDS, ClassVisitor)
		}
	}
}
