...
impl BlogTrait for Blog {
	fn get_blog_post(&self) -> &[BlogPostProperty] {
		self.r#blog_post.as_slice()
	}
	fn take_blog_post(&mut self) -> Vec<BlogPostProperty> {
		std::mem::take(&mut self.r#blog_post)
	}
	fn get_blog_posts(&self) -> &[BlogPostsProperty] {
		self.r#blog_posts.as_slice()
	}
	fn take_blog_posts(&mut self) -> Vec<BlogPostsProperty> {
		std::mem::take(&mut self.r#blog_posts)
	}
	fn get_issn(&self) -> &[IssnProperty] {
...
impl CreativeWorkTrait for Blog {
...
mod serde {
	...
	impl Serialize for Blog {
		...
		{
			let len: usize = [
				!Vec::is_empty(&self.r#blog_post) as usize,
				!Vec::is_empty(&self.r#blog_posts) as usize,
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Blog", len)?;
			if !Vec::is_empty(&self.r#blog_post) {
				serialize_struct.serialize_field("blogPost", {
					struct SerializeWith<'a>(&'a Vec<BlogPostProperty>);
					...
					&SerializeWith(&self.r#blog_post)
				})?;
			} else {
				serialize_struct.skip_field("blogPost")?;
			}
			if !Vec::is_empty(&self.r#blog_posts) {
				serialize_struct.serialize_field("blogPosts", {
					struct SerializeWith<'a>(&'a Vec<BlogPostsProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#blog_posts)
				})?;
			} else {
				serialize_struct.skip_field("blogPosts")?;
			}
	...
	impl<'de> Deserialize<'de> for Blog {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
				...
				{
					match value {
						"blogPost" => Ok(Field::BlogPost),
						"blogPosts" => Ok(Field::BlogPosts),
						"issn" => Ok(Field::Issn),
				...
				{
					match value {
						b"blogPost" => Ok(Field::BlogPost),
						b"blogPosts" => Ok(Field::BlogPosts),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Blog;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Blog")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#blog_post_property = None;
					let mut r#blog_posts_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::BlogPost => {
								if r#blog_post_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"blogPost",
									));
								}
								r#blog_post_property = Some({
									struct DeserializeWith(Vec<BlogPostProperty>);
							...
							Field::BlogPosts => {
								if r#blog_posts_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"blogPosts",
									));
								}
								r#blog_posts_property = Some({
									struct DeserializeWith(Vec<BlogPostsProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
					...
					Ok(Blog {
						r#blog_post: r#blog_post_property.unwrap_or_default(),
						r#blog_posts: r#blog_posts_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("Blog", FIELDS, ClassVisitor)
		}
	}
}
