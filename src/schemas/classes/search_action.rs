...
impl SearchActionTrait for SearchAction {
	fn get_query(&self) -> &[QueryProperty] {
		self.r#query.as_slice()
	}
	fn take_query(&mut self) -> Vec<QueryProperty> {
		std::mem::take(&mut self.r#query)
	}
}
...
impl ThingTrait for SearchAction {
...
mod serde {
	...
	use super::*;
	impl Serialize for SearchAction {
		fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
		where
			S: Serializer,
		{
			let len: usize = [
				!Vec::is_empty(&self.r#query) as usize,
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "SearchAction", len)?;
			if !Vec::is_empty(&self.r#query) {
				serialize_struct.serialize_field("query", {
					struct SerializeWith<'a>(&'a Vec<QueryProperty>);
					...
					&SerializeWith(&self.r#query)
				})?;
			} else {
				serialize_struct.skip_field("query")?;
			}
	...
	impl<'de> Deserialize<'de> for SearchAction {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = SearchAction;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema SearchAction")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#query_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::Query => {
								if r#query_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field("query"));
								}
								r#query_property = Some({
									struct DeserializeWith(Vec<QueryProperty>);
					...
					Ok(SearchAction {
						r#query: r#query_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("SearchAction", FIELDS, ClassVisitor)
		}
	}
}
