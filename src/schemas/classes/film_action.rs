...
/// This trait is for properties from <https://schema.org/FilmAction>.
pub trait FilmActionTrait {}
impl FilmActionTrait for FilmAction {}
...
impl CreateActionTrait for FilmAction {}
impl ThingTrait for FilmAction {
...
mod serde {
	...
	impl Serialize for FilmAction {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "FilmAction", len)?;
	...
	impl<'de> Deserialize<'de> for FilmAction {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = FilmAction;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema FilmAction")
				}
			...
			deserializer.deserialize_struct("FilmAction", FIELDS, ClassVisitor)
		}
	}
}
