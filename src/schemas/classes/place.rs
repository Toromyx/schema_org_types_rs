...
mod serde {
	...
	impl Serialize for Place {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Place", len)?;
	...
	impl<'de> Deserialize<'de> for Place {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Place;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Place")
				}
			...
			deserializer.deserialize_struct("Place", FIELDS, ClassVisitor)
		}
	}
}
