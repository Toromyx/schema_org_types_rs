...
mod serde {
	...
	impl Serialize for Action {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Action", len)?;
	...
	impl<'de> Deserialize<'de> for Action {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Action;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Action")
				}
			...
			deserializer.deserialize_struct("Action", FIELDS, ClassVisitor)
		}
	}
}
