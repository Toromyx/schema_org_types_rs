...
impl AirportTrait for Airport {
	fn get_iata_code(&self) -> &[IataCodeProperty] {
		self.r#iata_code.as_slice()
	}
	fn take_iata_code(&mut self) -> Vec<IataCodeProperty> {
		std::mem::take(&mut self.r#iata_code)
	}
	fn get_icao_code(&self) -> &[IcaoCodeProperty] {
		self.r#icao_code.as_slice()
	}
	fn take_icao_code(&mut self) -> Vec<IcaoCodeProperty> {
		std::mem::take(&mut self.r#icao_code)
	}
}
impl CivicStructureTrait for Airport {
	...
	impl Serialize for Airport {
			...
			S: Serializer,
		{
			let len: usize = [
				...
				!Vec::is_empty(&self.r#icao_code) as usize,
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Airport", len)?;
			...
			if !Vec::is_empty(&self.r#icao_code) {
				serialize_struct.serialize_field("icaoCode", {
					struct SerializeWith<'a>(&'a Vec<IcaoCodeProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#icao_code)
				})?;
			} else {
				serialize_struct.skip_field("icaoCode")?;
			}
	...
	impl<'de> Deserialize<'de> for Airport {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Airport;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Airport")
				}
					...
					A: de::MapAccess<'de>,
				{
					...
					let mut r#icao_code_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							...
							Field::IcaoCode => {
								if r#icao_code_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"icaoCode",
									));
								}
								r#icao_code_property = Some({
									struct DeserializeWith(Vec<IcaoCodeProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
					...
					Ok(Airport {
						...
						r#icao_code: r#icao_code_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("Airport", FIELDS, ClassVisitor)
		}
	}
}
