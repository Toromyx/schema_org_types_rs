...
/// This trait is for properties from <https://schema.org/Store>.
pub trait StoreTrait {}
impl StoreTrait for Store {}
impl LocalBusinessTrait for Store {
...
mod serde {
	...
	impl Serialize for Store {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "Store", len)?;
	...
	impl<'de> Deserialize<'de> for Store {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = Store;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema Store")
				}
			...
			deserializer.deserialize_struct("Store", FIELDS, ClassVisitor)
		}
	}
}
