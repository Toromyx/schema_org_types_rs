...
pub trait FinancialProductTrait {
	/// Get <https://schema.org/annualPercentageRate> from [`Self`] as borrowed slice.
	fn get_annual_percentage_rate(&self) -> &[AnnualPercentageRateProperty];
	/// Take <https://schema.org/annualPercentageRate> from [`Self`] as owned vector.
	fn take_annual_percentage_rate(&mut self) -> Vec<AnnualPercentageRateProperty>;
	/// Get <https://schema.org/feesAndCommissionsSpecification> from [`Self`] as borrowed slice.
	fn get_fees_and_commissions_specification(&self) -> &[FeesAndCommissionsSpecificationProperty];
	/// Take <https://schema.org/feesAndCommissionsSpecification> from [`Self`] as owned vector.
	fn take_fees_and_commissions_specification(
		&mut self,
	) -> Vec<FeesAndCommissionsSpecificationProperty>;
	/// Get <https://schema.org/interestRate> from [`Self`] as borrowed slice.
	fn get_interest_rate(&self) -> &[InterestRateProperty];
	/// Take <https://schema.org/interestRate> from [`Self`] as owned vector.
	fn take_interest_rate(&mut self) -> Vec<InterestRateProperty>;
}
impl FinancialProductTrait for FinancialProduct {
	fn get_annual_percentage_rate(&self) -> &[AnnualPercentageRateProperty] {
...
impl ServiceTrait for FinancialProduct {
...
impl ThingTrait for FinancialProduct {
...
mod serde {
	...
	use super::*;
	impl Serialize for FinancialProduct {
			...
			S: Serializer,
		{
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "FinancialProduct", len)?;
	...
	impl<'de> Deserialize<'de> for FinancialProduct {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = FinancialProduct;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema FinancialProduct")
				}
			...
			deserializer.deserialize_struct("FinancialProduct", FIELDS, ClassVisitor)
		}
	}
}
