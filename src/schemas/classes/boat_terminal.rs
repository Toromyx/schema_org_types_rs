...
mod serde {
	...
	use super::*;
	impl Serialize for BoatTerminal {
			...
			S: Serializer,
		{
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "BoatTerminal", len)?;
	...
	impl<'de> Deserialize<'de> for BoatTerminal {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = BoatTerminal;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema BoatTerminal")
				}
			...
			deserializer.deserialize_struct("BoatTerminal", FIELDS, ClassVisitor)
		}
	}
}
