...
pub struct MedicalConditionStage {
	...
	pub r#stage_as_number: Vec<StageAsNumberProperty>,
	/// <https://schema.org/subStageSuffix>
	pub r#sub_stage_suffix: Vec<SubStageSuffixProperty>,
...
/// This trait is for properties from <https://schema.org/MedicalConditionStage>.
pub trait MedicalConditionStageTrait {
	/// Get <https://schema.org/stageAsNumber> from [`Self`] as borrowed slice.
	fn get_stage_as_number(&self) -> &[StageAsNumberProperty];
	/// Take <https://schema.org/stageAsNumber> from [`Self`] as owned vector.
	fn take_stage_as_number(&mut self) -> Vec<StageAsNumberProperty>;
	/// Get <https://schema.org/subStageSuffix> from [`Self`] as borrowed slice.
	fn get_sub_stage_suffix(&self) -> &[SubStageSuffixProperty];
	/// Take <https://schema.org/subStageSuffix> from [`Self`] as owned vector.
	fn take_sub_stage_suffix(&mut self) -> Vec<SubStageSuffixProperty>;
}
impl MedicalConditionStageTrait for MedicalConditionStage {
	fn get_stage_as_number(&self) -> &[StageAsNumberProperty] {
		self.r#stage_as_number.as_slice()
	}
	fn take_stage_as_number(&mut self) -> Vec<StageAsNumberProperty> {
		std::mem::take(&mut self.r#stage_as_number)
	}
	fn get_sub_stage_suffix(&self) -> &[SubStageSuffixProperty] {
		self.r#sub_stage_suffix.as_slice()
	}
	fn take_sub_stage_suffix(&mut self) -> Vec<SubStageSuffixProperty> {
		std::mem::take(&mut self.r#sub_stage_suffix)
	}
}
impl MedicalEntityTrait for MedicalConditionStage {
...
impl MedicalIntangibleTrait for MedicalConditionStage {}
impl ThingTrait for MedicalConditionStage {
...
mod serde {
	...
	use super::*;
	impl Serialize for MedicalConditionStage {
		fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
		where
			S: Serializer,
		{
			let len: usize = [
				!Vec::is_empty(&self.r#stage_as_number) as usize,
				!Vec::is_empty(&self.r#sub_stage_suffix) as usize,
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "MedicalConditionStage", len)?;
			if !Vec::is_empty(&self.r#stage_as_number) {
				serialize_struct.serialize_field("stageAsNumber", {
					struct SerializeWith<'a>(&'a Vec<StageAsNumberProperty>);
					...
					&SerializeWith(&self.r#stage_as_number)
				})?;
			} else {
				serialize_struct.skip_field("stageAsNumber")?;
			}
			if !Vec::is_empty(&self.r#sub_stage_suffix) {
				serialize_struct.serialize_field("subStageSuffix", {
					struct SerializeWith<'a>(&'a Vec<SubStageSuffixProperty>);
					impl<'a> Serialize for SerializeWith<'a> {
						fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
						where
							S: Serializer,
					...
					&SerializeWith(&self.r#sub_stage_suffix)
				})?;
			} else {
				serialize_struct.skip_field("subStageSuffix")?;
			}
	...
	impl<'de> Deserialize<'de> for MedicalConditionStage {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for FieldVisitor {
				...
				{
					match value {
						"stageAsNumber" => Ok(Field::StageAsNumber),
						"subStageSuffix" => Ok(Field::SubStageSuffix),
						"code" => Ok(Field::Code),
				...
				{
					match value {
						b"stageAsNumber" => Ok(Field::StageAsNumber),
						b"subStageSuffix" => Ok(Field::SubStageSuffix),
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = MedicalConditionStage;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema MedicalConditionStage")
				}
				fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
				where
					A: de::MapAccess<'de>,
				{
					let mut r#stage_as_number_property = None;
					let mut r#sub_stage_suffix_property = None;
					...
					while let Some(key) = map.next_key::<Field>()? {
						match key {
							Field::StageAsNumber => {
								if r#stage_as_number_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"stageAsNumber",
									));
								}
								r#stage_as_number_property = Some({
									struct DeserializeWith(Vec<StageAsNumberProperty>);
							...
							Field::SubStageSuffix => {
								if r#sub_stage_suffix_property.is_some() {
									return Err(<A::Error as de::Error>::duplicate_field(
										"subStageSuffix",
									));
								}
								r#sub_stage_suffix_property = Some({
									struct DeserializeWith(Vec<SubStageSuffixProperty>);
									impl<'de> Deserialize<'de> for DeserializeWith {
										fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
										where
											D: Deserializer<'de>,
					...
					Ok(MedicalConditionStage {
						r#stage_as_number: r#stage_as_number_property.unwrap_or_default(),
						r#sub_stage_suffix: r#sub_stage_suffix_property.unwrap_or_default(),
			...
			deserializer.deserialize_struct("MedicalConditionStage", FIELDS, ClassVisitor)
		}
	}
}
