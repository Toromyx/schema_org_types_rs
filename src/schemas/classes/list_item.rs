...
mod serde {
	...
	impl Serialize for ListItem {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "ListItem", len)?;
	...
	impl<'de> Deserialize<'de> for ListItem {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = ListItem;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema ListItem")
				}
			...
			deserializer.deserialize_struct("ListItem", FIELDS, ClassVisitor)
		}
	}
}
