...
/// This trait is for properties from <https://schema.org/ItemPage>.
pub trait ItemPageTrait {}
impl ItemPageTrait for ItemPage {}
impl CreativeWorkTrait for ItemPage {
...
impl WebPageTrait for ItemPage {
...
mod serde {
	...
	impl Serialize for ItemPage {
			...
			S: Serializer,
		{
			...
			.iter()
			.sum();
			let mut serialize_struct = Serializer::serialize_struct(serializer, "ItemPage", len)?;
	...
	impl<'de> Deserialize<'de> for ItemPage {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = ItemPage;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema ItemPage")
				}
			...
			deserializer.deserialize_struct("ItemPage", FIELDS, ClassVisitor)
		}
	}
}
