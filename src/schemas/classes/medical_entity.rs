...
pub trait MedicalEntityTrait {
	...
	/// Take <https://schema.org/recognizingAuthority> from [`Self`] as owned vector.
...
impl MedicalEntityTrait for MedicalEntity {
	fn get_code(&self) -> &[CodeProperty] {
...
impl ThingTrait for MedicalEntity {
...
mod serde {
	...
	use super::*;
	impl Serialize for MedicalEntity {
			...
			S: Serializer,
		{
			...
			let mut serialize_struct =
				Serializer::serialize_struct(serializer, "MedicalEntity", len)?;
	...
	impl<'de> Deserialize<'de> for MedicalEntity {
			...
			D: Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for ClassVisitor {
				type Value = MedicalEntity;
				fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
					formatter.write_str("schema.org schema MedicalEntity")
				}
			...
			deserializer.deserialize_struct("MedicalEntity", FIELDS, ClassVisitor)
		}
	}
}
