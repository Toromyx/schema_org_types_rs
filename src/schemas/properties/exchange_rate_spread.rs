...
mod serde {
	...
	impl Serialize for ExchangeRateSpreadProperty {
		...
		{
			match *self {
				ExchangeRateSpreadProperty::MonetaryAmount(ref inner) => {
					inner.serialize(serializer)
				}
				ExchangeRateSpreadProperty::Number(ref inner) => inner.serialize(serializer),
				#[cfg(all(feature = "fallible", feature = "serde"))]
				ExchangeRateSpreadProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for ExchangeRateSpreadProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				ExchangeRateSpreadProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property exchangeRateSpread or was able to be deserialized into a generic value";
