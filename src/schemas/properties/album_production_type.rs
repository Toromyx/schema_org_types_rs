...
mod serde {
	...
	impl Serialize for AlbumProductionTypeProperty {
		...
		{
			match *self {
				AlbumProductionTypeProperty::MusicAlbumProductionType(ref inner) => {
					inner.serialize(serializer)
				}
				#[cfg(all(feature = "fallible", feature = "serde"))]
				AlbumProductionTypeProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for AlbumProductionTypeProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				AlbumProductionTypeProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property albumProductionType or was able to be deserialized into a generic value";
