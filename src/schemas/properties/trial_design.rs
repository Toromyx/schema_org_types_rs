	...
	impl Serialize for TrialDesignProperty {
			...
			S: Serializer,
		{
			match *self {
				...
				TrialDesignProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for TrialDesignProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				TrialDesignProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property trialDesign or was able to be deserialized into a generic value";
