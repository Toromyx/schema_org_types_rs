	...
	impl Serialize for DoseUnitProperty {
			...
			S: Serializer,
		{
			match *self {
				...
				DoseUnitProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for DoseUnitProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				DoseUnitProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property doseUnit or was able to be deserialized into a generic value";
