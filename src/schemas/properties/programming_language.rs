	...
	impl<'de> Deserialize<'de> for ProgrammingLanguageProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				ProgrammingLanguageProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property programmingLanguage or was able to be deserialized into a generic value";
