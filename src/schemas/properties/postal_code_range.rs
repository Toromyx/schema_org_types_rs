...
mod serde {
	...
	use super::*;
	impl Serialize for PostalCodeRangeProperty {
		fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
		where
			S: Serializer,
		{
			match *self {
				PostalCodeRangeProperty::PostalCodeRangeSpecification(ref inner) => {
	...
	impl<'de> Deserialize<'de> for PostalCodeRangeProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				<PostalCodeRangeSpecification as Deserialize>::deserialize(deserializer),
				PostalCodeRangeProperty::PostalCodeRangeSpecification,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
