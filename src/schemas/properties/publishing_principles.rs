...
mod serde {
	...
	use super::*;
	impl Serialize for PublishingPrinciplesProperty {
		fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
		where
			S: Serializer,
		{
			match *self {
				PublishingPrinciplesProperty::CreativeWork(ref inner) => {
	...
	impl<'de> Deserialize<'de> for PublishingPrinciplesProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				PublishingPrinciplesProperty::CreativeWork,
			) {
				return Ok(ok);
			}
			if let Ok(ok) = Result::map(
				<Url as Deserialize>::deserialize(deserializer),
				PublishingPrinciplesProperty::Url,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
