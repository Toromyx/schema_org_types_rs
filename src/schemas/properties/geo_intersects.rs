	...
	impl Serialize for GeoIntersectsProperty {
			...
			S: Serializer,
		{
			match *self {
				...
				GeoIntersectsProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for GeoIntersectsProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				GeoIntersectsProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property geoIntersects or was able to be deserialized into a generic value";
