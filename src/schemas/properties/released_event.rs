	...
	impl Serialize for ReleasedEventProperty {
			...
			S: Serializer,
		{
			match *self {
				...
				ReleasedEventProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for ReleasedEventProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				ReleasedEventProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property releasedEvent or was able to be deserialized into a generic value";
