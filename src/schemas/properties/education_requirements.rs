	...
	impl Serialize for EducationRequirementsProperty {
			...
			S: Serializer,
		{
			match *self {
				...
				EducationRequirementsProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for EducationRequirementsProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				EducationRequirementsProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property educationRequirements or was able to be deserialized into a generic value";
