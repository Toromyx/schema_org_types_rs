...
mod serde {
	...
	impl Serialize for AcceptedPaymentMethodProperty {
		...
		{
			match *self {
				AcceptedPaymentMethodProperty::LoanOrCredit(ref inner) => {
	...
	impl<'de> Deserialize<'de> for AcceptedPaymentMethodProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				AcceptedPaymentMethodProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property acceptedPaymentMethod or was able to be deserialized into a generic value";
