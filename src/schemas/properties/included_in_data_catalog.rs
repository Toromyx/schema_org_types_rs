...
mod serde {
	...
	use super::*;
	impl Serialize for IncludedInDataCatalogProperty {
	...
	impl<'de> Deserialize<'de> for IncludedInDataCatalogProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				IncludedInDataCatalogProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property includedInDataCatalog or was able to be deserialized into a generic value";
			#[cfg(any(not(feature = "fallible"), not(feature = "serde")))]
			const CUSTOM_ERROR: &str =
				"data did not match any variant of schema.org property includedInDataCatalog";
