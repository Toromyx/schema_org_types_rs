	...
	impl Serialize for ByMonthDayProperty {
			...
			S: Serializer,
		{
			match *self {
				...
				ByMonthDayProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for ByMonthDayProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				ByMonthDayProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property byMonthDay or was able to be deserialized into a generic value";
