	...
	impl Serialize for MathExpressionProperty {
			...
			S: Serializer,
		{
			match *self {
				...
				MathExpressionProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for MathExpressionProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				MathExpressionProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property mathExpression or was able to be deserialized into a generic value";
