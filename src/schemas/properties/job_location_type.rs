	...
	impl Serialize for JobLocationTypeProperty {
			...
			S: Serializer,
		{
			match *self {
				...
				JobLocationTypeProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for JobLocationTypeProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				JobLocationTypeProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property jobLocationType or was able to be deserialized into a generic value";
