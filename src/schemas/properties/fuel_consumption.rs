...
mod serde {
	...
	impl Serialize for FuelConsumptionProperty {
		...
		{
			match *self {
				FuelConsumptionProperty::QuantitativeValue(ref inner) => {
					inner.serialize(serializer)
				}
				#[cfg(all(feature = "fallible", feature = "serde"))]
				FuelConsumptionProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for FuelConsumptionProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				FuelConsumptionProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property fuelConsumption or was able to be deserialized into a generic value";
