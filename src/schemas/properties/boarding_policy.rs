...
mod serde {
	...
	impl Serialize for BoardingPolicyProperty {
		...
		{
			match *self {
				BoardingPolicyProperty::BoardingPolicyType(ref inner) => {
					inner.serialize(serializer)
				}
				#[cfg(all(feature = "fallible", feature = "serde"))]
				BoardingPolicyProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for BoardingPolicyProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				BoardingPolicyProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property boardingPolicy or was able to be deserialized into a generic value";
