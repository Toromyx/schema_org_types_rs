...
mod serde {
	...
	impl Serialize for ItemDefectReturnFeesProperty {
		...
		{
			match *self {
				ItemDefectReturnFeesProperty::ReturnFeesEnumeration(ref inner) => {
					inner.serialize(serializer)
				}
				#[cfg(all(feature = "fallible", feature = "serde"))]
				ItemDefectReturnFeesProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for ItemDefectReturnFeesProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				ItemDefectReturnFeesProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property itemDefectReturnFees or was able to be deserialized into a generic value";
