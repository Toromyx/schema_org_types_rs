	...
	impl Serialize for TotalPriceProperty {
			...
			S: Serializer,
		{
			match *self {
				...
				TotalPriceProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for TotalPriceProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				TotalPriceProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property totalPrice or was able to be deserialized into a generic value";
