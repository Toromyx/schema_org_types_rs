	...
	impl Serialize for TaxonomicRangeProperty {
			...
			S: Serializer,
		{
			match *self {
				...
				TaxonomicRangeProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for TaxonomicRangeProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				TaxonomicRangeProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property taxonomicRange or was able to be deserialized into a generic value";
