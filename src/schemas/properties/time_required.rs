	...
	impl Serialize for TimeRequiredProperty {
			...
			S: Serializer,
		{
			match *self {
				...
				TimeRequiredProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for TimeRequiredProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				TimeRequiredProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property timeRequired or was able to be deserialized into a generic value";
