	...
	impl Serialize for QuarantineGuidelinesProperty {
			...
			S: Serializer,
		{
			match *self {
				...
				QuarantineGuidelinesProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for QuarantineGuidelinesProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				QuarantineGuidelinesProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property quarantineGuidelines or was able to be deserialized into a generic value";
