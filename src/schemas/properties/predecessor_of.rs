	...
	impl Serialize for PredecessorOfProperty {
			...
			S: Serializer,
		{
			match *self {
				...
				PredecessorOfProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for PredecessorOfProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				PredecessorOfProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property predecessorOf or was able to be deserialized into a generic value";
