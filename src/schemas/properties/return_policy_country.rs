	...
	impl Serialize for ReturnPolicyCountryProperty {
			...
			S: Serializer,
		{
			match *self {
				...
				ReturnPolicyCountryProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for ReturnPolicyCountryProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				ReturnPolicyCountryProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property returnPolicyCountry or was able to be deserialized into a generic value";
