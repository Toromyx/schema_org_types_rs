	...
	impl Serialize for InstallUrlProperty {
			...
			S: Serializer,
		{
			match *self {
				...
				InstallUrlProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for InstallUrlProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				InstallUrlProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property installUrl or was able to be deserialized into a generic value";
