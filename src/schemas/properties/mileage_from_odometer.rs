...
mod serde {
	...
	impl Serialize for MileageFromOdometerProperty {
		...
		{
			match *self {
				MileageFromOdometerProperty::QuantitativeValue(ref inner) => {
					inner.serialize(serializer)
				}
				#[cfg(all(feature = "fallible", feature = "serde"))]
				MileageFromOdometerProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for MileageFromOdometerProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				MileageFromOdometerProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property mileageFromOdometer or was able to be deserialized into a generic value";
