	...
	impl Serialize for CcRecipientProperty {
			...
			S: Serializer,
		{
			match *self {
				...
				CcRecipientProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for CcRecipientProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				CcRecipientProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property ccRecipient or was able to be deserialized into a generic value";
