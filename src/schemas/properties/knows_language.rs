	...
	impl Serialize for KnowsLanguageProperty {
			...
			S: Serializer,
		{
			match *self {
				...
				KnowsLanguageProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for KnowsLanguageProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				KnowsLanguageProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property knowsLanguage or was able to be deserialized into a generic value";
