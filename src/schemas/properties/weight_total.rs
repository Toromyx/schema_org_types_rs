	...
	impl Serialize for WeightTotalProperty {
			...
			S: Serializer,
		{
			match *self {
				...
				WeightTotalProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for WeightTotalProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				WeightTotalProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property weightTotal or was able to be deserialized into a generic value";
