	...
	impl Serialize for BreadcrumbProperty {
			...
			S: Serializer,
		{
			match *self {
				...
				BreadcrumbProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for BreadcrumbProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				BreadcrumbProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property breadcrumb or was able to be deserialized into a generic value";
