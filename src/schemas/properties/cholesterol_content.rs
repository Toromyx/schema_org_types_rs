	...
	impl Serialize for CholesterolContentProperty {
			...
			S: Serializer,
		{
			match *self {
				...
				CholesterolContentProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for CholesterolContentProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				CholesterolContentProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property cholesterolContent or was able to be deserialized into a generic value";
