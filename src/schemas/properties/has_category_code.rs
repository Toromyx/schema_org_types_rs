	...
	impl Serialize for HasCategoryCodeProperty {
			...
			S: Serializer,
		{
			match *self {
				...
				HasCategoryCodeProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for HasCategoryCodeProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				HasCategoryCodeProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property hasCategoryCode or was able to be deserialized into a generic value";
