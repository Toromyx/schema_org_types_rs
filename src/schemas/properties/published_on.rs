	...
	impl Serialize for PublishedOnProperty {
			...
			S: Serializer,
		{
			match *self {
				...
				PublishedOnProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for PublishedOnProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				PublishedOnProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property publishedOn or was able to be deserialized into a generic value";
