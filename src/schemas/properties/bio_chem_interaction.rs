	...
	impl Serialize for BioChemInteractionProperty {
			...
			S: Serializer,
		{
			match *self {
				...
				BioChemInteractionProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for BioChemInteractionProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				BioChemInteractionProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property bioChemInteraction or was able to be deserialized into a generic value";
