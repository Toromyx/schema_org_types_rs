	...
	impl Serialize for BroadcastFrequencyProperty {
			...
			S: Serializer,
		{
			match *self {
				...
				BroadcastFrequencyProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for BroadcastFrequencyProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				BroadcastFrequencyProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property broadcastFrequency or was able to be deserialized into a generic value";
