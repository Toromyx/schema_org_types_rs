...
mod serde {
	...
	impl Serialize for LegislationJurisdictionProperty {
		...
		{
			match *self {
				LegislationJurisdictionProperty::AdministrativeArea(ref inner) => {
					inner.serialize(serializer)
				}
				LegislationJurisdictionProperty::Text(ref inner) => inner.serialize(serializer),
				#[cfg(all(feature = "fallible", feature = "serde"))]
				LegislationJurisdictionProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for LegislationJurisdictionProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				LegislationJurisdictionProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property legislationJurisdiction or was able to be deserialized into a generic value";
