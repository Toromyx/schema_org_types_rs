	...
	impl Serialize for DeliveryAddressProperty {
			...
			S: Serializer,
		{
			match *self {
				...
				DeliveryAddressProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for DeliveryAddressProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				DeliveryAddressProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property deliveryAddress or was able to be deserialized into a generic value";
