	...
	impl Serialize for ChemicalCompositionProperty {
			...
			S: Serializer,
		{
			match *self {
				...
				ChemicalCompositionProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for ChemicalCompositionProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				ChemicalCompositionProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property chemicalComposition or was able to be deserialized into a generic value";
