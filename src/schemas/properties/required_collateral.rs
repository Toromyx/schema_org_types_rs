	...
	impl Serialize for RequiredCollateralProperty {
			...
			S: Serializer,
		{
			match *self {
				...
				RequiredCollateralProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for RequiredCollateralProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				RequiredCollateralProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property requiredCollateral or was able to be deserialized into a generic value";
