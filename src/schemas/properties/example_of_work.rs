	...
	impl Serialize for ExampleOfWorkProperty {
			...
			S: Serializer,
		{
			match *self {
				...
				ExampleOfWorkProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for ExampleOfWorkProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				ExampleOfWorkProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property exampleOfWork or was able to be deserialized into a generic value";
