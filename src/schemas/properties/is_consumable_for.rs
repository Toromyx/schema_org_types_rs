	...
	impl Serialize for IsConsumableForProperty {
			...
			S: Serializer,
		{
			match *self {
				...
				IsConsumableForProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for IsConsumableForProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				IsConsumableForProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property isConsumableFor or was able to be deserialized into a generic value";
