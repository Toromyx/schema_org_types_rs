	...
	impl Serialize for TitleEidrProperty {
			...
			S: Serializer,
		{
			match *self {
				...
				TitleEidrProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for TitleEidrProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				TitleEidrProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property titleEIDR or was able to be deserialized into a generic value";
