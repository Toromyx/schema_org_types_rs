	...
	impl Serialize for BillingIncrementProperty {
			...
			S: Serializer,
		{
			match *self {
				...
				BillingIncrementProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for BillingIncrementProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				BillingIncrementProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property billingIncrement or was able to be deserialized into a generic value";
