...
mod serde {
	...
	impl Serialize for RelatedAnatomyProperty {
		...
		{
			match *self {
				RelatedAnatomyProperty::AnatomicalStructure(ref inner) => {
					inner.serialize(serializer)
				}
				RelatedAnatomyProperty::AnatomicalSystem(ref inner) => inner.serialize(serializer),
				#[cfg(all(feature = "fallible", feature = "serde"))]
				RelatedAnatomyProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for RelatedAnatomyProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				RelatedAnatomyProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property relatedAnatomy or was able to be deserialized into a generic value";
