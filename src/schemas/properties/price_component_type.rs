	...
	impl Serialize for PriceComponentTypeProperty {
			...
			S: Serializer,
		{
			match *self {
				...
				PriceComponentTypeProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for PriceComponentTypeProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				PriceComponentTypeProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property priceComponentType or was able to be deserialized into a generic value";
