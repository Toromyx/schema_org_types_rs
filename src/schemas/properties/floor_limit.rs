	...
	impl Serialize for FloorLimitProperty {
			...
			S: Serializer,
		{
			match *self {
				...
				FloorLimitProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for FloorLimitProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				FloorLimitProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property floorLimit or was able to be deserialized into a generic value";
