	...
	impl Serialize for SuitableForDietProperty {
			...
			S: Serializer,
		{
			match *self {
				...
				SuitableForDietProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for SuitableForDietProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				SuitableForDietProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property suitableForDiet or was able to be deserialized into a generic value";
