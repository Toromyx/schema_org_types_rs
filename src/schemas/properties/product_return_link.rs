	...
	impl Serialize for ProductReturnLinkProperty {
			...
			S: Serializer,
		{
			match *self {
				...
				ProductReturnLinkProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for ProductReturnLinkProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				ProductReturnLinkProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property productReturnLink or was able to be deserialized into a generic value";
