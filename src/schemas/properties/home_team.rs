	...
	impl Serialize for HomeTeamProperty {
			...
			S: Serializer,
		{
			match *self {
				...
				HomeTeamProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for HomeTeamProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				HomeTeamProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property homeTeam or was able to be deserialized into a generic value";
