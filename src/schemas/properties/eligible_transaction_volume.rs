...
mod serde {
	...
	impl Serialize for EligibleTransactionVolumeProperty {
		...
		{
			match *self {
				EligibleTransactionVolumeProperty::PriceSpecification(ref inner) => {
					inner.serialize(serializer)
				}
				#[cfg(all(feature = "fallible", feature = "serde"))]
				EligibleTransactionVolumeProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for EligibleTransactionVolumeProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				EligibleTransactionVolumeProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property eligibleTransactionVolume or was able to be deserialized into a generic value";
