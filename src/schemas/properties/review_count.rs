	...
	impl Serialize for ReviewCountProperty {
			...
			S: Serializer,
		{
			match *self {
				...
				ReviewCountProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for ReviewCountProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				ReviewCountProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property reviewCount or was able to be deserialized into a generic value";
