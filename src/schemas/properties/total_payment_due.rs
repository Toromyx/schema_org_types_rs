	...
	impl Serialize for TotalPaymentDueProperty {
			...
			S: Serializer,
		{
			match *self {
				...
				TotalPaymentDueProperty::PriceSpecification(ref inner) => {
					inner.serialize(serializer)
				}
				#[cfg(all(feature = "fallible", feature = "serde"))]
				TotalPaymentDueProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for TotalPaymentDueProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				TotalPaymentDueProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property totalPaymentDue or was able to be deserialized into a generic value";
