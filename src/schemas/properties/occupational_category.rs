...
mod serde {
	...
	impl Serialize for OccupationalCategoryProperty {
		...
		{
			match *self {
				OccupationalCategoryProperty::CategoryCode(ref inner) => {
					inner.serialize(serializer)
				}
				OccupationalCategoryProperty::Text(ref inner) => inner.serialize(serializer),
				#[cfg(all(feature = "fallible", feature = "serde"))]
				OccupationalCategoryProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for OccupationalCategoryProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				OccupationalCategoryProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property occupationalCategory or was able to be deserialized into a generic value";
