	...
	impl<'de> Deserialize<'de> for RequiresSubscriptionProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				RequiresSubscriptionProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property requiresSubscription or was able to be deserialized into a generic value";
