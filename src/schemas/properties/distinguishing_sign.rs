...
mod serde {
	...
	impl Serialize for DistinguishingSignProperty {
		...
		{
			match *self {
				DistinguishingSignProperty::MedicalSignOrSymptom(ref inner) => {
					inner.serialize(serializer)
				}
				#[cfg(all(feature = "fallible", feature = "serde"))]
				DistinguishingSignProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for DistinguishingSignProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				DistinguishingSignProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property distinguishingSign or was able to be deserialized into a generic value";
