...
mod serde {
	...
	impl Serialize for EarlyPrepaymentPenaltyProperty {
		...
		{
			match *self {
				EarlyPrepaymentPenaltyProperty::MonetaryAmount(ref inner) => {
					inner.serialize(serializer)
				}
				#[cfg(all(feature = "fallible", feature = "serde"))]
				EarlyPrepaymentPenaltyProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for EarlyPrepaymentPenaltyProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				EarlyPrepaymentPenaltyProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property earlyPrepaymentPenalty or was able to be deserialized into a generic value";
