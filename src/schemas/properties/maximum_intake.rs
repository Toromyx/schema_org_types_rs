...
mod serde {
	...
	impl Serialize for MaximumIntakeProperty {
		...
		{
			match *self {
				MaximumIntakeProperty::MaximumDoseSchedule(ref inner) => {
					inner.serialize(serializer)
				}
				#[cfg(all(feature = "fallible", feature = "serde"))]
				MaximumIntakeProperty::SerdeFail(ref inner) => inner.serialize(serializer),
			}
		}
	}
	impl<'de> Deserialize<'de> for MaximumIntakeProperty {
			...
			D: Deserializer<'de>,
		{
			...
			if let Ok(ok) = Result::map(
				...
				MaximumIntakeProperty::SerdeFail,
			) {
				return Ok(ok);
			}
			#[cfg(all(feature = "fallible", feature = "serde"))]
			const CUSTOM_ERROR: &str = "data did neither match any variant of schema.org property maximumIntake or was able to be deserialized into a generic value";
